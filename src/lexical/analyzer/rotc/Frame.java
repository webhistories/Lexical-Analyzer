/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package lexical.analyzer.rotc;


import java.awt.Toolkit;
import java.awt.event.WindowEvent;
import javax.swing.text.AttributeSet;
import java.util.ArrayList;
import java.util.logging.Level;
import java.util.logging.Logger;
import javax.swing.JOptionPane;
import javax.swing.text.BadLocationException;
import javax.swing.text.Document;
import javax.swing.text.StyleConstants;

/**
 *
 * @author PC
 */
public class Frame extends javax.swing.JFrame {

    /**
     * Creates new form Frame
     */
    public Frame() {
        initComponents();
    }
    
    public void close(){
     WindowEvent winClosingEvent = new WindowEvent(this,WindowEvent.WINDOW_CLOSING);
     Toolkit.getDefaultToolkit().getSystemEventQueue().postEvent(winClosingEvent);
    }

    /**
     * This method is called from within the constructor to initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is always
     * regenerated by the Form Editor.
     */
    @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        jFrame1 = new javax.swing.JFrame();
        jFrame2 = new javax.swing.JFrame();
        jMenu1 = new javax.swing.JMenu();
        jMenuBar1 = new javax.swing.JMenuBar();
        jMenu2 = new javax.swing.JMenu();
        jMenu3 = new javax.swing.JMenu();
        jTabbedPane5 = new javax.swing.JTabbedPane();
        Clear = new javax.swing.JLabel();
        Lexical = new javax.swing.JLabel();
        Exit = new javax.swing.JLabel();
        Clear1 = new javax.swing.JLabel();
        Syntax = new javax.swing.JLabel();
        Exit1 = new javax.swing.JLabel();
        jTabbedPane2 = new javax.swing.JTabbedPane();
        jScrollPane3 = new javax.swing.JScrollPane();
        LexicalTbl = new javax.swing.JTable();
        jScrollPane9 = new javax.swing.JScrollPane();
        SyntaxTbl = new javax.swing.JTable();
        jTabbedPane3 = new javax.swing.JTabbedPane();
        jScrollPane2 = new javax.swing.JScrollPane();
        ErrorTxt = new javax.swing.JEditorPane();
        jTabbedPane4 = new javax.swing.JTabbedPane();
        jScrollPane4 = new javax.swing.JScrollPane();
        ErrorTxt1 = new javax.swing.JEditorPane();
        jTabbedPane6 = new javax.swing.JTabbedPane();
        jScrollPane5 = new javax.swing.JScrollPane();
        ErrorTxt2 = new javax.swing.JEditorPane();
        jTabbedPane7 = new javax.swing.JTabbedPane();
        jScrollPane6 = new javax.swing.JScrollPane();
        ErrorTxt3 = new javax.swing.JEditorPane();
        jScrollPane7 = new javax.swing.JScrollPane();
        jTabbedPane8 = new javax.swing.JTabbedPane();
        jTabbedPane1 = new javax.swing.JTabbedPane();
        jPanel1 = new javax.swing.JPanel();
        jsp = new javax.swing.JScrollPane();
        jScrollPane1 = new javax.swing.JScrollPane();
        SourceTxt = new javax.swing.JTextArea();
        jLabel1 = new javax.swing.JLabel();

        javax.swing.GroupLayout jFrame1Layout = new javax.swing.GroupLayout(jFrame1.getContentPane());
        jFrame1.getContentPane().setLayout(jFrame1Layout);
        jFrame1Layout.setHorizontalGroup(
            jFrame1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGap(0, 400, Short.MAX_VALUE)
        );
        jFrame1Layout.setVerticalGroup(
            jFrame1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGap(0, 300, Short.MAX_VALUE)
        );

        javax.swing.GroupLayout jFrame2Layout = new javax.swing.GroupLayout(jFrame2.getContentPane());
        jFrame2.getContentPane().setLayout(jFrame2Layout);
        jFrame2Layout.setHorizontalGroup(
            jFrame2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGap(0, 400, Short.MAX_VALUE)
        );
        jFrame2Layout.setVerticalGroup(
            jFrame2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGap(0, 300, Short.MAX_VALUE)
        );

        jMenu1.setText("jMenu1");

        jMenu2.setText("File");
        jMenuBar1.add(jMenu2);

        jMenu3.setText("Edit");
        jMenuBar1.add(jMenu3);

        setDefaultCloseOperation(javax.swing.WindowConstants.DISPOSE_ON_CLOSE);
        setTitle("ROTCompiler ver12.0");
        addWindowListener(new java.awt.event.WindowAdapter() {
            public void windowActivated(java.awt.event.WindowEvent evt) {
                formWindowActivated(evt);
            }
        });
        getContentPane().setLayout(null);

        Clear.setForeground(new java.awt.Color(0, 255, 0));
        Clear.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                ClearMouseClicked(evt);
            }
        });
        getContentPane().add(Clear);
        Clear.setBounds(400, 0, 140, 50);

        Lexical.setForeground(new java.awt.Color(0, 255, 0));
        Lexical.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                LexicalMouseClicked(evt);
            }
        });
        getContentPane().add(Lexical);
        Lexical.setBounds(540, 0, 90, 60);

        Exit.setForeground(new java.awt.Color(0, 255, 0));
        Exit.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                ExitMouseClicked(evt);
            }
        });
        getContentPane().add(Exit);
        Exit.setBounds(740, 0, 140, 50);

        Clear1.setForeground(new java.awt.Color(0, 255, 0));
        Clear1.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                Clear1MouseClicked(evt);
            }
        });
        getContentPane().add(Clear1);
        Clear1.setBounds(400, 0, 140, 50);

        Syntax.setForeground(new java.awt.Color(0, 255, 0));
        Syntax.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                SyntaxMouseClicked(evt);
            }
        });
        getContentPane().add(Syntax);
        Syntax.setBounds(630, 0, 100, 60);

        Exit1.setForeground(new java.awt.Color(0, 255, 0));
        Exit1.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                Exit1MouseClicked(evt);
            }
        });
        getContentPane().add(Exit1);
        Exit1.setBounds(740, 0, 140, 50);

        LexicalTbl.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        LexicalTbl.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null},
                {null, null},
                {null, null},
                {null, null}
            },
            new String [] {
                "Lexeme", "Token"
            }
        ));
        jScrollPane3.setViewportView(LexicalTbl);

        jTabbedPane2.addTab("Lexical Table", jScrollPane3);

        SyntaxTbl.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        SyntaxTbl.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null},
                {null, null},
                {null, null},
                {null, null}
            },
            new String [] {
                "Production", ""
            }
        ));
        jScrollPane9.setViewportView(SyntaxTbl);

        jTabbedPane2.addTab("Syntax Table", jScrollPane9);

        getContentPane().add(jTabbedPane2);
        jTabbedPane2.setBounds(960, 80, 300, 570);

        ErrorTxt.setContentType("text/html");
        jScrollPane2.setViewportView(ErrorTxt);

        jTabbedPane3.addTab("Error", jScrollPane2);

        getContentPane().add(jTabbedPane3);
        jTabbedPane3.setBounds(30, 510, 910, 140);

        ErrorTxt1.setContentType("text/html");
        jScrollPane4.setViewportView(ErrorTxt1);

        jTabbedPane4.addTab("Error", jScrollPane4);

        getContentPane().add(jTabbedPane4);
        jTabbedPane4.setBounds(30, 510, 440, 140);

        ErrorTxt2.setContentType("text/html");
        jScrollPane5.setViewportView(ErrorTxt2);

        jTabbedPane6.addTab("Error", jScrollPane5);

        getContentPane().add(jTabbedPane6);
        jTabbedPane6.setBounds(30, 510, 440, 140);

        ErrorTxt3.setContentType("text/html");
        jScrollPane6.setViewportView(ErrorTxt3);

        jTabbedPane7.addTab("Error", jScrollPane6);

        getContentPane().add(jTabbedPane7);
        jTabbedPane7.setBounds(30, 510, 440, 140);
        getContentPane().add(jScrollPane7);
        jScrollPane7.setBounds(480, 530, 2, 2);
        getContentPane().add(jTabbedPane8);
        jTabbedPane8.setBounds(480, 530, 460, 120);

        jTabbedPane1.setBackground(new java.awt.Color(204, 153, 0));

        SourceTxt.setColumns(20);
        SourceTxt.setFont(new java.awt.Font("Courier New", 0, 18)); // NOI18N
        SourceTxt.setRows(5);
        jScrollPane1.setViewportView(SourceTxt);

        jsp.setViewportView(jScrollPane1);

        javax.swing.GroupLayout jPanel1Layout = new javax.swing.GroupLayout(jPanel1);
        jPanel1.setLayout(jPanel1Layout);
        jPanel1Layout.setHorizontalGroup(
            jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(jPanel1Layout.createSequentialGroup()
                .addContainerGap()
                .addComponent(jsp, javax.swing.GroupLayout.DEFAULT_SIZE, 885, Short.MAX_VALUE)
                .addContainerGap())
        );
        jPanel1Layout.setVerticalGroup(
            jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(jPanel1Layout.createSequentialGroup()
                .addContainerGap()
                .addComponent(jsp, javax.swing.GroupLayout.DEFAULT_SIZE, 360, Short.MAX_VALUE)
                .addContainerGap())
        );

        jTabbedPane1.addTab("Source", jPanel1);

        getContentPane().add(jTabbedPane1);
        jTabbedPane1.setBounds(30, 80, 910, 410);

        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/lexical/analyzer/rotc/pubg ui.jpg"))); // NOI18N
        getContentPane().add(jLabel1);
        jLabel1.setBounds(-10, 0, 1280, 720);

        pack();
    }// </editor-fold>//GEN-END:initComponents

    private void formWindowActivated(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowActivated
        // TODO add your handling code here:
        // Lines.append("1\n");
    }//GEN-LAST:event_formWindowActivated

    private void ClearMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_ClearMouseClicked
        close();
        Frame frame = new Frame();
        frame.setExtendedState(Home.MAXIMIZED_BOTH);
        frame.setVisible(true);
    }//GEN-LAST:event_ClearMouseClicked

    private void LexicalMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_LexicalMouseClicked
        ErrorTxt.setText(null);

        //populate LexicalTbl
        javax.swing.table.DefaultTableModel model = (javax.swing.table.DefaultTableModel) LexicalTbl.getModel();
        model.setRowCount(0);

        ArrayList<Lexical> list = null;
        try {
            list = ListLexical();
        } catch (BadLocationException ex) {
            Logger.getLogger(Frame.class.getName()).log(Level.SEVERE, null, ex);
        }
        Object rowData[] = new Object[(list.size())+1];
        //    System.out.print(list.size());
        for(int i = 0; i <list.size(); i++)
        {
            rowData[0] = list.get(i).Lexeme;
            rowData[1] = list.get(i).Token;

            model.addRow(rowData);
        }
        System.out.println(list.size()+"\n");
     if(ErrorTxt.getDocument().getLength()==0){
            JOptionPane.showMessageDialog(null, "No Lexical Error found!");
        }
        else{
            JOptionPane.showMessageDialog(null, "Lexical  Error found!","Error",JOptionPane.ERROR_MESSAGE);
        }
    }//GEN-LAST:event_LexicalMouseClicked

    private void ExitMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_ExitMouseClicked
        Home home = new Home();
        home.setExtendedState(Home.MAXIMIZED_BOTH);
        home.setVisible(true);
        close();
    }//GEN-LAST:event_ExitMouseClicked

    private void Clear1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_Clear1MouseClicked
        close();
        Frame frame = new Frame();
        frame.setExtendedState(Home.MAXIMIZED_BOTH);
        frame.setVisible(true);
    }//GEN-LAST:event_Clear1MouseClicked

    private void Exit1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_Exit1MouseClicked
        Home home = new Home();
        home.setExtendedState(Home.MAXIMIZED_BOTH);
        home.setVisible(true);
        close();
    }//GEN-LAST:event_Exit1MouseClicked

    private void SyntaxMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_SyntaxMouseClicked
        ErrorTxt.setText(null);

        //populate LexicalTbl
        javax.swing.table.DefaultTableModel model = (javax.swing.table.DefaultTableModel) SyntaxTbl.getModel();
        model.setRowCount(0);

        ArrayList<Syntax> listSyntax = null;
        try {
            listSyntax = SynAnalyze();
        } catch (BadLocationException ex) {
            Logger.getLogger(Frame.class.getName()).log(Level.SEVERE, null, ex);
        }
        Object rowData[] = new Object[(listSyntax.size())+1];
        //    System.out.print(list.size());
        for(int i = 0; i <listSyntax.size(); i++)
        {
            rowData[0] = listSyntax.get(i).Production;
            rowData[1] = listSyntax.get(i).Syntax;

            model.addRow(rowData);
        }
        System.out.println(listSyntax.size()+"\n");

        if(ErrorTxt.getDocument().getLength()==0){
            JOptionPane.showMessageDialog(null, "No Syntax Error found!");
        }
        else{
            JOptionPane.showMessageDialog(null, "Syntax Error found!","Error",JOptionPane.ERROR_MESSAGE);
        }
        //SynAnalyze();
    }//GEN-LAST:event_SyntaxMouseClicked
    
    public class Lexical{
       public String Lexeme;
       public String Token;
       public String Line;
        public Lexical(String Lexeme, String Token, String Line)
        {
            this.Lexeme = Lexeme;
            this.Token = Token;
            this.Line= Line;
        
        }
    }
    
 // creates a class and use it to populate the arraylist
    public ArrayList ListLexical() throws BadLocationException
    {
    //    SourceTxt.setText("1:\t");
        char[] chars = SourceTxt.getText().toCharArray();
        //char[] line = SourceTxt.getText().toCharArray();
        
        String id="";
       String fontfamily = ErrorTxt.getFont().getFamily();//font for error list
       int i = 0, flag = 0, IdentifierCount=0, lineNumber=1, IntFlag=0, LexError=0;
       int delimCheck =0, zerowatch=0;
      ArrayList<Lexical> list = new ArrayList<Lexical>();
//     /list.add(new Lexical("", "", ""));
          Document doc = ErrorTxt.getDocument();
       
       
     while(flag==0){
     
      IdentifierCount=0;
      
      if(chars[i]=='a'){
          flag = 1;
           while(flag==1){
               flag=1;
               id = String.valueOf(chars[i]);
                 i++;
                 if(chars.length==i){//if no delimiter found
//                     doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
//                             + ", Line "+lineNumber+". No delimiter found.\n" , null);
                   //  ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> LEXICAL ERROR on <span style=\"color:red\">"+id+""
                     //        + "</span>,<span style=\"color:red\"> Line "+lineNumber+"</span>. No delimiter found.");
                    list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                     break;
                    }//if
                if(chars[i]=='s'){
                 id = id.concat(String.valueOf(chars[i]));
                 i++;
                  if(chars.length==i){//if no delimiter found

//                        doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
//                             + ", Line "+lineNumber+". No delimiter found.\n" , null);
//                       list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                     break;
                    }//if
                }
 //=========================================================ATTENTION=========/
                else if(chars[i]=='t'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
 list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                     break;
  }
   if(chars[i]=='t'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
 list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                     break;
  }}
    else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='e'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
    list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                     break;
  }}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='n'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
   list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                     break;
  }}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='t'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
   list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                     break;
  }}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='i'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
  list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                     break;
  }}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='o'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
    list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                     break;
  }}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='n'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
    list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                     break;
                 
  }}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  //============ATTENTION DELIMITER==========
                if(chars[i]==':'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("attention","attention", String.valueOf(lineNumber)));
                        id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                         flag=8;
                       // System.out.print(flag);
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("attention","attention", String.valueOf(lineNumber)));
                    
                    if(chars[i]=='\n'||chars[i]==' '){
                        id="";
                          
                     id = id.concat(String.valueOf(chars[i-1]));
                     list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;
                    }
                    else{
                        
                      id=":";
                      
                       doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. '[SPACE]' or '[NEWLINE]' is expected.\n" , null);
                       list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                     i++;
                        if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                    }
                    }
                 }
                  else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER OF ASSEMBLE
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. [SPACE] or [NEWLINE] is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }//if
 //===========================================================================
                else{
                    IdentifierCount++;
                    flag=3;
                    break; //proceed to Identifier
                }
                if(chars[i]=='s'){
                id = id.concat(String.valueOf(chars[i]));
                i++;
                if(chars.length==i){//if no delimiter found
               
                      list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;
                    }//if
                }
                else{
                    IdentifierCount++;
                    flag=3;
                    break; //proceed to Identifier
                }
                 if(chars[i]=='e'){
                  id = id.concat(String.valueOf(chars[i]));
                 i++;
                 if(chars.length==i){//if no delimiter found
                  
                list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;
                    }//if
                 }
                  else{
                     IdentifierCount++;
                    flag=3;
                    break; //proceed to Identifier
                }
                  if(chars[i]=='m'){
                    id = id.concat(String.valueOf(chars[i]));
                  i++;
                  if(chars.length==i){//if no delimiter found
                     
                     list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;
                    }//if
                  }
                   else{
                    IdentifierCount++;
                    flag=3;
                    break; //proceed to Identifier
                }
                 if(chars[i]=='b'){
                 id = id.concat(String.valueOf(chars[i]));
                  i++;
                  if(chars.length==i){//if no delimiter found
                    
                     list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;
                    }//if
                 }
                  else{
                    IdentifierCount++;
                    flag=3;
                    break; //proceed to Identifier
                }
                  if(chars[i]=='l'){
                 id = id.concat(String.valueOf(chars[i]));
                   i++;
                   if(chars.length==i){//if no delimiter found
                    
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;
                    }//if
                  }
                  else{
                    IdentifierCount++;
                    flag=3;
                    break; //proceed to Identifier
                }
                   if(chars[i]=='e'){
                 id = id.concat(String.valueOf(chars[i]));
                 i++;
                 
                    if(chars.length==i){//if no delimiter found
                   
                     list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;
                    }
                   
                   }
                    else{
                    IdentifierCount++;
                    flag=3;
                    break; //proceed to Identifier
                }
                 if(chars[i]==' '){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("assemble","assemble", String.valueOf(lineNumber)));
                         flag=8;
                        System.out.print(flag);
                    //break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("assemble","assemble", String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;}
                 }
        else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER OF ASSEMBLE
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. [SPACE] is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                 
      }//while flag=1
         //  System.out.print("id is: "+id+" ");
      }//if chars [i]
      //=====================================================BOOL=========
      else if(chars[i]=='b'){
                flag = 1;
           while(flag==1){
               flag=1;id = String.valueOf(chars[i]);i++;
             if(chars.length==i){//if no delimiter found
                     
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}//if
                             else if(chars[i]=='o'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
   if(chars[i]=='o'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
   }
    else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='l'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  //============BOOL DELIMITER==========
                if(chars[i]==' '){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("bool","bool", String.valueOf(lineNumber)));
                         flag=8;
                        System.out.print(flag);
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("bool","bool", String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;}
                 }
               else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. [SPACE] is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }//if
                 else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
      }//while
      }//else if
    //=============================================================BOOL=======
         //=====================================================CHR=========
      else if(chars[i]=='c'){
                flag = 1;
           while(flag==1){
               flag=1;id = String.valueOf(chars[i]);i++;
             if(chars.length==i){//if no delimiter found
                      
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}//if
                             else if(chars[i]=='h'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
                             }
   if(chars[i]=='r'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
  //============CHR DELIMITER==========
                if(chars[i]==' '){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("chr","chr", String.valueOf(lineNumber)));
                         flag=8;
                        System.out.print(flag);
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("chr","chr", String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;}
                 }
               else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. [SPACE] is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                
                }//if ch==r
 //=========================================================CHECKPONT=========/
                else if(chars[i]=='e'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;
  }
   if(chars[i]=='c'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
    else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='k'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break; /**proceed to Identifier**/}
  if(chars[i]=='p'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='o'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='i'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='n'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
    if(chars[i]=='t'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
      
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  //============CHECKPOINT DELIMITER==========
                if(chars[i]==';'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("checkpoint","checkpoint", String.valueOf(lineNumber)));
                       id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                         flag=8;
                        
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("checkpoint","checkpoint",String.valueOf(lineNumber)));
                    id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;}
                 }
                 else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ';' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }//else if ch==e
                
 //========================================================CHECKPOINT===================
                else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
                }//if ch==h
 //=========================================================COMMAND=========/
                else if(chars[i]=='o'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
   if(chars[i]=='m'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
  if(chars[i]=='m'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='a'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='n'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='d'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
    if(chars[i]=='('){id="";id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  //============COMMAND DELIMITER==========
                if(chars[i]==')'){
                i++;
                    if(i==(chars.length)){
                        
                        list.add(new Lexical("command","command",String.valueOf(lineNumber)));
                        
                         id="";
                        id = id.concat(String.valueOf(chars[i-2]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                         id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                         flag=8;
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("command","command",String.valueOf(lineNumber)));
                     id="";
                        id = id.concat(String.valueOf(chars[i-2]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                         id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        if(chars[i]=='\n'||chars[i]==' '){
                        id="";
                          
                    // id = id.concat(String.valueOf(chars[i-1]));
                    // list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;
                    }
                    else{
                        
                      //id=":";
                      
                       doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. '[SPACE]' or '[NEWLINE]' is expected.\n" , null);
                      // list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                     i++;
                        if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                    }
//                    flag=0;
//                    id="";
//                    break;
                    
                    }
                 }
             else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ')' is expected.\n" , null);
              list.add(new Lexical("command","command",String.valueOf(lineNumber)));
                     id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }// if ch==m
    
   //=====================================================CONST==============
   else if(chars[i]=='n'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
  if(chars[i]=='s'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='t'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}

  //============CONST DELIMITER==========
                if(chars[i]==' '){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("const","const",String.valueOf(lineNumber)));
                         flag=8;
                        System.out.print(flag);
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("const","const",String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;}
                 }
           else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. [SPACE] is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }// if ch==n
   //================================================CONST==================
      //=====================================================COVER==============
   else if(chars[i]=='v'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;
}
  if(chars[i]=='e'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;
}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='r'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;
}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}

  //============COVER DELIMITER==========
                if(chars[i]==';'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("cover","cover",String.valueOf(lineNumber)));
                         id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                         flag=8;
                break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("cover","cover",String.valueOf(lineNumber)));
                    id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;}
                 }
           else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ';' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }// if ch==v
   //=============================================COVER=======================
    else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
                }//else if ch==o
                
 //========================================================COMMAND===================       
                 else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
             
      }//while
      }//else if
    //=============================================================CHR=======
          //=====================================================DISMISS=========
      else if(chars[i]=='d'){
                flag = 1;
           while(flag==1){
               flag=1;id = String.valueOf(chars[i]);i++;
             if(chars.length==i){//if no delimiter found
                     
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}//if
                             else if(chars[i]=='i'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
   if(chars[i]=='s'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
    else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='m'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='i'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='s'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='s'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){list.add(new Lexical("dismiss","dismiss",String.valueOf(lineNumber))); flag=8;break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
   
  //============DISMISS DELIMITER==========
                if(chars[i]==' '||chars[i]=='\n'||chars[i]=='\t'
              ||chars[i]=='\r'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("dismiss","dismiss",String.valueOf(lineNumber)));
                        
                         flag=8;
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("dismiss","dismiss",String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;}
                 }
            /*else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'){//ERROR TRAP FOR DELIMITER 
             ErrorTxt.setText("'"+chars[i]+"' is not a valid character");
                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> LEXICAL ERROR on <span style=\"color:red\">"+id+""
                             + "</span>,<span style=\"color:red\"> Line "+lineNumber+"</span>. '"+chars[i]+"' is not a valid delimiter.");
                
             break;
                 }*/
        
                     else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ' ', '\\n', '\\t' or '\\r' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }//if char==d(i)smiss
   //=====================================================DO==============
   else if(chars[i]=='o'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
  //============DO DELIMITER==========
                if(chars[i]==' '){
                i++;
                    if(i==(chars.length) ||chars[i]==' '){
                        list.add(new Lexical("do","do",String.valueOf(lineNumber)));
                         flag=8;
                        System.out.print(flag);
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("do","do",String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;}
                 }
                        else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ' ', '\\n', '\\t' or '\\r' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }// if ch==o
   //=============================================DO=======================
                 else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
      }//while
      }//else if
    //=============================================================DISMISS=======
          //=====================================================ELSE=========
      else if(chars[i]=='e'){
                flag = 1;
           while(flag==1){
               flag=1;id = String.valueOf(chars[i]);i++;
             if(chars.length==i){//if no delimiter found
                    
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;
}//if
                             else if(chars[i]=='l'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;
}
   if(chars[i]=='s'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;
}}
    else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='e'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;
}
  
  //============ELSE DELIMITER==========
                if(chars[i]==' '||chars[i]=='\n'||chars[i]=='\t'
              ||chars[i]=='\r'||chars[i]=='{'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("else","else",String.valueOf(lineNumber)));
                        if(chars[i-1]=='{'){
                        id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));}
                         flag=8;
                        System.out.print(flag);
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("else","else",String.valueOf(lineNumber)));
                    if(chars[i-1]=='{'){
                        id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));}
                    flag=0;
                    id="";
                    break;}
                 }
         
//              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on"+id+""
//                             + ", Line "+lineNumber+". '"+chars[i]+"' is expected.\n" , null);
                    else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ' ', '\\n', '\\t', '\\r' or '{'  is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
             //=====================================================elseif==============
   if(chars[i]=='i'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
  if(chars[i]=='f'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  
  //============ELSEIF DELIMITER==========
                if(chars[i]==' '||chars[i]=='('||chars[i]=='\r'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("elseif","elseif",String.valueOf(lineNumber)));
                          if(chars[i-1]=='('){
                        id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));}
                         flag=8;
                        System.out.print(flag);
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("elseif","elseif",String.valueOf(lineNumber)));
                      if(chars[i-1]=='('){
                        id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));}
                    flag=0;
                    id="";
                    break;}
                 }
            /*else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'){//ERROR TRAP FOR DELIMITER
             ErrorTxt.setText("'"+chars[i]+"' is not a valid character");
                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> LEXICAL ERROR on <span style=\"color:red\">"+id+""
                             + "</span>,<span style=\"color:red\"> Line "+lineNumber+"</span>. '"+chars[i]+"' is not a valid delimiter. '(' is expected.");
                
             break;
                 }*/
                
                else if(chars[i]!='a'||chars[i]!='b'||chars[i]!='c'||chars[i]!='d'||chars[i]!='e'
                ||chars[i]!='f'||chars[i]!='g'||chars[i]!='h'||chars[i]!='i'||chars[i]!='j'
                ||chars[i]!='k'||chars[i]!='l'||chars[i]!='m'||chars[i]!='n'||chars[i]!='o'
                ||chars[i]!='p'||chars[i]!='q'||chars[i]!='r'||chars[i]!='s'||chars[i]!='t'
                ||chars[i]!='u'||chars[i]!='v'||chars[i]!='w'||chars[i]!='x'||chars[i]!='y'
                ||chars[i]!='z'||chars[i]!='_'||chars[i]!='1'||chars[i]!='2'||chars[i]!='3'
                ||chars[i]!='4'||chars[i]!='5'||chars[i]!='6'||chars[i]!='7'||chars[i]!='8'
                ||chars[i]!='9'){//ERROR TRAP FOR DELIMITER OF ASSEMBLE
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on"+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ' ', '(' or '\r' is expected.\n" , null);
               list.add(new Lexical("chr","chr", String.valueOf(lineNumber)));
                     i++;
                    
              if(i==(chars.length)){
                         id="";
                        
                    break;}
                     else{
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                    IdentifierCount++;
                    flag=3;
                    break; //proceed to Identifier
                }
                }// if ch==ELSEIF
   //=============================================ELSEIF======================= else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
                }//if char==else
                 else{
                    IdentifierCount++;
                    flag=3;
                    break; //proceed to Identifier
                }
                }//if char==else
  
               
   //=====================================================ENDMISSION==============
   else if(chars[i]=='n'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
  if(chars[i]=='d'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
   if(chars[i]=='m'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
    if(chars[i]=='i'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
     if(chars[i]=='s'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
      if(chars[i]=='s'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
       if(chars[i]=='i'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
        if(chars[i]=='o'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
         if(chars[i]=='n'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  
  //============ENDMISSION DELIMITER==========
                if(chars[i]==' '||chars[i]=='\n'||chars[i]=='\t'
              ||chars[i]=='\r'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("endmission","endmission",String.valueOf(lineNumber)));
                         flag=8;
                        System.out.print(flag);
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("endmission","endmission",String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;}
                 }
          
//              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on"+id+""
//                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter.   is expected.\n" , null);
              else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ' ', '\\n','\\t' or '\\r' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
            
                }// if ch==ENDMISSION
   //=============================================ENDMISSION======================= 
   //=====================================================EXECUTE==============
   else if(chars[i]=='x'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
  if(chars[i]=='e'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
   if(chars[i]=='c'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
    if(chars[i]=='u'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
     if(chars[i]=='t'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
      if(chars[i]=='e'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
   
  //============EXECUTE DELIMITER==========
                if(chars[i]==' '||chars[i]=='\r'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("execute","execute",String.valueOf(lineNumber)));
                         flag=8;
                        System.out.print(flag);
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("execute","execute",String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;}
                 }
        
                    
        else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ' ' or '\\r' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }// if ch==EXECUTE
   //=============================================EXECUTE=======================   
                 else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
      }//while
      }//else if char==else
    //=============================================================ELSE=======
            //=====================================================FALLIN=========
      else if(chars[i]=='f'){
                flag = 1;
           while(flag==1){
               flag=1;id = String.valueOf(chars[i]);i++;
             if(chars.length==i){//if no delimiter found
                      
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}//if
                             else if(chars[i]=='a'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
   if(chars[i]=='l'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='l'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
   if(chars[i]=='i'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
    if(chars[i]=='n'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  //============FALLIN DELIMITER==========
                 if(chars[i]==' '||chars[i]=='\n'||chars[i]=='\t'
              ||chars[i]=='\r'||chars[i]=='{'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("fallin","fallin",String.valueOf(lineNumber)));
                        if(chars[i-1]=='{'){
                            id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        }
                         flag=8;
                       
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("fallin","fallin",String.valueOf(lineNumber)));
                    if(chars[i-1]=='{'){
                        id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                    }
                    flag=0;
                    id="";
                    break;}
                 }
   
              
             //                + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. is expected.\n" , null);
     else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ' ', '\\n', '\\t', '\\r' or '{' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }//if char==fall
         //=====================================================FALSE==============
   else if(chars[i]=='s'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
   if(chars[i]=='e'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  //============FALSE DELIMITER==========
                if(chars[i]==' '||chars[i]==';'||chars[i]==')'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("false","false",String.valueOf(lineNumber)));
                        if(chars[i-1]==';'||chars[i-1]==')'){
                            id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        }
                         flag=8;
                     
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("false","false",String.valueOf(lineNumber)));
                      if(chars[i-1]==';'||chars[i-1]==')'){
                            id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        }
                    flag=0;
                    id="";
                    break;}
                 }
     
         else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ' ', ';' or ')' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }// if ch==FAL(S)E
   //=============================================FALSE=======================
    else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
                }//if
   //=====================================================FLT==============
   else if(chars[i]=='l'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
   if(chars[i]=='t'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  //============FLT DELIMITER==========
                if(chars[i]==' '||chars[i]=='\r'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("flt","flt",String.valueOf(lineNumber)));
                         flag=8;
                        System.out.print(flag);
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("flt","flt",String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;}
                 }
         
        else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ' ' or '\\r' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }// if ch==flt
   //=============================================FLT=======================
   //=====================================================FOR LOOP==============
   else if(chars[i]=='o'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;
}
   if(chars[i]=='r'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;
}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  //============FOR DELIMITER==========
                    if(chars[i]==' '||chars[i]=='('||chars[i]=='\r'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("for","for",String.valueOf(lineNumber)));
                        if(chars[i-1]=='('){
                            id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        }
                         flag=8;
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("for","for",String.valueOf(lineNumber)));
                       if(chars[i-1]=='('){
                            id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        }
                    flag=0;
                    id="";
                    break;}
                 }
         
              else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ' ', '(' or '\\r' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }// if ch==for
   //=============================================FOR=======================
    else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
                
      }//while
      }//else if
    //=============================================================FALLIN=======
           //=====================================================IF=========
      else if(chars[i]=='i'){
                flag = 1;
           while(flag==1){
               flag=1;id = String.valueOf(chars[i]);i++;
             if(chars.length==i){//if no delimiter found
                    
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}//if
                             else if(chars[i]=='f'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
  //============IF DELIMITER==========
                if(chars[i]==' '||chars[i]=='('||chars[i]=='\r'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("if","if",String.valueOf(lineNumber)));
                        if(chars[i-1]=='('){
                            id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        }
                         flag=8;
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("if","if",String.valueOf(lineNumber)));
                    if(chars[i-1]=='('){
                            id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        }
                    flag=0;
                    id="";
                    break;}
                 }
           
        else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. '(' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }//if 
     //=====================================================INT==============
   else if(chars[i]=='n'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
   if(chars[i]=='t'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   
  //============INT DELIMITER==========
                if(chars[i]==' '||chars[i]=='\r'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("int","int",String.valueOf(lineNumber)));
                         flag=8;
                        System.out.print(flag);
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("int","int",String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;}
                 }
        
                else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER OF ASSEMBLE
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on"+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. [SPACE] is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
   }// if ch==o
   //=============================================INT=======================
                 else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
      }//while
      }//else if char==if
    //=============================================================IF=======
            //=====================================================NULL=========
      else if(chars[i]=='n'){
                flag = 1;
           while(flag==1){
               flag=1;id = String.valueOf(chars[i]);i++;
             if(chars.length==i){//if no delimiter found
                     
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}//if
                             else if(chars[i]=='u'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
   if(chars[i]=='l'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
    else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='l'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  //============NULL DELIMITER==========
                if(chars[i]==' '||chars[i]==';'||chars[i]=='('){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("null","null",String.valueOf(lineNumber)));
                        if(chars[i-1]==';'||chars[i-1]==')'){id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));}
                         flag=8;
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("null","null",String.valueOf(lineNumber)));
                          if(chars[i-1]==';'||chars[i-1]==')'){id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));}
                    flag=0;
                    id="";
                    break;}
                 }
     
          else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ' ', '(' or ';' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }//if
                 else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
      }//while
      }//else if
    //=============================================================NULL=======
      
            //=====================================================ONDUTY=========
      else if(chars[i]=='o'){
                flag = 1;
           while(flag==1){
               flag=1;id = String.valueOf(chars[i]);i++;
             if(chars.length==i){//if no delimiter found
                     
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;
}//if
                             else if(chars[i]=='n'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;
}
   if(chars[i]=='d'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
    else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='u'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
   if(chars[i]=='t'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
    if(chars[i]=='y'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  //============ONDUTY DELIMITER==========
                if(chars[i]==' '||chars[i]=='\r'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("onduty","onduty",String.valueOf(lineNumber)));
                         flag=8;
                        System.out.print(flag);
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("onduty","onduty",String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;}
                 }
      
            else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ' ' or '\\r' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }//if
                 else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
      }//while
      }//else if
    //=============================================================ONDUTY=======
         //=====================================================REPORT=========
      else if(chars[i]=='r'){
                flag = 1;
           while(flag==1){
               flag=1;id = String.valueOf(chars[i]);i++;
             if(chars.length==i){//if no delimiter found
                        break;}//if
                             else if(chars[i]=='e'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
   if(chars[i]=='p'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
    else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='o'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
   if(chars[i]=='r'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
    if(chars[i]=='t'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  //============REPORT DELIMITER==========
                if(chars[i]==' '||chars[i]=='\r'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("report","report",String.valueOf(lineNumber)));
                         flag=8;
                        System.out.print(flag);
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("report","report",String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;}
                 }
     
        
            else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ' ' or '\\r' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }//if
                 else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
      }//while
      }//else if
    //=============================================================REPORT=======
   //=====================================================STARTMISSION=========
      else if(chars[i]=='s'){
                flag = 1;
           while(flag==1){
               flag=1;id = String.valueOf(chars[i]);i++;
             if(chars.length==i){//if no delimiter found
                    
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}//if
                        else if(chars[i]=='t'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
   if(chars[i]=='a'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
  if(chars[i]=='r'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
    if(chars[i]=='t'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
      if(chars[i]=='m'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
      if(chars[i]=='i'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
        if(chars[i]=='s'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
          if(chars[i]=='s'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
            if(chars[i]=='i'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
              if(chars[i]=='o'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
                if(chars[i]=='n'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
                
  //============STARTMISSION DELIMITER==========
                if(chars[i]==':'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("startmission","startmission",String.valueOf(lineNumber)));
                        id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                         flag=8;
                    break;
                    }
                           else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("startmission","startmission", String.valueOf(lineNumber)));
                    if(chars[i]=='\n'||chars[i]==' '){
                        id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;}
                    else{
                                    
                      id=":";
                      
                       doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. '[SPACE]' or '[NEWLINE]' is expected.\n" , null);
                       list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                     i++;
                        if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                    
                    }
                    }
                 }
            
        else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ':' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
   }//IF CHAR== ST(A)RTMISSION
   //=====================================================STR==============
   else if(chars[i]=='r'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
  //============STR DELIMITER==========
                if(chars[i]==' '||chars[i]=='\r'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("str","str",String.valueOf(lineNumber)));
                         flag=8;
                        System.out.print(flag);
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("str","str",String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;}
                 }
        
            else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ' ' or '\\r' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }// if ch==ST(R)
   //=============================================STR=======================
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
                }//if char==s(t)artmission
      //=====================================================SURRENDER==============
   else if(chars[i]=='u'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
      if(chars[i]=='r'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
          if(chars[i]=='r'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
              if(chars[i]=='e'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
                  if(chars[i]=='n'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
                  
                      if(chars[i]=='d'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
                          if(chars[i]=='e'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
                              if(chars[i]=='r'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  //============SURRENDER DELIMITER==========
                if(chars[i]==' '){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("surrender","surrender",String.valueOf(lineNumber)));
                         flag=8;
                        System.out.print(flag);
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("surrender","surrender",String.valueOf(lineNumber)));
                    flag=0;
                    id="";
                    break;}
                 }
         
           else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ' ' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }//else if ch==s(u)rrender
   //=============================================surrender=======================
                 else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
      }//while
      }//else if
    //=============================================================STARTMISSION=======
          //=====================================================TRUE=========
      else if(chars[i]=='t'){
                flag = 1;
           while(flag==1){
               flag=1;id = String.valueOf(chars[i]);i++;
             if(chars.length==i){//if no delimiter found
                   
                        break;}//if
                             else if(chars[i]=='r'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
   if(chars[i]=='u'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
    else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='e'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  //============TRUE DELIMITER==========
                if(chars[i]==' '||chars[i]==';'||chars[i]==')'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("true","true",String.valueOf(lineNumber)));
                        if(chars[i-1]==';'||chars[i-1]==')'){
                            id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        }
                         flag=8;
                        System.out.print(flag);
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("true","true",String.valueOf(lineNumber)));
                     if(chars[i-1]==';'||chars[i-1]==')'){
                            id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        }
                    flag=0;
                    id="";
                    break;}
                 }
        
       else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ' ', ';' or ')' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }//if
                 else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
      }//while
      }//else if
    //=============================================================TRUE=======  
            //=====================================================BOOL=========
      else if(chars[i]=='w'){
                flag = 1;
           while(flag==1){
               flag=1;id = String.valueOf(chars[i]);i++;
             if(chars.length==i){//if no delimiter found
                     
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}//if
                             else if(chars[i]=='h'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}
   if(chars[i]=='i'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
    else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  if(chars[i]=='l'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
    if(chars[i]=='e'){id = id.concat(String.valueOf(chars[i]));i++;
  /**if no delim**/ if(chars.length==i){ 
                        list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                    break;}}
   else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
  //============WHILE DELIMITER==========
                     if(chars[i]==' '||chars[i]=='('||chars[i]=='\r'){
                i++;
                    if(i==(chars.length)){
                        list.add(new Lexical("while","while",String.valueOf(lineNumber)));
                           if(chars[i-1]=='('){
                            id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        }
                         flag=8;
                    break;
                    }
                    else{//IF THERE ARE MORE CHARACTERS AFTER RESERVED WORD
                    list.add(new Lexical("while","while",String.valueOf(lineNumber)));
                      if(chars[i-1]=='('){
                            id="";
                        id = id.concat(String.valueOf(chars[i-1]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        }
                    flag=0;
                    id="";
                    break;}
                 }
    else if(chars[i]!='a'&&chars[i]!='b'&&chars[i]!='c'&&chars[i]!='d'&&chars[i]!='e'
                &&chars[i]!='f'&&chars[i]!='g'&&chars[i]!='h'&&chars[i]!='i'&&chars[i]!='j'
                &&chars[i]!='k'&&chars[i]!='l'&&chars[i]!='m'&&chars[i]!='n'&&chars[i]!='o'
                &&chars[i]!='p'&&chars[i]!='q'&&chars[i]!='r'&&chars[i]!='s'&&chars[i]!='t'
                &&chars[i]!='u'&&chars[i]!='v'&&chars[i]!='w'&&chars[i]!='x'&&chars[i]!='y'
                &&chars[i]!='z'&&chars[i]!='_'&&chars[i]!='1'&&chars[i]!='2'&&chars[i]!='3'
                &&chars[i]!='4'&&chars[i]!='5'&&chars[i]!='6'&&chars[i]!='7'&&chars[i]!='8'
                &&chars[i]!='9'){//ERROR TRAP FOR DELIMITER 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. ' ', '(' or '\\r' is expected.\n" , null);
             
                     i++;
                    
              if(i==(chars.length)){
                   
                  id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
                
                 else{
                     if(i+1==(chars.length)){
                id = id.concat(String.valueOf(chars[i]));
                         list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));       
                  id="";
                        flag=8;
                    break;}
                     else{
                    IdentifierCount++;
                    flag=3;
                }
                }
                }//if
                 else{IdentifierCount++;flag=3;break; /**proceed to Identifier**/}
      }//while
      }//else if char==while
    //=============================================================WHILE=======
      else if(chars[i]=='g'||chars[i]=='h'||chars[i]=='j'
           ||chars[i]=='k'||chars[i]=='l'||chars[i]=='m'
           ||chars[i]=='p'||chars[i]=='q'
           ||chars[i]=='u'||chars[i]=='v'||chars[i]=='x'||chars[i]=='y'
           ||chars[i]=='z'){
          flag=3;
          IdentifierCount++;
      }
     //===========================================MILLER 
      //=========================================================================Reserved Symbols
      
else if(chars[i]=='*'&&chars[i+1]=='*'){
        try{
          int commentctr=0;
          id="";
          id = id.concat("**");
       
          i+=2;
          while(commentctr==0&&chars.length>i){
                if(i==chars.length) break;
                System.out.println(chars.length+"Thisismylength");
                if(chars[i]=='*'){
                    id = id.concat(String.valueOf(chars[i]));
                    i++;
                    if(i==chars.length) break;
                    
                    else if(chars[i]=='*'){
                        id = id.concat(String.valueOf(chars[i]));
                        list.add(new Lexical(id,"Comments",String.valueOf(lineNumber)));
                        i++;
                        commentctr=1;
                        break;
                    }
                    
                    else{
                        id = id.concat(String.valueOf(chars[i])); i++;
                        if(i==chars.length) break;
                    }
                }
                
                else{
                    id = id.concat(String.valueOf(chars[i])); i++;
                    if(i==chars.length) break;
                }
          }
          System.out.println(i+"Thiis I");
          if(i==(chars.length)){
              if(commentctr==0){ 
                  doc.insertString(doc.getLength(), "\nLEXICAL ERROR on COMMENTS"
                             + ", Line "+lineNumber+". Comments are expected to be closed\n" , null);
              }
                            
              id="";
              flag = 8; 
              i--;
              break;
          }
          
          else{
              System.out.println("I: "+i+"Len: "+chars.length);
              if(chars[i]=='\n'||chars[i]==' '||chars[i]=='*'){
                  id="";
                  if(chars[i]=='*'){ 
                      i++;
                      if(i==chars.length){ 
                          doc.insertString(doc.getLength(), "\nLEXICAL ERROR on COMMENTS"
                             + ", Line "+lineNumber+". '"+chars[i-1]+"' is not a valid delimiter. [SPACE], [NULL] or [NEWLINE] is expected.\n" , null); 
                          flag=8;
                          break;
                      }
                      else{
                          if(chars[i]!='*') 
                            doc.insertString(doc.getLength(), "\nLEXICAL ERROR on COMMENTS"
                             + ", Line "+lineNumber+". '"+chars[i-1]+"' is not a valid delimiter. [SPACE], [NULL] or [NEWLINE] is expected.\n" , null);
                          else{ flag=0; i--; }
                          
                      }
                  }
                  
                  else{
                  i++;
                  if(i==chars.length){flag=8; break;}
                  else flag=0; }
              }
              
              
              
              else{
                  
                  doc.insertString(doc.getLength(), "\nLEXICAL ERROR on COMMENTS"
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. [SPACE], [NULL] or [NEWLINE] is expected.\n" , null);
                  i++; 
                  id="";
                  if(i==chars.length){ flag=8; break;}
                  else flag=0;
              }
            }
        }
        
        catch(ArrayIndexOutOfBoundsException e){
                        System.out.println("Out of bounds error occured!!\n");
                        i-=2;
                         break;
        }
      }
      //===================================RESERVED SYMBOLS===
      
         else if(chars[i]==' '||chars[i]=='\n'||chars[i]=='{'/**||chars[i]==':'**/||chars[i]=='}'||chars[i]==']'||chars[i]=='['
              ||chars[i]==')'||chars[i]=='('||chars[i]=='\t'
              ||chars[i]=='\r'){
         id="";
          i++;
          if((i==(chars.length))){//IF NOT ARITH OP
                   if(chars[i-1]==' '||chars[i-1]=='\n'||chars[i-1]=='\t'||chars[i-1]=='\r'){
                   if(chars[i-1]=='\n'){
                     //System.out.print("HELLOETO"); 
                     lineNumber++;
                    
                     break;
                   }
                   else{
//                       doc.insertString(doc.getLength(), "LEXICAL ERROR on "+id+""
//                             + ", Line "+lineNumber+". No delimiter found.\n" , null);//BEA EDIT 10/09/2018

                break;}
                   }
                  else{
                      
                 id="";
               id = id.concat(String.valueOf(chars[i-1]));
            
               list.add(new Lexical(id, id, String.valueOf(lineNumber)));
             
            flag=8;
              // break;
               
                          }
          }
          else{
               if(chars[i-1]==' '||chars[i-1]=='\n'||chars[i-1]=='\t'||chars[i-1]=='\r')
              //i++; 
               {
                if(chars[i-1]=='\n'){
                    
                    lineNumber++;
                    
                     flag=0;
                id="";}
                   else{
                    
                     System.out.print("SPACe\n");
                     //i++;
                     //list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                   flag=0;
                id="";
                
                continue;
                }}
                
                else{
                   
              id = id.concat(String.valueOf(chars[i-1])); //===BEA EDIT 10/05/2018
             list.add(new Lexical(id, id,String.valueOf(lineNumber)));
          // System.out.print("HMM"); +
         // i++;
           id="";
           
           //flag=0;
           if(chars[i-1]=='('){
            //    id = id.concat(String.valueOf(chars[i-1]));
               
                if(i==chars.length)//if no delimiter found              
                        break;
                else if(chars[i]==','||chars[i]=='('||chars[i]==')'||chars[i]==' '||chars[i]=='\n'
          ||chars[i]=='-'||chars[i]=='!'||chars[i]=='a'||chars[i]=='b'||chars[i]=='c'||chars[i]=='d'||chars[i]=='e'
           ||chars[i]=='f'||chars[i]=='g'||chars[i]=='h'||chars[i]=='i'||chars[i]=='j'
           ||chars[i]=='k'||chars[i]=='l'||chars[i]=='m'||chars[i]=='n'||chars[i]=='o'
           ||chars[i]=='p'||chars[i]=='q'||chars[i]=='r'||chars[i]=='s'||chars[i]=='t'
           ||chars[i]=='u'||chars[i]=='v'||chars[i]=='w'||chars[i]=='x'||chars[i]=='y'
           ||chars[i]=='z'||chars[i]=='1'||chars[i]=='2'||chars[i]=='3'||chars[i]=='4'
                       ||chars[i]=='5'||chars[i]=='6'||chars[i]=='7'||chars[i]=='8'
                       ||chars[i]=='9'||chars[i]==';'){
                   delimCheck=1;
                  
               }
               else
                   delimCheck=0;
           }
           else if(chars[i-1]==')'){
               
                id = id.concat(String.valueOf(chars[i-1]));
               
                if(i==chars.length)//if no delimiter found              
                        break;
                else if(chars[i]=='+'||chars[i]=='-'||chars[i]=='*'||chars[i]=='/'||
          chars[i]=='%'||chars[i]=='!'||chars[i]=='<'||chars[i]=='>'||
                   chars[i]==')'||chars[i]==';'||chars[i]==','||chars[i]=='&'||
                    chars[i]=='{'||chars[i]==' '||chars[i]=='\t'||chars[i]=='\r'
                   ||chars[i]=='\n'||chars[i]==','||chars[i]=='|'||chars[i]=='}'){
                    id="";
                   delimCheck=1;
                 
               }
               else
                   delimCheck=0;
           }
    
           else if(chars[i-1]==';'){
               
                id = id.concat(String.valueOf(chars[i-1]));
               
                if(i==chars.length)//if no delimiter found              
                        break;
           else if(chars[i]==';'
          ||chars[i]==' '||chars[i]=='\n'||chars[i]=='\t'||chars[i]=='r'||chars[i]=='a'||chars[i]=='b'||chars[i]=='c'||chars[i]=='d'||chars[i]=='e'
           ||chars[i]=='f'||chars[i]=='g'||chars[i]=='h'||chars[i]=='i'||chars[i]=='j'
           ||chars[i]=='k'||chars[i]=='l'||chars[i]=='m'||chars[i]=='n'||chars[i]=='o'
           ||chars[i]=='p'||chars[i]=='q'||chars[i]=='r'||chars[i]=='s'||chars[i]=='t'
           ||chars[i]=='u'||chars[i]=='v'||chars[i]=='w'||chars[i]=='x'||chars[i]=='y'
           ||chars[i]=='z'||chars[i]==';'||chars[i]==')'){
                    id="";
                   delimCheck=1;
                 
               }
               else
                   delimCheck=0;
           }
           
         else if(chars[i-1]==':'){
               
                id = id.concat(String.valueOf(chars[i-1]));
               
                if(i==chars.length)//if no delimiter found              
                        break;
           else if(chars[i]=='\n'||chars[i]==' '){
                    id="";
                   delimCheck=1;
                 
               }
               else
                   delimCheck=0;
           }
        else if(chars[i-1]=='{'){
               
                id = id.concat(String.valueOf(chars[i-1]));
               
                if(i==chars.length)//if no delimiter found              
                        break;
           else if(chars[i]=='{'||chars[i]=='\''||chars[i]=='\"'||chars[i]=='}'
          ||chars[i]==' '||chars[i]=='\n'||chars[i]=='\t'||chars[i]=='r'
          ||chars[i]=='a'||chars[i]=='b'||chars[i]=='c'||chars[i]=='d'||chars[i]=='e'
           ||chars[i]=='f'||chars[i]=='g'||chars[i]=='h'||chars[i]=='i'||chars[i]=='j'
           ||chars[i]=='k'||chars[i]=='l'||chars[i]=='m'||chars[i]=='n'||chars[i]=='o'
           ||chars[i]=='p'||chars[i]=='q'||chars[i]=='r'||chars[i]=='s'||chars[i]=='t'
           ||chars[i]=='u'||chars[i]=='v'||chars[i]=='w'||chars[i]=='x'||chars[i]=='y'
           ||chars[i]=='z'||chars[i]=='1'||chars[i]=='2'||chars[i]=='3'||chars[i]=='4'
                       ||chars[i]=='5'||chars[i]=='6'||chars[i]=='7'||chars[i]=='8'
                       ||chars[i]=='9'||chars[i]=='('){
                    id="";
                   delimCheck=1;
                 
               }
               else
                   delimCheck=0;
           }
        else if(chars[i-1]=='}'){
               
                id = id.concat(String.valueOf(chars[i-1]));
               
                if(i==chars.length)//if no delimiter found              
                        break;
           else if(chars[i]==';'||chars[i]=='}'||
          chars[i]==' '||chars[i]=='\n'||chars[i]=='\t'||chars[i]=='\r'||chars[i]=='a'||chars[i]=='b'||chars[i]=='c'||chars[i]=='d'||chars[i]=='e'
           ||chars[i]=='f'||chars[i]=='g'||chars[i]=='h'||chars[i]=='i'||chars[i]=='j'
           ||chars[i]=='k'||chars[i]=='l'||chars[i]=='m'||chars[i]=='n'||chars[i]=='o'
           ||chars[i]=='p'||chars[i]=='q'||chars[i]=='r'||chars[i]=='s'||chars[i]=='t'
           ||chars[i]=='u'||chars[i]=='v'||chars[i]=='w'||chars[i]=='x'||chars[i]=='y'
           ||chars[i]=='z'){
                    id="";
                   delimCheck=1;
                 
               }
               else
                   delimCheck=0;
           }
           
           else if(chars[i-1]=='['){
               
                id = id.concat(String.valueOf(chars[i-1]));
               
                if(i==chars.length)//if no delimiter found              
                        break;
           else if(chars[i]=='a'||chars[i]=='b'||chars[i]=='c'||chars[i]=='d'||chars[i]=='e'
           ||chars[i]=='f'||chars[i]=='g'||chars[i]=='h'||chars[i]=='i'||chars[i]=='j'
           ||chars[i]=='k'||chars[i]=='l'||chars[i]=='m'||chars[i]=='n'||chars[i]=='o'
           ||chars[i]=='p'||chars[i]=='q'||chars[i]=='r'||chars[i]=='s'||chars[i]=='t'
           ||chars[i]=='u'||chars[i]=='v'||chars[i]=='w'||chars[i]=='x'||chars[i]=='y'
           ||chars[i]=='z'||chars[i]=='1'||chars[i]=='2'||chars[i]=='3'||chars[i]=='4'
                       ||chars[i]=='5'||chars[i]=='6'||chars[i]=='7'||chars[i]=='8'
                       ||chars[i]=='9'||chars[i]==' '||chars[i]=='\n'||chars[i]=='\t'||chars[i]=='r'||chars[i]=='('){
                    id="";
                   delimCheck=1;
                 
               }
               else
                   delimCheck=0;
           }
           
           else if(chars[i-1]==']'){
               
                id = id.concat(String.valueOf(chars[i-1]));
               
                if(i==chars.length)//if no delimiter found              
                        break;
           else if(chars[i]=='+'||chars[i]=='-'||chars[i]=='*'||chars[i]=='/'||
          chars[i]=='%'||chars[i]=='!'||chars[i]=='<'||chars[i]=='>'||
                   chars[i]==')'||chars[i]==';'||chars[i]=='.'||chars[i]=='&'
                   ||chars[i]==' '||chars[i]=='\t'||chars[i]=='\r'
                   ||chars[i]=='\n'||chars[i]=='|'||chars[i]==']'||chars[i]==','
                   ||chars[i]=='['||chars[i]=='='){
                    id="";
                   delimCheck=1;
                 
               }
               else
                   delimCheck=0;
           }
           
           if(delimCheck==1){
               System.out.print("H:"+chars[i]+" :"+chars.length+" ");
               flag=0;
               if(i+1==chars.length){
                   id="";
                   id = id.concat(String.valueOf(chars[i]));
                   list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                   break;}
               else if(Character.isAlphabetic(chars[i]) == true){
               id="";
                id = id.concat(String.valueOf(chars[i]));
               flag=0;
               //continue;
                }
               else{
//                   id="";
//                   id = id.concat(String.valueOf(chars[i]));
//                   list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                   flag=0;
                   id="";
               }
           }
           else {//ERROR TRAP 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter for "+chars[i-1]+".\n" , null);
               //list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                     i++;
                    
              if(i==(chars.length)){
                         id="";
                        flag=8;
                    break;}
                     else{
                         id="";
                         flag=0;
                 //        break;
                     }
                 }
               
                }
                    
              
          }
      }
      //=================================================
      
      ///MILLER END COMMENT============================================
      //=====================================================NEW INT/FLOAT LITERAL      
         else if(chars[i]=='0'&&zerowatch==0){
             id="";
             id = id.concat(String.valueOf(chars[i]));
             i++;
             System.out.print("ETOTOH");
            
                                   if(i==chars.length){
                                       list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber)));
                                        break;}
                                     else{    
                                         if( chars[i] == '<' || chars[i] == ' '
                                || chars[i] == '=' || chars[i] == '>'
                                || chars[i] == '-' || chars[i] == '+'
                                || chars[i] == '*' || chars[i] == '}'
                                || chars[i] == '/' || chars[i] == '%'
                                || chars[i] == '|' || chars[i] == '&'
                                || chars[i] == ')' || chars[i] == ';' || chars[i] == '('|| chars[i] == '!' 
                                || chars[i] == '[' || chars[i] == ']'
                                || chars[i] == ',' || chars[i] == ':'
                                || chars[i] == '\t'|| chars[i] == '\n')
                                {
                              
                                 list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber)));
                                  if(chars[i]==' '||chars[i] == '\t'|| chars[i] == '\n'){
                                    if(chars[i] == '\n')
                                        lineNumber++;
                                id="";
                                 if(i+1==chars.length)
                                 break;
                                 else
                                i++;
                                 flag=0;
                                }
                                else{
                                id="";
                                id = id.concat(String.valueOf(chars[i]));
                                list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                                id="";
                                 if(i+1==chars.length)
                                        break;
                                 else
                                i++;
                                 flag=0;}
                                }
                              else{
                                  doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+chars[i]+""
                                      + ", Line "+lineNumber+".Invalid delimiter. \n" , null);
                                    id = "";
                                     if(i+1==chars.length)
                                        break;
                                     else
                                    i++;
                                     flag=0;}
                            }
                            }
                    else if(Character.isDigit(chars[i]) == true&&chars[i]!='0') // Integer and Float Literals
                    {
                    try{
                        zerowatch=1;
                        //System.out.print("Here");
                        
                         if(i+1 == (chars.length)){
                             id = id.concat(String.valueOf(chars[i]));
                             list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber)));
                             break;
                         }
                         else{
                             id = id.concat(String.valueOf(chars[i]));
                             if(Character.isDigit(chars[i+1])==false&&chars[i+1]!='.'){
                                
                              list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber)));}
                          
                        i++;
                        flag=0;
                         }
                        int sample = 0; //check if correct si delimeter
                        int k = 1, fltLit=0;
                        int a=10,run=0;
                        char b =(char)a;
                        
                        if(i == (chars.length)) //Detects if the line has reached the end
                        {   
                            id="";
                             id = id.concat(String.valueOf(chars[i]));
                              list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                            break;
                        }
                       
                        else
                        {
                            if(chars[i] == '<' || chars[i] == ' '
                            || chars[i] == '=' || chars[i] == '>'
                            || chars[i] == '-' || chars[i] == '+'
                            || chars[i] == '*' || chars[i] == '}'
                            || chars[i] == '/' || chars[i] == '%'
                            || chars[i] == '|' || chars[i] == '&'
                            || chars[i] == ')' || chars[i] == ';' || chars[i] == '('|| chars[i] == '!'
                            || chars[i] == '[' || chars[i] == ']'
                            || chars[i] == ',' 
                            || chars[i] == '\t'|| chars[i] == '\n') //Delimiters for INT Literals (INT Literals)
                            {
                                
                               
                                     if(i+1 == (chars.length)) //Detects if the line has reached the end
                        {   
                            System.out.print("Kello:"+chars[i]+";;");
                           //list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber)));
                            id="";
                             id = id.concat(String.valueOf(chars[i]));
                              list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                            break;
                        }
                                         else if(chars[i]=='='||chars[i]=='!'||chars[i]=='>'||chars[i]=='<'||chars[i]=='+'
                                                 ||chars[i]=='-'){
                                         if(chars[i]=='='&&chars[i+1]=='='){
                                           //  list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber))); 
                                             list.add(new Lexical("==","==",String.valueOf(lineNumber))); 
                                           if(i+2!=chars.length)  i=i+2;
                                           else break;
                                         }
                                         else if(chars[i]=='+'&&chars[i+1]=='+'){
                                           //  list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber))); 
                                             list.add(new Lexical("++","++",String.valueOf(lineNumber))); 
                                           if(i+2!=chars.length)  i=i+2;
                                           else break;
                                         }
                                          else if(chars[i]=='-'&&chars[i+1]=='-'){
                                           //  list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber))); 
                                             list.add(new Lexical("--","--",String.valueOf(lineNumber))); 
                                           if(i+2!=chars.length)  i=i+2;
                                           else break;
                                         }
                                         else if(chars[i]=='!'&&chars[i+1]=='='){
                                           //  list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber))); 
                                             list.add(new Lexical("!=","!=",String.valueOf(lineNumber))); 
                                             if(i+2!=chars.length)  i=i+2;
                                           else break;
                                         }   
                                             else if(chars[i]=='<'&&chars[i+1]=='='){
                                           //  list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber))); 
                                             list.add(new Lexical("<=","<=",String.valueOf(lineNumber))); 
                                             if(i+2!=chars.length)  i=i+2;
                                           else break;
                                         }
                                             else if(chars[i]=='>'&&chars[i+1]=='='){
                                           //  list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber))); 
                                             list.add(new Lexical(">=",">=",String.valueOf(lineNumber))); 
                                            if(i+2!=chars.length)  i=i+2;
                                           else break;
                                         }
                                         
                                         flag=0;
                                         sample=0;
                                         id="";
                                       // break;
                                     }
//                                if(Character.isDigit(chars[i-2])==false)
//                                {
//                                list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber)));
//                                }

                                     else if(chars[i]=='+'||chars[i]=='-')
                                {
                                     // System.out.print("PKSKS");
                                         id = id.concat(String.valueOf(chars[i]));
                                        if(chars[i+1]=='-'&& chars[i]=='-'){                                        

                                              id = id.concat(String.valueOf(chars[i+1]));
                                             list.add(new Lexical(id,"--",String.valueOf(lineNumber)));
                                             i++;
                                             flag=0;
                                        }
                                        else if(chars[i+1]=='+'&& chars[i]=='+'){
                                           // System.out.print("PPMM");
                                              id = id.concat(String.valueOf(chars[i+1]));
                                             list.add(new Lexical(id,"++",String.valueOf(lineNumber)));
                                             i++;
                                              flag=0;
                                        }
                                        else{
                                            
                                            //list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                                            id="";
                                            i++;
                                             flag=0;
                                        }
                                }
                                else
                                {
                                if(chars[i]==' '||chars[i] == '\t'|| chars[i] == '\n'){
                                    if(chars[i] == '\n')
                                        lineNumber++;
                                  
                                id="";
//                                 if(i+1 == (chars.length)) {//Detects if the line has reached the end
//                                 flag=0;
//                                    list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber)));
//                                    break;
//                                
//                                 }
//                                 else{
                                i++;
                                 flag=0;
                                //}
                                }
                                else{
                                    id="";
                                id = id.concat(String.valueOf(chars[i]));
                                list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                                id="";
                                i++;
                                 flag=0;}
                                }
                            }
                            else if(Character.isDigit(chars[i])==false&&chars[i]!='.'){
                                 doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+chars[i]+""
                                      + ", Line "+lineNumber+".Invalid delimiter. \n" , null);
                                    id = "";
                                     if(i+1==chars.length)
                                        break;
                                    else
                                        flag=0;
                                     i++;
                                     sample=0;
                            }
                      
                        else
                        {
                            
                            while(Character.isDigit(chars[i]) == true && k <= 8) //Determines the length of the INT
                            {
                                
                                
                                 id = id.concat(String.valueOf(chars[i]));
                                i++;
                                if((i == chars.length) || chars[i] == '<' || chars[i] == ' '
                                || chars[i] == '=' || chars[i] == '>'
                                || chars[i] == '-' || chars[i] == '+'
                                || chars[i] == '*' || chars[i] == '}'
                                || chars[i] == '/' || chars[i] == '%'
                                || chars[i] == '|' || chars[i] == '&'
                                || chars[i] == ')' || chars[i] == ';'
                                || chars[i] == '[' || chars[i] == ']' || chars[i] == '('|| chars[i] == '!'
                                || chars[i] == ',' 
                                || chars[i] == '\t'|| chars[i] == '\n' &&(k<=8))
                                {
                                   if(i==chars.length){
                                    list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber))); 
                                    flag=0;
                                    sample=0;break;
                                }
                           
                                  
                                    else if(chars[i]=='='||chars[i]=='!'||chars[i]=='>'||chars[i]=='<'||chars[i]=='+'||chars[i]=='-'){
                                         if(chars[i]=='='&&chars[i+1]=='='){
                                           list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber))); 
                                             list.add(new Lexical("==","==",String.valueOf(lineNumber))); 
                                             if(i+1!=chars.length)  i++;
                                           else break;
                                         }
                                         else if(chars[i]=='+'&&chars[i+1]=='+'){
                                            list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber))); 
                                             list.add(new Lexical("++","++",String.valueOf(lineNumber))); 
                                              if(i+1!=chars.length)  i++;
                                           else break;
                                         }  
                                         else if(chars[i]=='-'&&chars[i+1]=='-'){
                                            list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber))); 
                                             list.add(new Lexical("--","--",String.valueOf(lineNumber))); 
                                              if(i+1!=chars.length)  i++;
                                           else break;
                                         }  
                                         else if(chars[i]=='!'&&chars[i+1]=='='){
                                            list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber))); 
                                             list.add(new Lexical("!=","!=",String.valueOf(lineNumber))); 
                                              if(i+1!=chars.length)  i++;
                                           else break;
                                         }   
                                             else if(chars[i]=='<'&&chars[i+1]=='='){
                                           list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber))); 
                                             list.add(new Lexical("<=","<=",String.valueOf(lineNumber))); 
                                              if(i+1!=chars.length)  i++;
                                           else break;
                                         }
                                             else if(chars[i]=='>'&&chars[i+1]=='='){
                                           list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber))); 
                                             list.add(new Lexical(">=",">=",String.valueOf(lineNumber))); 
                                              if(i+1!=chars.length)  i++;
                                           else break;
                                         }
                                         
                                         flag=0;
                                         sample=0;
                                        // id="";
                                        break;
                                     }
                                   else if(chars[i]==' '||chars[i] == '\t'|| chars[i] == '\n'){
                                        
                                        if(chars[i] == '\n')
                                        lineNumber++;
                                        
                                    list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber)));
                                         id = "";
                                    sample = 0;
                                    i--; //LookAhead
                                    break;  //break na kasi valid naman na si id
                                    }
                                    //Adds the Lexeme, Line # and Column # to the Lexical Table
                                    list.add(new Lexical(id,"Int Literal",String.valueOf(lineNumber)));
                                    id = "";
                                    sample = 0;
                                    i--; //LookAhead
                                    flag=0;
                                    break;  //break na kasi valid naman na si id
                                }
                             
                                    
                                else
                                {
                                    
                                    sample = 1;
                                }
                                k++;
                            }

                            if(chars[i] == '.') //Determines if the value if FLOAT
                            {
                                 id = id.concat(String.valueOf(chars[i]));
                                i++;
                                k = 0;
                             if(i+1==chars.length){
                                        id = id.concat(String.valueOf(chars[i]));
                                       list.add(new Lexical(id,"Float Literal",String.valueOf(lineNumber)));
                                    break;}
                                while(i==chars.length || Character.isDigit(chars[i]) == true && k <= 3)
                                {
                                     id = id.concat(String.valueOf(chars[i]));
                                    i++;
                                    if(chars[i] == '<' || chars[i] == ' '
                                    || chars[i] == '=' || chars[i] == '>'
                                    || chars[i] == '-' || chars[i] == '+'
                                    || chars[i] == '*' || chars[i] == '}'
                                    || chars[i] == '/' || chars[i] == '%'
                                    || chars[i] == '|' || chars[i] == '&'|| chars[i] == '!'
                                    || chars[i] == ')' || chars[i] == ';'
                                    || chars[i] == '[' || chars[i] == ']'
                                    || chars[i] == ',' 
                                    || chars[i] == '\t'|| chars[i] == '\n') // Delimeter
                                    {
                                        //TRAILING ZEROES ON WORKING
                                       //id.replace(' ',id.charAt(id.length()-1));
                                       // id.charAt(id.length()-1);
                                        if(chars[i] == '\n')
                                        lineNumber++;
                                       if(chars[i] == '[' || chars[i] == ']'){
                                          doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+chars[i]+""
                                      + ", Line "+lineNumber+".Invalid delimiter. \n" , null);  
                                       if(i+1==chars.length){
                                             list.add(new Lexical(id,"Float Literal",String.valueOf(lineNumber)));
                                        break;}
                                       else{
                                            list.add(new Lexical(id,"Float Literal",String.valueOf(lineNumber)));
                                            id="";
                                            sample=0;
                                        flag=0;
                                    // i--; 
                                       break;
                                       }}
                                       else{
                                        list.add(new Lexical(id,"Float Literal",String.valueOf(lineNumber)));
                                        id = ""; //Clears the variable for Lexeme
                                        sample = 0;
                                        i--; // LookAhead Character
                                        break;  //break na kasi valid naman na si id
                                       }
                                    }
                                    else if(i+1==chars.length){
                                        id = id.concat(String.valueOf(chars[i]));
                                        fltLit=1;
                                    break;}
                                    else
                                    {
                                        sample = 1;
                                    }
                                    k++;
                                }
                                if( Character.isDigit(chars[i]) == true&&(k >= 4))
                                {
                                    //System.out.print("fl1-"+k+"-["+chars.length+"]");
                                  doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+chars[i]+""
                                      + ", Line "+lineNumber+". Invalid number of  number digits. Only 4 digits are allowed.\n" , null);
                                    id = "";
                                    //i--;
                                    if(i+1==chars.length)
                                        break;
                                sample=0;
                                i++;
                                }
                                else if(Character.isDigit(chars[i]) == false|| (sample == 1)){
                                 
                                     if(i+1==chars.length)
                                        break;
                                     else{    
                                         System.out.print("uuu-"+k+"-["+chars.length+"]");
                                  doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+chars[i]+""
                                      + ", Line "+lineNumber+".Invalid delimiter. \n" , null);
                                    id = "";
                                     i++;
                                     flag=0;
                                     sample=0;
                                    }
                                }
                            }
                
                            if((Character.isDigit(chars[i]) == false)&& (sample == 1)) //Determines if the next character is not digit
                                {
                                    System.out.print("fl1");
                                  doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+chars[i]+""
                                      + ", Line "+lineNumber+".Invalid delimiter. \n" , null);
                                    id = "";
                                     if(i+1==chars.length)
                                        break;
                                    else
                                        flag=0;
                                     i++;
                                }
                             
                            else if((k >= 8 && sample == 1)) //Determines if the number exceeds the length of 10 digits
                            {
                                System.out.print("fl2==");
                                  doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+chars[i]+""
                                      + ", Line "+lineNumber+". Invalid number of  number digits. Only 9 digits are allowed.\n" , null);
                                id = "";
                                
                                 if(i+1==chars.length)
                                        break;
                                    else
                                        flag=0;
                            }
                            else if(k >= 8 && sample == 1)
                            {
                                System.out.print("fl3");
                                  doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+chars[i]+""
                                      + ", Line "+lineNumber+". Invalid number of  number digits. Only 9 digits are allowed.\n" , null);
                                id = "";
                                if(i+1==chars.length)
                                        break;
                                    else
                                        flag=0;
                            }
                            else if(flag==0){
                                //flag=0;
                                id="";
                              System.out.print("zz-"+i+"-["+chars[i]+"]");   
                              
                                   if(fltLit==1){
                                  
                                  list.add(new Lexical(id,"Float Literal",String.valueOf(lineNumber)));  
                                  break;
                              }
                              i++;
                            }
                        }
                    } 
                    }
                        
                    catch(ArrayIndexOutOfBoundsException e){
                        System.out.println("Out of bounds error occured!!\n");
                        i--;
                         break;
                    }
                        
                    }
     
//=======================================================================RESERVED SYMBOLS
      else if(chars[i]=='='||chars[i]=='+'||chars[i]=='-'||chars[i]=='*'||chars[i]=='/'||
            chars[i]=='%'||chars[i]=='!'||chars[i]=='<'||chars[i]=='>'||chars[i]=='&'||chars[i]=='|'
              ||chars[i]==';'||chars[i]==',')
            {
                
                System.out.print("FLS"+chars[i]+" i:"+(i+1)+" ch:"+chars.length+"");
                id="";
              id = id.concat(String.valueOf(chars[i]));
              i++;
              if(i==chars.length){
                  if(chars[i-1]==';'){
                      //list.add(new Lexical(id, id,String.valueOf(lineNumber)));
//                       doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
//                             + ", Line "+lineNumber+". No delimiter founddd.\n" , null);
System.out.print("UUU");
                      list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                      id="";
                      id = id.concat(String.valueOf(chars[i-1]));
                    
                     }
                  else{
                  
                  list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                      id="";
                  }
              break;}
             else if(chars[i-1]=='='&&chars[i]==';'){
                         doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+chars[i-1]+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter.\n" , null);
            
                     System.out.print("LL");
                    
              if(i+1==(chars.length)){
                         id="";
                        
                    break;}
                     else{
                  i++;
                         id="";
                         flag=0;
                    //     break;
                     }
                           
              }
              
              else if(chars[i]==' '||chars[i]=='\n'||chars[i]=='\t'||chars[i]=='\r'){
                              if(chars[i]=='\n')
                               lineNumber++;
                           list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                           //i++; 
                           flag=0;
                           
                           id="";
                          
                              //break;
                          }
                 else if(chars[i-1]=='='&&chars[i]=='{'){
                  list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                  flag=0;
                  id="";
                 // continue;
              }
                     else if((chars[i-1]=='!'&&Character.isAlphabetic(chars[i]))||(chars[i-1]=='!'&&Character.isDigit(chars[i]))){
                  list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                  flag=0;
                  id="";
                 // continue;
              }
              //if RELOP
                      else if(chars[i-1]=='='||chars[i-1]=='!'||chars[i-1]=='>'||chars[i-1]=='<'){
                                         if(chars[i-1]=='='&&chars[i]=='='){
                                          
                                             list.add(new Lexical("==","==",String.valueOf(lineNumber))); 
                                           
                                         }
                                         else if(chars[i-1]=='!'&&chars[i]=='='){
                                           
                                             list.add(new Lexical("!=","!=",String.valueOf(lineNumber))); 
                                           
                                         }   
                                             else if(chars[i-1]=='<'&&chars[i]=='='){
                                          
                                             list.add(new Lexical("<=","<=",String.valueOf(lineNumber))); 
                                            
                                         }
                                             else if(chars[i-1]=='>'&&chars[i]=='='){
                                          
                                             list.add(new Lexical(">=",">=",String.valueOf(lineNumber))); 
                                             
                                         }
                                         if(i+1!=chars.length){
                                         i++;
                                          flag=0;
                           
                                        id="";}
                                         else break;
                                         
                                      
                                     }
              //IF NUMBER
              else if(Character.isDigit(chars[i])==true){
                  list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                  flag=0;
                  id="";
                 // continue;
              }
                 else if((chars[i-1]==','&&Character.isDigit(chars[i])==true)||(chars[i-1]==','&&Character.isAlphabetic(chars[i])==true)
                         ||(chars[i-1]==','&&chars[i]=='(')){
                  list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                  flag=0;
                  id="";
                 // continue;
              }
             else if((chars[i-1]==','&&Character.isDigit(chars[i])!=true)||(chars[i-1]==','&&Character.isAlphabetic(chars[i])!=true)
                         ||(chars[i-1]==','&&chars[i]!='(')){
                        doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+chars[i-1]+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter.\n" , null);
            
              if(i+1==(chars.length)){
                         id="";break;}
                     else{
                  i++; id="";flag=0;
                     }
                       
              }
               else if(chars[i-1]=='+'&&chars[i]=='"'){
                  list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                  flag=0;
                  id="";
                 // continue;
              }
             
              
              else if(chars[i-1]==';'&&chars[i]==';'||chars[i-1]==';'&&chars[i]==')'){
                   list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                  flag=0;
                  id="";
              }
              
              //BEA EDIT SA PLM
              else if(i-2>=0){
                  if(chars[i-2]=='='&&chars[i-1]==';'){
                         doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+chars[i-2]+""
                             + ", Line "+lineNumber+". '"+chars[i-1]+"' is not a valid delimiter.\n" , null);
              // list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                     //i++;
                     System.out.print("JJJJ");
                    
              if(i+1==(chars.length)){
                         id="";
                        
                    break;}
                     else{
                         id="";
                         flag=0;
                    //     break;
                     }
                           
              }
              }
               else if(chars[i-1]==';'&&chars[i]=='1'||chars[i-1]==';'&&chars[i]=='2'||chars[i-1]==';'&&chars[i]=='3'||chars[i-1]==';'&&chars[i]=='4'
                      ||chars[i-1]==';'&&chars[i]=='5'||chars[i-1]==';'&&chars[i]=='6'||chars[i-1]==';'&&chars[i]=='7'||chars[i-1]==';'&&chars[i]=='8'
                      ||chars[i-1]==';'&&chars[i]=='9')
                    {//ERROR TRAP 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter.\n" , null);
               list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                     i++;
                    
              if(i==(chars.length)){
                         id="";
                        
                    break;}
                     else{
                         id="";
                         flag=0;
                         break;
                     }
                 }
               
               else if(chars[i-1]=='&'&&chars[i]!='&'||chars[i-1]=='|'&&chars[i]!='|')            {//ERROR TRAP 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i-1]+"' is not a valid reserved symbol.\n" , null);
               list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                     i++;
                    
              if(i==(chars.length)){
                         id="";
                        
                    break;}
                     else{
                  
                         id="";
                         flag=0;
                         break;
                     }
                 }
              else if(chars[i-1]=='+'&&chars[i]=='+'||chars[i-1]=='-'&&chars[i]=='-'
                      ||chars[i-1]=='<'&&chars[i]=='<'||chars[i-1]=='>'&&chars[i]=='>'
                      ||chars[i]=='='||chars[i-1]=='&'&&chars[i]=='&'||chars[i-1]=='|'&&chars[i]=='|'){
                  
               System.out.print("EO na");
                id = id.concat(String.valueOf(chars[i]));
                i++;
                 if(i==chars.length){
                
                        list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                    break;}
                 else{
                     
                 list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                 id=""; //MILLER EDIT 1002
                 
                  if(chars[i-2]=='+'&&chars[i-1]=='+'&&chars[i]==';'
                     ||chars[i-2]=='-'&&chars[i-1]=='-'&&chars[i]==';' ||chars[i-1]==';'&&chars[i]==')'){
                  //MILLER EDIT 1002    
                  id = id.concat(String.valueOf(chars[i]));
                  list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                  i++;
                  //========================    
                  if(i==chars.length){
                      break;}
                  }
                  else if(chars[i]=='('||chars[i]=='{'||chars[i-1]==';'&&chars[i]==';'){
                   //   System.out.print("\n print "+id+" ");
                      //list.add(new Lexical(id, id, String.valueOf(lineNumber)));
                      //id="";
                      
                      if(chars[i]=='('||chars[i]=='{' ||chars[i-1]==';'&&chars[i]==';' ||chars[i-1]==';'&&chars[i]==')'){
                       id = id.concat(String.valueOf(chars[i]));
                  list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                  i++;
                      }
                      
                   
                       if(i==chars.length){
                      break;}
                  
                       else{
                  id="";
                 flag=0;
                 delimCheck=1;
                 
                       }
                  }
                  
                  id="";
                 flag=0;}
              //}//else
              }//IF ++ -- << >>'
              else{
                     //BEA EDIT 10/07/2018
                  if(chars[i]=='('||chars[i]=='{' ||chars[i-1]==';'&&chars[i]==';' ||chars[i-1]==';'&&chars[i]==')'){
                      System.out.print("\n print "+id+" ");
                      list.add(new Lexical(id, id, String.valueOf(lineNumber)));
                      id="";
                      
                      if(chars[i]=='('||chars[i]=='{' ||chars[i-1]==';'&&chars[i]==';' ||chars[i-1]==';'&&chars[i]==')'){
                       id = id.concat(String.valueOf(chars[i]));
                  list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                  i++;
                      }
                      
                   
                       if(i==chars.length){
                      break;}
                  
                       else{
                           
                  id="";
                 flag=0;
                 delimCheck=1;
                 
                       }
                  }
                  else if(chars[i]==' '||chars[i]=='\n'||chars[i]=='a'||chars[i]=='b'||chars[i]=='c'||chars[i]=='d'||chars[i]=='e'
           ||chars[i]=='f'||chars[i]=='g'||chars[i]=='h'||chars[i]=='i'||chars[i]=='j'
           ||chars[i]=='k'||chars[i]=='l'||chars[i]=='m'||chars[i]=='n'||chars[i]=='o'
           ||chars[i]=='p'||chars[i]=='q'||chars[i]=='r'||chars[i]=='s'||chars[i]=='t'
           ||chars[i]=='u'||chars[i]=='v'||chars[i]=='w'||chars[i]=='x'||chars[i]=='y'
           ||chars[i]=='z'||chars[i]=='0'||chars[i]=='1'||chars[i]=='2'||chars[i]=='3'||chars[i]=='4'
           ||chars[i]=='5'||chars[i]=='6'||chars[i]=='7'||chars[i]=='8'||chars[i]=='9'){
              
       
                list.add(new Lexical(id, id,String.valueOf(lineNumber)));
               
                      if(chars[i]==' '||chars[i]=='\n'){
                        i++;
                        
                      }
                      else if(chars[i]=='0'||chars[i]=='1'||chars[i]=='2'||chars[i]=='3'||chars[i]=='4'
           ||chars[i]=='5'||chars[i]=='6'||chars[i]=='7'||chars[i]=='8'||chars[i]=='9'){
//                          System.out.print("Hello");
                          flag =0;
                          
                          id="";
                          continue;
                        //  break;
                      }
           else if(chars[i]=='a'||chars[i]=='b'||chars[i]=='c'||chars[i]=='d'||chars[i]=='e'
           ||chars[i]=='f'||chars[i]=='g'||chars[i]=='h'||chars[i]=='i'||chars[i]=='j'
           ||chars[i]=='k'||chars[i]=='l'||chars[i]=='m'||chars[i]=='n'||chars[i]=='o'
           ||chars[i]=='p'||chars[i]=='q'||chars[i]=='r'||chars[i]=='s'||chars[i]=='t'
           ||chars[i]=='u'||chars[i]=='v'||chars[i]=='w'||chars[i]=='x'||chars[i]=='y'
           ||chars[i]=='z') {
                   id="";
                      id = id.concat(String.valueOf(chars[i]));
            i++;
             if(i==chars.length){
                  list.add(new Lexical(id, "Identifier",String.valueOf(lineNumber)));
                      break;}
             else if(chars[i]=='a'||chars[i]=='b'||chars[i]=='c'||chars[i]=='d'||chars[i]=='e'
           ||chars[i]=='f'||chars[i]=='g'||chars[i]=='h'||chars[i]=='i'||chars[i]=='j'
           ||chars[i]=='k'||chars[i]=='l'||chars[i]=='m'||chars[i]=='n'||chars[i]=='o'
           ||chars[i]=='p'||chars[i]=='q'||chars[i]=='r'||chars[i]=='s'||chars[i]=='t'
           ||chars[i]=='u'||chars[i]=='v'||chars[i]=='w'||chars[i]=='x'||chars[i]=='y'
           ||chars[i]=='z'||chars[i]=='_'||
             chars[i]=='A'||chars[i]=='B'||chars[i]=='C'||chars[i]=='D'||chars[i]=='E'
           ||chars[i]=='F'||chars[i]=='G'||chars[i]=='H'||chars[i]=='I'||chars[i]=='J'
           ||chars[i]=='K'||chars[i]=='L'||chars[i]=='M'||chars[i]=='N'||chars[i]=='O'
           ||chars[i]=='P'||chars[i]=='Q'||chars[i]=='R'||chars[i]=='S'||chars[i]=='T'
           ||chars[i]=='U'||chars[i]=='V'||chars[i]=='W'||chars[i]=='X'||chars[i]=='Y'
           ||chars[i]=='Z'){
             int check=1;
             while(check==1){
                 if(chars[i]=='a'||chars[i]=='b'||chars[i]=='c'||chars[i]=='d'||chars[i]=='e'
           ||chars[i]=='f'||chars[i]=='g'||chars[i]=='h'||chars[i]=='i'||chars[i]=='j'
           ||chars[i]=='k'||chars[i]=='l'||chars[i]=='m'||chars[i]=='n'||chars[i]=='o'
           ||chars[i]=='p'||chars[i]=='q'||chars[i]=='r'||chars[i]=='s'||chars[i]=='t'
           ||chars[i]=='u'||chars[i]=='v'||chars[i]=='w'||chars[i]=='x'||chars[i]=='y'
           ||chars[i]=='z'||chars[i]=='_'||
             chars[i]=='A'||chars[i]=='B'||chars[i]=='C'||chars[i]=='D'||chars[i]=='E'
           ||chars[i]=='F'||chars[i]=='G'||chars[i]=='H'||chars[i]=='I'||chars[i]=='J'
           ||chars[i]=='K'||chars[i]=='L'||chars[i]=='M'||chars[i]=='N'||chars[i]=='O'
           ||chars[i]=='P'||chars[i]=='Q'||chars[i]=='R'||chars[i]=='S'||chars[i]=='T'
           ||chars[i]=='U'||chars[i]=='V'||chars[i]=='W'||chars[i]=='X'||chars[i]=='Y'
           ||chars[i]=='Z'){
            id = id.concat(String.valueOf(chars[i]));
            i++;
           
            IdentifierCount++;
             if(i==chars.length){//if no delimiter found              
               list.add(new Lexical(id, "Identifier",String.valueOf(lineNumber)));
                        break;
                    }
            if(IdentifierCount<=32)
               check=1;
            else{
                check=0;
                  doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". Identifier character exceeds 32 limit.\n" , null);
               list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                     i++;
                    
              if(i==(chars.length)){
                         id="";
                        
                    break;}
                     else{
                         id="";
                         flag=0;
                         break;
                     }
            }
                  
                      }//if
                 
                 /////BOOKMARK NA DITO TY
                    else if(chars[i]=='+'||chars[i]=='-'||chars[i]=='*'||chars[i]=='/'||
          chars[i]=='%'||chars[i]=='!'||chars[i]=='<'||chars[i]=='>'||
                    chars[i]=='('||chars[i]==')'||chars[i]==';'||chars[i]==','||chars[i]=='}'||
                    chars[i]=='{'||chars[i]==' '||chars[i]=='\t'||chars[i]=='\r'
                   ||chars[i]=='\n'||chars[i]=='='||
                   chars[i]=='['||chars[i]==']'||chars[i]=='&'||chars[i]==','||chars[i]=='|'){
             System.out.print("aaaa");
                      
                          if(chars[i]==' '||chars[i]=='\n'||chars[i]=='\t'||chars[i]=='\r'){
                              if(chars[i]=='\n')
                               lineNumber++;
                           list.add(new Lexical(id, "Identifier",String.valueOf(lineNumber)));
                           i++; 
                           flag=0;
                           id="";
                          
                              break;
                          }
                          else{
                                list.add(new Lexical(id, "Identifier",String.valueOf(lineNumber)));
                         id="";
                          id = id.concat(String.valueOf(chars[i]));
                              list.add(new Lexical(id, id,String.valueOf(lineNumber)));i++;}
                        check=0;
                        if(i==chars.length)//if no delimiter found              
                        break;
                        else
                            flag=0;
                        break;
                    }
                 else {//ERROR TRAP 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter.\n" , null);
               list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                     i++;
                    
              if(i==(chars.length)){
                         id="";
                        flag=8;
                    break;}
                     else{
                         id="";
                         flag=0;
                      
                     }
                 }
             }//while
           if(flag!=0){
               list.add(new Lexical(id, id,String.valueOf(lineNumber)));
               System.out.print("RE");
           }
           else{
              // System.out.print("Hello");
              id=""; flag=0;break;}
            }//if   
             else{
             list.add(new Lexical(id, "Identifier",String.valueOf(lineNumber))); id="";// i++;
            flag=0; System.out.print("Eto:"+chars[i]+" ");
             }
             
           if(i+1==chars.length&&chars[i]==';')//if no delimiter found    {
           {
               //System.out.print("REzzz");
               id = id.concat(String.valueOf(chars[i]));
              list.add(new Lexical(id, id, String.valueOf(lineNumber))); id="";// i++;
            break;
               
           }
           else if(flag==0)
               flag=0;
         
            else {//ERROR TRAP 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter.\n" , null);
               list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                     i++;
                    
              if(i==(chars.length)){
                         id="";
                        flag=8;
                    break;}
                     else{
                         id="";
                         flag=0;
                         break;
                     }
                 }
             
           }//else if  
                   else{
               System.out.print("Here an");
                  list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                  id="";
                  id = id.concat(String.valueOf(chars[i]));
                 // list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                    i++;
                     }
                     if(i==chars.length){
                      break;}
                     else{
                  id="";
                 flag=0;}
                  }//if
              
                  //MILLER =+
                  else if(i+1!=chars.length){
                  if((chars[i]=='+'&&chars[i+1]=='+')||(chars[i]=='-'&&chars[i]=='-')){
                      list.add(new Lexical(id, id, String.valueOf(lineNumber)));
                      id="";
                      if((chars[i]=='+'))
                      list.add(new Lexical("++", "++", String.valueOf(lineNumber)));
                      else if((chars[i]=='-'))
                      list.add(new Lexical("--", "--", String.valueOf(lineNumber)));
                      if(i+2!=chars.length)
                      i=i+2;
                      else{
                          break;
                      }
                      flag=0;
                     System.out.print("\nHere\n");
                  }
                  }
                  
                  //=++
                  //================/================================================
                  
                  else {//ERROR TRAP 
                     
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter.\n" , null);
               list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                     i++;
                    
              if(i==(chars.length)){
                         id="";
                        flag=8;
                    break;}
                     else{
                         id="";
                         flag=0;
                        //break;
                     }
               
                 }
                 
              }
            }
   
//========================================================STRING LITERAL====
      
      //ERROR ON ESCAPE SEQUENCE '//'
      else if(chars[i]=='"'){
          try{
           id = id.concat(String.valueOf(chars[i]));
          
          i++;
         while(chars[i]!='"'){
              if(chars[i]=='\\'&&chars[i+1]!='\\'&&
                chars[i]=='\\'&&chars[i+1]!='n'&&   
                 chars[i]=='\\'&&chars[i+1]!='t'&&   
                 chars[i]=='\\'&&chars[i+1]=='"'&&chars[i+2]!='"'){
        id = id.concat("\\");
        id = id.concat(String.valueOf(chars[i+1]));
                // ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> LEXICAL ERROR on <span style=\"color:red\">"+id+""
                //             + "</span>,<span style=\"color:red\"> Line "+lineNumber+"</span>. '"+chars[i+1]+"' is not a valid String Literal. [\\] is expected.");
                  doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid String Literal.[\\] is expected.\n" , null);
               list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                     i++;
                    
              if(i==(chars.length)){
                         id="";
                        flag=8;
                    break;}
                     else{
                         id="";
                         flag=0;
                         break;
                     }
                 //flag=8;
                 //break; 
             }
             else if(chars[i]=='\\' && chars[i+1]=='n'){
                 id = id.concat("\n");
                 i=i+2;
             }
             else if(chars[i]=='\\' && chars[i+1]=='t'){
                 id = id.concat("\t");
                 i=i+2;
             }
              else if(chars[i]=='\\' && chars[i+1]=='\\'){
                 id = id.concat("\\");
                 i=i+2;
             }
             else if(chars[i]=='\\' && chars[i+1]!='\\'){
//                 id = id.concat("\\");
id="";
 doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+", Line "+lineNumber+". '"+chars[i]+"' is not a valid escape character.\n" , null);
 
                 i=i+2;
             }
           
                else if(chars[i]=='\\' && chars[i+1]=='"'){
                 id = id.concat("\"");
                 i=i+2;
             }
   
            else if(chars[i+1]=='\n'){
             id = id.concat(String.valueOf(chars[i]));
            lineNumber++;
            i++;
            }
             else{
             id = id.concat(String.valueOf(chars[i]));
             i++;}
                if(i+1==chars.length&&chars[i]!='"'){
         // ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> LEXICAL ERROR on <span style=\"color:red\">"+id+""
           //                  + "</span>,<span style=\"color:red\"> Line "+lineNumber+"</span>. '"+chars[i]+"' is not a valid delimiter. \" is expected.");
//        doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. \" is expected.\n" , null);
                        list.add(new Lexical(id,"String Literal", String.valueOf(lineNumber)));
                      flag=8;
                    break;

                }
         }
         
        
          if(chars[i]=='"'){ 
         
         id = id.concat(String.valueOf(chars[i]));         list.add(new Lexical(id, "String Literal",String.valueOf(lineNumber)));
          
         i++;
         if(i==chars.length){         
              break;
          }
    
         
         else{
             if(chars[i]==')'||chars[i]=='('||chars[i]=='}'||chars[i]==')'||chars[i]=='\r'||chars[i]=='\t'||chars[i]=='\n'||chars[i]==' '||chars[i]==';'||chars[i]==','
                     ||chars[i]=='+'||chars[i]=='!'||chars[i]=='='){
                    id="";
         //id = id.concat(String.valueOf(chars[i])); BEA EDIT 10/11/2018
           //list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                    if((i+1)==chars.length){
                         id="";
                        id = id.concat(String.valueOf(chars[i]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        
                        break;}
                    else{
//                         id="";
//                        id = id.concat(String.valueOf(chars[i]));
//                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
//                        
//                        
//                        i++;
                        flag=0;
                        id="";}
             }   
                else{//ERROR TRAP 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter.\n" , null);
               list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                     i++;
                    
              if(i==(chars.length)){
                         id="";
                        flag=8;
                    break;}
                     else{
                         id="";
                         flag=0;
                         break;
                     }
                 }
//             else{
//                 i++;
//             flag=0;
//             id="";}
         }
      } //if"
          } catch(ArrayIndexOutOfBoundsException e){
                        System.out.println("Out of bounds error occured!!\n");
                        i--;
                         break;
                    }
         }
      
//==========================================================================
//===============================================================CHAR LITERAL
     else if(chars[i]=='\''){
     id="";
         id = id.concat(String.valueOf(chars[i]));
//           list.add(new Lexical(id, id,String.valueOf(lineNumber)));
//           id="";
       i++;
             id = id.concat(String.valueOf(chars[i]));
      
      i++;
      if(i==chars.length){//if no delimiter found
              
//                     doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
//                             + ", Line "+lineNumber+". No delimiter found. \n" , null);
//                        list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                        flag=8;
                    break;
                    }
      else if(chars[i]=='\''){
            
         id = id.concat(String.valueOf(chars[i])); list.add(new Lexical(id, "Character Literal",String.valueOf(lineNumber)));
          
         if((i+1)==chars.length){    
             
              break;
          }
    
         else{
             i++;
             if(chars[i]=='='||chars[i]=='!'||chars[i]=='}'||chars[i]=='('||chars[i]==')'||chars[i]=='\r'||chars[i]=='\t'||chars[i]==' '
                     ||chars[i]=='\n'||chars[i]==';'||chars[i]==','){
                    if((i+1)==chars.length){
                         id = id.concat(String.valueOf(chars[i]));
                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        
                        break;}
                    else{
                        //System.out.print("hello");
//                        id="";
//                        id = id.concat(String.valueOf(chars[i]));
//                        list.add(new Lexical(id,id,String.valueOf(lineNumber)));
//                        
//                        i++;
                        flag=0;
                        id="";}
             }   
                  else 
             {//ERROR TRAP 
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter.\n" , null);
               list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                     i++;
                    
              if(i==(chars.length)){
                         id="";
                        flag=8;
                    break;}
                     else{
                         id="";
                         flag=0;
                         break;
                     }
                 }
//             else{
//             flag=0;
//             id="";}
         }
         }//if '
         //ERROR TRAp=============
          else if(chars[i]!='\''){
          //    ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> LEXICAL ERROR on <span style=\"color:red\">"+id+""
            //                 + "</span>,<span style=\"color:red\"> Line "+lineNumber+"</span>. '"+chars[i]+"' is not a valid delimiter. [\'] is expected.");
          LexError=1;
          doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid delimiter. [\'] is expected.\n" , null);
               list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                     i++;
                    
              if(i==(chars.length)){
                         id="";
                        flag=8;
                    break;}
                     else{
                         id="";
                         flag=0;
                         break;
                     }
          }
           
         }
     
     
//===========================================================================

 
      else{
          id = id.concat(String.valueOf(chars[i]));
          flag=9;
      }
      
//=================== IDENTIFIER ==============================================
      if(flag==3){
     
          int check, checker;
         
           if(chars[i]=='.'||chars[i]=='+'||chars[i]=='-'||chars[i]=='*'||chars[i]=='/'||
          chars[i]=='%'||chars[i]=='!'||chars[i]=='<'||chars[i]=='>'||
                    chars[i]=='('||chars[i]==')'||chars[i]==';'||chars[i]==','||chars[i]=='}'||
                    chars[i]=='{'||chars[i]==' '||chars[i]=='\t'||chars[i]=='\r'
                   ||chars[i]=='\n'||chars[i]=='='||
                   chars[i]=='['||chars[i]==']'||chars[i]=='&'||chars[i]==','||chars[i]=='|'){//DELIMITER
               
               
               //IF RESERVED WORD IS SLIGHTLY SIMILAR TO IDENTIFIER
                check=1;
            checker=0;
            //RESERVED OPERATORS========================================
            if(chars[i]=='='||chars[i]=='+'||chars[i]=='-'||chars[i]=='*'||chars[i]=='/'||
            chars[i]=='%'||chars[i]=='!'||chars[i]=='<'||chars[i]=='>'||chars[i]=='&'||chars[i]=='|')
            {
                if(chars[i]=='+'&&chars[i+1]=='"'){
                  //
                       list.add(new Lexical(id,"Identifier",String.valueOf(lineNumber)));
              id="";
              id = id.concat(String.valueOf(chars[i]));list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                  flag=0;
                  id="";
               i++;
               continue;
                }
                 // continue;
              
                else{
              list.add(new Lexical(id,"Identifier",String.valueOf(lineNumber)));
              id="";
              id = id.concat(String.valueOf(chars[i]));
           
              i++;
                }
               if(i==chars.length){
               list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        break;
               }
           
                //BEA EDIT 10/07/2018
               else if(chars[i]=='('||chars[i]=='{'){
                      System.out.print("\n print "+id+" ");
                      list.add(new Lexical(id, id, String.valueOf(lineNumber)));
                      id="";
                      
                      if(chars[i]=='('||chars[i]=='{'){
                       id = id.concat(String.valueOf(chars[i]));
                  list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                  i++;
                      }
                   
                       if(i==chars.length){
                      break;}
                  
                       else{
                  id="";
                 flag=0;
                 checker=1;
                 
                 
                       }
                  }
               else{
               if(chars[i-1]=='+'&&chars[i]=='+'||chars[i-1]=='-'&&chars[i]=='-'
                      ||chars[i-1]=='<'&&chars[i]=='<'||chars[i-1]=='>'&&chars[i]=='>'
                      ||chars[i]=='='||chars[i-1]=='&'&&chars[i]=='&'||chars[i-1]=='|'&&chars[i]=='|'){
                   System.out.print("Here po");
                   id = id.concat(String.valueOf(chars[i]));
                   if((i+1)==chars.length){
               list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        break;
               }
                    else{
                 
                   list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                  if(chars[i+1]=='('){
                   //   System.out.print("\n print "+id+" ");
                      //list.add(new Lexical(id, id, String.valueOf(lineNumber)));
                      id="";
                      
                      if(chars[i+1]=='('||chars[i]=='{'){
                       id = id.concat(String.valueOf(chars[i+1]));
                  list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                  i++;
                      }
                      
                   
                       if(i==chars.length){
                      break;}
                  
                       else{
                  id="";
                 flag=0;
                 delimCheck=1;
                 
                       }
                  }
                   id="";
                   flag=0;
                   i++;
                   checker=1;
                   }
               }
               else{
                   System.out.print("Check 0");  
                   
                   list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                   //id="";
             // id = id.concat(String.valueOf(chars[i]));
                
                   
                   
                  // else{
                       checker=1;
                  System.out.print("Check 1");    
                 // break;
                  // }
            //  checker=1;
               }
              //break;
            }
            
            }
            
   
            //===========================================================
            else //if not arith op
                i++;
        if((i==(chars.length))&&checker==0){//IF NOT ARITH OP
                   if(chars[i-1]==' '||chars[i-1]=='\n'||chars[i-1]=='\t'||chars[i-1]=='\r')
                list.add(new Lexical(id,"Identifier",String.valueOf(lineNumber)));
                  else{
                  list.add(new Lexical(id,"Identifier",String.valueOf(lineNumber)));
                id="";
               id = id.concat(String.valueOf(chars[i-1]));
               list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                          }
                break;
                }
             else if(i==(chars.length)&&checker==1){//IF ARITH OP
                  //list.add(new Lexical(id,"Identifier",String.valueOf(lineNumber)));
                
                  flag=8;
                }
              else if(checker==1){//IF MAY KASUNOD YUNG ARITH OP
                  
                   if(chars[i-2]=='+'&&chars[i-1]=='+'&&chars[i]==';'
                     ||chars[i-2]=='-'&&chars[i-1]=='-'&&chars[i]==';'){i++;
                  if(i==chars.length)
                      break;
                   id = id.concat(String.valueOf(chars[i-1]));
                  
               list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                   flag=0;
                id="";
                  }
                   else if(chars[i]=='a'||chars[i]=='b'||chars[i]=='c'||chars[i]=='d'||chars[i]=='e'
           ||chars[i]=='f'||chars[i]=='g'||chars[i]=='h'||chars[i]=='i'||chars[i]=='j'
           ||chars[i]=='k'||chars[i]=='l'||chars[i]=='m'||chars[i]=='n'||chars[i]=='o'
           ||chars[i]=='p'||chars[i]=='q'||chars[i]=='r'||chars[i]=='s'||chars[i]=='t'
           ||chars[i]=='u'||chars[i]=='v'||chars[i]=='w'||chars[i]=='x'||chars[i]=='y'
           ||chars[i]=='z'||chars[i]=='1'||chars[i]=='2'||chars[i]=='3'||chars[i]=='4'||chars[i]=='5'
                    ||chars[i]=='6'||chars[i]=='7'||chars[i]=='8'||chars[i]=='9'||chars[i]=='0'||chars[i]=='('||chars[i]==' '
                           ||chars[i]=='\n'||chars[i-1]=='='&&chars[i]=='\''||chars[i-1]=='='&&chars[i]=='"'){
                    //   i++;
                   // System.out.print("Here pls");
                  if(i+1==chars.length){
                      list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                      break;}
//                  else if(chars[i]==' '||chars[i]=='\n'){
//                      i++;
//                      System.out.print("Here pls");
//                       if(i==chars.length)
//                      break;
//                       else if(chars[i]=='a'||chars[i]=='b'||chars[i]=='c'||chars[i]=='d'||chars[i]=='e'
//           ||chars[i]=='f'||chars[i]=='g'||chars[i]=='h'||chars[i]=='i'||chars[i]=='j'
//           ||chars[i]=='k'||chars[i]=='l'||chars[i]=='m'||chars[i]=='n'||chars[i]=='o'
//           ||chars[i]=='p'||chars[i]=='q'||chars[i]=='r'||chars[i]=='s'||chars[i]=='t'
//           ||chars[i]=='u'||chars[i]=='v'||chars[i]=='w'||chars[i]=='x'||chars[i]=='y'
//           ||chars[i]=='z'||chars[i]=='1'||chars[i]=='2'||chars[i]=='3'||chars[i]=='4'||chars[i]=='5'
//                    ||chars[i]=='6'||chars[i]=='7'||chars[i]=='8'||chars[i]=='9'||chars[i]=='0'||chars[i]=='('||chars[i]==' '
//                           ||chars[i]=='\n'||chars[i]=='\''){
//                        flag=0;
//                        id="";
//                       // break;
//                       }
//                       
//                       else{
//                                  doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
//                             + ", Line "+lineNumber+". '"+chars[i]+"' is an invalid delimiter\n" , null);
//               //list.add(new Lexical(id,id, String.valueOf(lineNumber)));
//                     i++;
//                    
//              if(i==(chars.length)){
//                         id="";
//                        flag=8;
//                    break;}
//                     else{
//                         id="";
//                         flag=0;
//                         break;
//                     } 
//                       }
//                  }
                  else{
                   flag=0;
                id="";}
                   }
                   else{
                               doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is an invalid delimiter\n" , null);
               //list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                     i++;
                    
              if(i==(chars.length)){
                         id="";
                        flag=8;
              //    break;
              }
                     else{
                         id="";
                         flag=0;
                         //break;
                     }
                   }
                
                
               
                //break; 
              }
              else if(checker==0){
                
                
                if(chars[i-1]==' '||chars[i-1]=='\n'||chars[i-1]=='\t'||chars[i-1]=='\r'){
                list.add(new Lexical(id,"Identifier",String.valueOf(lineNumber)));
                if(chars[i-1]=='\n'){
                    lineNumber++;
                }
                }
                else{
               list.add(new Lexical(id,"Identifier",String.valueOf(lineNumber)));
               id="";
              id = id.concat(String.valueOf(chars[i-1]));
              list.add(new Lexical(id, id,String.valueOf(lineNumber)));
          
                }
                    
               flag=0;
                id="";
                //break;
              }
                 }//IF SYMBOL
          
           else{ //IF NOT SYMBOL, PROCEED TO LETTERS
               check=0;
       
           }
          while(check==0){
         
          if(chars[i]=='a'||chars[i]=='b'||chars[i]=='c'||chars[i]=='d'||chars[i]=='e'
           ||chars[i]=='f'||chars[i]=='g'||chars[i]=='h'||chars[i]=='i'||chars[i]=='j'
           ||chars[i]=='k'||chars[i]=='l'||chars[i]=='m'||chars[i]=='n'||chars[i]=='o'
           ||chars[i]=='p'||chars[i]=='q'||chars[i]=='r'||chars[i]=='s'||chars[i]=='t'
           ||chars[i]=='u'||chars[i]=='v'||chars[i]=='w'||chars[i]=='x'||chars[i]=='y'
           ||chars[i]=='z'||chars[i]=='_'||
             chars[i]=='A'||chars[i]=='B'||chars[i]=='C'||chars[i]=='D'||chars[i]=='E'
           ||chars[i]=='F'||chars[i]=='G'||chars[i]=='H'||chars[i]=='I'||chars[i]=='J'
           ||chars[i]=='K'||chars[i]=='L'||chars[i]=='M'||chars[i]=='N'||chars[i]=='O'
           ||chars[i]=='P'||chars[i]=='Q'||chars[i]=='R'||chars[i]=='S'||chars[i]=='T'
           ||chars[i]=='U'||chars[i]=='V'||chars[i]=='W'||chars[i]=='X'||chars[i]=='Y'
           ||chars[i]=='Z'||chars[i]=='1'||chars[i]=='2'||chars[i]=='3'||chars[i]=='4'||chars[i]=='5'
                    ||chars[i]=='6'||chars[i]=='7'||chars[i]=='8'||chars[i]=='9'){
            
              
                if(chars[i]=='+'&&chars[i+1]=='"'){
                  //list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                  flag=0;
                  id="";
               i++;
               continue;
                }
                else{
              id = id.concat(String.valueOf(chars[i]));
            i++;
           
            IdentifierCount++;}
             if(i==chars.length){//if no delimiter found
              
list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
flag=8;
                    break;
                    }
            if(IdentifierCount<=32)
               check=0;
            else{
             //  ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> LEXICAL ERROR on <span style=\"color:red\">"+id+""
               //              + "</span>,<span style=\"color:red\"> Line "+lineNumber+"</span>. Identifier characters exceeds the 31 limit.");
              //break;
              doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". Identifier characters exceeds the 31 limit.\n" , null);
               list.add(new Lexical(id,"Identifier", String.valueOf(lineNumber)));
                     i++;
                    
              if(i==(chars.length)){
                         id="";
                        flag=8;
                    break;}
                     else{
                         id="";
                         flag=0;
                         break;
                     }
            
            }
            }//if
            
            
             else if(chars[i]=='.'||chars[i]=='+'||chars[i]=='-'||chars[i]=='*'||chars[i]=='/'||
          chars[i]=='%'||chars[i]=='!'||chars[i]=='<'||chars[i]=='>'||
                    chars[i]=='('||chars[i]==')'||chars[i]==';'||chars[i]==','||
                    chars[i]=='{'||chars[i]==' '||chars[i]=='\n'||chars[i]=='='||
                   chars[i]=='['||chars[i]==']'||chars[i]=='&'||chars[i]==','||chars[i]=='|'){//DELIMITER
               //IF RESERVED WORD IS SLIGHTLY SIMILAR TO IDENTIFIER
                check=1;
            checker=0;
            //RESERVED OPERATORS========================================
            if(chars[i]=='='||chars[i]=='+'||chars[i]=='-'||chars[i]=='*'||chars[i]=='/'||
            chars[i]=='%'||chars[i]=='!'||chars[i]=='<'||chars[i]=='>'||chars[i]=='&'||chars[i]=='|')
            {
              list.add(new Lexical(id,"Identifier",String.valueOf(lineNumber)));
              id="";
              id = id.concat(String.valueOf(chars[i]));
              //list.add(new Lexical(id, id));
              i++;
if(i==chars.length){
               list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        break;
               }
               else{
               if(chars[i-1]=='+'&&chars[i]=='+'||chars[i-1]=='-'&&chars[i]=='-'
                      ||chars[i-1]=='<'&&chars[i]=='<'||chars[i-1]=='>'&&chars[i]=='>'
                      ||chars[i]=='='||chars[i-1]=='&'&&chars[i]=='&'||chars[i-1]=='|'&&chars[i]=='|'){
                   id = id.concat(String.valueOf(chars[i]));
                   if((i+1)==chars.length){
               list.add(new Lexical(id,id,String.valueOf(lineNumber)));
                        break;
               }
                    else{
                 
                   list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                                if(chars[i+1]=='('||chars[i]=='{'){
                   //   System.out.print("\n print "+id+" ");
                      //list.add(new Lexical(id, id, String.valueOf(lineNumber)));
                      id="";
                      
                      if(chars[i+1]=='('||chars[i]=='{'){
                       id = id.concat(String.valueOf(chars[i+1]));
                  list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                  i++;
                      }
                      
                   
                       if(i==chars.length){
                      break;}
                  
                       else{
                  id="";
                 flag=0;
                 delimCheck=1;
                 
                       }
                  }
                   i++;}
               }
         
                 //BEA EDIT 10/07/2018
               else if(chars[i]=='('||chars[i]=='{'){
                      System.out.print("\n print "+id+" ");
                      list.add(new Lexical(id, id, String.valueOf(lineNumber)));
                      id="";
                      
                      if(chars[i]=='('||chars[i]=='{'){
                       id = id.concat(String.valueOf(chars[i]));
                  list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                  i++;
                      }
                   
                       if(i==chars.length){
                      break;}
                  
                       else{
                  id="";
                 flag=0;
                 checker=1;
                 
                 
                       }
                  }

               else
                   list.add(new Lexical(id, id,String.valueOf(lineNumber)));
              checker=1;
              //break;
            }}
            
            //===========================================================
            else //if not arith op
                i++;
            if((i==(chars.length))&&checker==0){//IF NOT ARITH OP
                   if(chars[i-1]==' '||chars[i-1]=='\n'||chars[i-1]=='\t'||chars[i-1]=='\r')
                list.add(new Lexical(id,"Identifier",String.valueOf(lineNumber)));
                  else{
                  list.add(new Lexical(id,"Identifier",String.valueOf(lineNumber)));
                id="";
               id = id.concat(String.valueOf(chars[i-1]));
               list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                          }
                break;
                }
              else if(i==(chars.length)&&checker==1){//IF ARITH OP
                  //list.add(new Lexical(id,"Identifier"));
                  break;
                }
              else if(checker==1){//IF MAY KASUNOD YUNG ARITH OP
                    if(chars[i-2]=='+'&&chars[i-1]=='+'&&chars[i]==';'
                     ||chars[i-2]=='-'&&chars[i-1]=='-'&&chars[i]==';'){i++;
                     id="";
                     id = id.concat(String.valueOf(chars[i-1]));
                  list.add(new Lexical(id, id,String.valueOf(lineNumber)));
                  if(i==chars.length)
                      break;
                  }
                    
                 flag=0;
                id="";
                
                
                //break; 
              }
           else if(checker==0){
                
                
                       if(chars[i-1]==' '||chars[i-1]=='\n'||chars[i-1]=='\t'||chars[i-1]=='\r'){
                list.add(new Lexical(id,"Identifier",String.valueOf(lineNumber)));
                  if(chars[i-1]=='\n'){
                    lineNumber++;
                }
                       }
                else{
               list.add(new Lexical(id,"Identifier",String.valueOf(lineNumber)));
               id="";
              id = id.concat(String.valueOf(chars[i-1]));
              list.add(new Lexical(id, id,String.valueOf(lineNumber)));
          
                }
                    
               flag=0;
                id="";
                //break;
              }
             }//else if DELIMITER
            
             else{//ERROR TRAP FOR DELIMITER OF Identifier
               // ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> LEXICAL ERROR on <span style=\"color:red\">"+id+""
                 //            + "</span>,<span style=\"color:red\"> Line "+lineNumber+"</span>. '"+chars[i]+"' is not a valid delimiter.");
            // break;
                    doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". '"+chars[i]+"' is not a valid00 delimiter.\n" , null);
               list.add(new Lexical(id,id, String.valueOf(lineNumber)));
                     i++;
                    
              if(i==(chars.length)){
                         id="";
                        flag=8;
                    break;}
                     else{
                         id="";
                         flag=0;
                         break;
                     }
                 }
          }

      if(flag==9)
          System.out.print("Error");
          //list.add(new Lexical("error","error"));
          
   //LORZANO BEATRIZ 2018
      }///if flag =3 
      
       else if(flag==9){
           if(id.equals("0"))
               System.out.print("");
           else{
                  doc.insertString(doc.getLength(), "\nLEXICAL ERROR on "+id+""
                             + ", Line "+lineNumber+". It is not a valid character for a reserved word, symbol, or Identifier.\n" , null);}
              
                     i++;
                    
              if(i==(chars.length)){
                         id="";
                        flag=8;
                    break;}
                     else{
                         id="";
                         flag=0;
//                         /break;
                     }
          
      }
       else if(flag==8){
           //System.out.print("Token:"+list.get(0).Token);
           break;
       }
        else if(flag==0){
          continue; }
       else
          //list.add(new Lexical("error","error"));
            System.out.print("error");
      
      if(flag==0){
          System.out.print("Cont:"+flag);
         //break gohere;
      }
  
     } //if flag =0 ---the program will check again
      
     int size = list.size();
    //   System.out.print(size);
        return list;
  
    }
    
    
    public class Syntax{
       public String Production;
       public String Syntax;
 
        public Syntax(String Production, String Syntax)
        {
            this.Production = Production;
            this.Syntax = Syntax;
            
        
        }
    }

 int arrayFlag = 0;
 String dtype="";
  int braceFlag=0;
//==================================================Syntax Analyzer==========================================
    
    public ArrayList SynAnalyze() throws BadLocationException{
        ErrorTxt.setText(null);
        int listctr=0, flag=0, LexError=0; //const==1
        int operandCtr = 0;
       
  
        ArrayList<Lexical> list = ListLexical();
        ArrayList<Syntax> listSyntax = new ArrayList<Syntax>();
        String iden="";
       // ErrorTxt.getDocument().getText(flag, WIDTH)

                String fontfamily = ErrorTxt.getFont().getFamily();//font for error list
     if(ErrorTxt.getDocument().getLength()!=0){
             ErrorTxt.setText(null);
                    JOptionPane.showMessageDialog(null, "Lexical Error found! Click Lexical button to check","Error",JOptionPane.ERROR_MESSAGE);
                    LexError=1;
                   
                    
                }
     
     
if(LexError==0){
        while(listctr<=list.size()){
             if(list.get(0).Token=="attention"){
               // listSyntax.add(new Lexical("<program",));
                listctr=listctr+2;
                iden ="";
                iden = iden.concat("attention: ");
                listSyntax.add(new Syntax("<program> ",iden));
               
                
             }//if attention
             else{
                 ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting 'attention:' to start the program. ");
             break;
             }//else not attention
              /////DISMISS///////////////////////////////////////////////////////////////////////
             if(list.get(list.size()-1).Token=="dismiss"){
                  iden="";
                iden = iden.concat("dismiss ");
                listSyntax.add(new Syntax("<program> ",iden));
                System.out.print("nn["+(list.size()-1)+" ");
             }//if dismiss
             else{
         ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR: Expecting 'dismiss' to end the program. ");
               System.out.print("ERROR");
              break;
             }//else not dismisss
             ////1 GLOBAL DECLARATION//////////////////////////////////////////////////////////
             if(list.get(listctr).Token=="dismiss"&&(list.size()-1)==listctr){
                 System.out.print("");
             }
             else if(list.get(listctr).Token=="const"||list.get(listctr).Token=="int"||list.get(listctr).Token=="flt"
                    ||list.get(listctr).Token=="chr"||list.get(listctr).Token=="bool"||list.get(listctr).Token=="str"||list.get(listctr).Token=="assemble"||list.get(listctr).Token=="onduty"){
               
                  iden="";
                           iden = iden.concat("<gdeclr>");
                         listSyntax.add(new Syntax("<program> ",iden));
                 
                  while(list.get(listctr).Token=="const"||list.get(listctr).Token=="int"||list.get(listctr).Token=="flt"
                    ||list.get(listctr).Token=="chr"||list.get(listctr).Token=="bool"||list.get(listctr).Token=="str"||list.get(listctr).Token=="assemble"||list.get(listctr).Token=="onduty"){
                    
                    if(list.get(listctr).Token=="const"){
                       while(list.get(listctr).Token.equals("const")){
                           iden="";
                           iden = iden.concat("<constdec>");
                         listSyntax.add(new Syntax("<gdeclr> ",iden));
                         ///
                          iden="";
                            iden = iden.concat(list.get(listctr).Token);
                            listSyntax.add(new Syntax("<constdec> ",iden));
                            
                         listctr++;
                         
                          if(list.get(listctr).Token=="int"||list.get(listctr).Token=="flt"||list.get(listctr).Token=="chr"||list.get(listctr).Token=="bool"||list.get(listctr).Token=="str"){
                            dtype = list.get(listctr).Token;
                              iden="";
                            iden = iden.concat("<cdtype>");
                            listSyntax.add(new Syntax("<constdec> ",iden));
                            iden="";
                             iden = iden.concat(list.get(listctr).Token+" ");
                              listctr++;
                            
                            
                            if(list.get(listctr).Token=="Identifier"){
                                iden = iden.concat("id");
                                 listSyntax.add(new Syntax("<cdtype> ",iden));
                                listctr++;
                                
                                if(list.get(listctr).Token.equals("=")){
                                    
                                    listctr++;
                                     System.out.print("ETO NAPO:"+list.get(listctr).Token);
                                     
                                     System.out.print("ETO NAPO:"+dtype);
                                  if(list.get(listctr).Token=="false"||list.get(listctr).Token=="true"||list.get(listctr).Token=="Int Literal"||list.get(listctr).Token=="Float Literal"||list.get(listctr).Token.equals("(")||list.get(listctr).Token.equals("Character Literal")||list.get(listctr).Token.equals("String Literal")){
                                    
                            
                                 
                                        if(dtype=="int"&&list.get(listctr).Token=="Int Literal"){
                                              iden="";
                            iden = iden.concat("<init1>");
                            listSyntax.add(new Syntax("<cdtype> ",iden));
                                    iden="";
                            
                                            iden = iden.concat("=");
                            listSyntax.add(new Syntax("<init1> ",iden));
                                                 iden="";
                            iden = iden.concat("<intlit1>");
                            listSyntax.add(new Syntax("<init1> ",iden));
                                            iden="";
                            iden = iden.concat("integer literal");
                            listSyntax.add(new Syntax("<intlit1> ",iden));
                                            listctr++;
                                        }
                                         else if(dtype=="flt"&&list.get(listctr).Token=="Float Literal"){
                                                    iden="";
                            iden = iden.concat("<init2>");
                            listSyntax.add(new Syntax("<cdtype> ",iden));
                                    iden="";
                            
                                            iden = iden.concat("=");
                            listSyntax.add(new Syntax("<init2> ",iden));
                                                 iden="";
                            iden = iden.concat("<fltlit>");
                            listSyntax.add(new Syntax("<init2> ",iden));
                                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<fltlit> ",iden));
                                            listctr++;
                                        }
                                         else if(dtype=="bool"&&list.get(listctr).Token=="true"||
                                                 dtype=="bool"&&list.get(listctr).Token=="false"){
                                             
                                                   iden="";
                            iden = iden.concat("<init3>");
                            listSyntax.add(new Syntax("<cdtype> ",iden));
                                    iden="";
                            
                                            iden = iden.concat("=");
                            listSyntax.add(new Syntax("<init3> ",iden));
                                                 iden="";
                            iden = iden.concat("<boollit>");
                            listSyntax.add(new Syntax("<init3> ",iden));
                                            iden="";
                            iden = iden.concat("boolean literal");
                            listSyntax.add(new Syntax("<boolit> ",iden));
                                            listctr++;
                                        }
                                      else if(dtype=="str"&&list.get(listctr).Token=="String Literal"){
                                         
                                                    iden="";
                           iden = iden.concat("<init4>");
                            listSyntax.add(new Syntax("<cdtype> ",iden));
                                    iden="";
                            
                                            iden = iden.concat("=");
                            listSyntax.add(new Syntax("<init4> ",iden));
                                                 iden="";
                            iden = iden.concat("<strlit>");
                            listSyntax.add(new Syntax("<init4> ",iden));
                                            iden="";
                            iden = iden.concat("String Literal");
                            listSyntax.add(new Syntax("<strlit> ",iden));
                                            listctr++;
                                        }
                                      else if(dtype=="chr"&&list.get(listctr).Token=="Character Literal"){
                                                    iden="";
                            iden = iden.concat("<init5>");
                            listSyntax.add(new Syntax("<cdtype> ",iden));
                                    iden="";
                            iden = iden.concat("=");
                            listSyntax.add(new Syntax("<init5> ",iden));
                                                iden="";
                            iden = iden.concat("Character Literal");
                            listSyntax.add(new Syntax("<chrlit> ",iden));
                                            listctr++;
                                        }
                                        else if(dtype=="int"&&list.get(listctr).Token.equals("(")){
                                            listctr++;
                                            if(list.get(listctr).Token=="Negative Int Literal"){
                                                
                                                listctr= listctr+2;
                                            }
                                      
                                            else{
                                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting 'Negative Integer Literal'. ");  
                                             flag=1;
                                       break; 
                                            }//else not negative int lit 
                                            //IBOOKMARK MO TO
                                        }
                                        else{
                                            
                                         ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting "+dtype+" literal. ");  
                                         flag=1;
                                       break;
                                        }
                                        
                                         if(list.get(listctr).Token.equals(";")||list.get(listctr).Token.equals(" ;")){
                                          iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<constdec> ",iden));
                                    
                            
                                            listctr++;
                                            break;
                                        }
                                         else{
                                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';'. ");  
                                            flag=1;
                                       break; 
                                         }
                                    }//if LITERAL
                                    else{
                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '"+dtype+" literal'. ");
                                        flag=1;
                                    break;
                                    }//else not LITERAL
                                }//if
                                else{
                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '='. ");
                                        flag=1;
                                break;
                                }//else not = 
                                
                            }//if Identifier
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting 'Identifier'. "); 
                             flag=1;
                            break;
                            }//else not Identifier
                          }//if 
                          else{
                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting 'int', 'flt', 'chr', 'str', or 'bool'. "); 
                            flag=1;
                          break;
                          }//else
//                         
                       }//WHILE CONST
                    if(flag==1)
                        break;
                    }//IF CONST
                  ///////////////////////////////STRUCT DECLARATION ASSEMBLE/////////////////////////////////////////////
                    else if(list.get(listctr).Token=="assemble"){
                  while(list.get(listctr).Token=="assemble"){
                        iden="";
                            iden = iden.concat("<structdec>");
                            listSyntax.add(new Syntax("<gdeclr> ",iden));
                            iden="";
                            iden = iden.concat("assemble");
                            listSyntax.add(new Syntax("<structdec> ",iden));
                   listctr++;
                    if(list.get(listctr).Token=="Identifier"){
                                iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<structdec> ",iden));
                                listctr++;
                                if(list.get(listctr).Token.equals("{")){
                                  
                                  iden="";
                            iden = iden.concat("{");
                            listSyntax.add(new Syntax("<structdec> ",iden));
                                listctr++;
                                              if(list.get(listctr).Token.equals("}")){
                                                    listctr++;
                                                    flag=1;
                                                     }
                                                      ////////////////////////////////VARIABLE DECLARATION///////////////////////////////////////////////////////
                    else if(list.get(listctr).Token=="int"||list.get(listctr).Token=="flt"||list.get(listctr).Token=="chr"||list.get(listctr).Token=="bool"||list.get(listctr).Token=="str"){
                    while(list.get(listctr).Token=="int"||list.get(listctr).Token=="flt"||list.get(listctr).Token=="chr"||list.get(listctr).Token=="bool"||list.get(listctr).Token=="str"){
                      dtype =list.get(listctr).Token;
                        listctr++;
                            if(list.get(listctr).Token=="Identifier"){
                                listctr++;
                                
                                if(list.get(listctr).Token.equals("=")){
                                    listctr++;
                                    
                                    if(list.get(listctr).Token=="true"||list.get(listctr).Token=="false"||list.get(listctr).Token=="Int Literal"||list.get(listctr).Token=="Float Literal"||list.get(listctr).Token.equals("(")||list.get(listctr).Token.equals("Character Literal")||list.get(listctr).Token.equals("String Literal")){
                                        if(dtype=="int"&&list.get(listctr).Token=="Int Literal"){
                                            listctr++;
                                        }
                                         else if(dtype=="flt"&&list.get(listctr).Token=="Float Literal"){
                                            listctr++;
                                        }
                                         else if(dtype=="bool"&&list.get(listctr).Token=="true"||
                                                 dtype=="bool"&&list.get(listctr).Token=="false"){
                                            listctr++;
                                        }
                                    else if(dtype=="str"&&list.get(listctr).Token=="String Literal"){
                                            listctr++;
                                        }
                                     else if(dtype=="chr"&&list.get(listctr).Token=="Character Literal"){
                                            listctr++;
                                        }
                                        else if(dtype=="int"&&list.get(listctr).Token.equals("(")){
                                            listctr++;
                                            if(list.get(listctr).Token=="Negative Int Literal"){
                                                listctr= listctr+2;
                                            }
                                      
                                            else{
                                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Negative Integer Literal. "
); 
                                             flag=1;
                                       break; 
                                            }//else not negative int lit
                                        }
                                        else{
                                            System.out.print("JJEEKEE");
                                         ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '"+dtype+" literal'. ");  
                                       flag=1;
                                       listctr++;
                                         break;
                                        }
                                        
                     /////////////////////////////////SEPARATOR LOOP FOR a=number///////////////////////////////////////////////////////////////////   
                    // if(flag==0){
                     if(list.get(listctr).Token.equals(";")||list.get(listctr).Token.equals(" ;")){
                                            listctr++;
                                            
                                            //break;
                                        }
                                    else if(list.get(listctr).Token.equals(",")){
                                    arrayFlag=0;
                              int result2[] = comma(listctr, iden, flag, fontfamily, arrayFlag);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                                 }//else if ,
                                         else{
                                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ',' or ';'. ");  
                                            flag=1;
                                       break; 
                                         }
                                   
                                    }//if LITERAL
                                    else{
                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '"+dtype+" literal'. ");
                                        flag=1;
                                    break;
                                    }//else not LITERAL
                                }//if =
                            /////////////////////////////////END OF SEPARATOR LOOP FOR a=number///////////////////////////////////////////////////////////////////  
                                 else if(list.get(listctr).Token.equals(",")){
                                     arrayFlag=0;
                              int result2[] = comma(listctr, iden, flag, fontfamily, arrayFlag);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                                 }//else if ,
                                else if(list.get(listctr).Token.equals(";")){
                                    listctr++;
                                    System.out.print("LOLOL");
                                  //  break;
                                }
                                ////////////////////////////////////////ARRAY DECLARATION//////////////////////////////////////////////////////////////////
                                else if(list.get(listctr).Token.equals("[")){
                                    arrayFlag=1;
                             int result1[] = arrayAccess(listctr, iden, flag, fontfamily, arrayFlag,dtype);
                                                         flag=result1[0];
                                                         listctr=result1[1];
                        
                           arrayFlag=0;
                        
                                    
                                }

//////////////////////////////END OF ARRAY DECLARATION//////////////////////////////////////////////////////////////////////  
                                         else{
                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '=', ',', '[', or ';'. ");
                                        flag=1;
                                        System.out.print("\nSHALLOW\n");
                                break;
                                }//else not = 
                                
                            }//if Identifier
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier. ");   
                             flag=1;
                            break;
                            }//else not Identifier
                          }//while VAR DEC
                        if(flag==0){
                           if(list.get(listctr).Token.equals("}")||list.get(listctr).Token.equals(" }")){
                            iden="";
                            iden = iden.concat("}");
                            listSyntax.add(new Syntax("<structdec> ",iden));
                               listctr++;
                      
                             if(list.get(listctr).Token.equals(";")){
                                 iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<structdec> ",iden));
                             listctr++;
                             
                             }
                             else if(list.get(listctr).Token.equals("Identifier")){
                                      iden="";
                            iden = iden.concat("<objvar>");
                            listSyntax.add(new Syntax("<structdec> ",iden));
                             listctr++;
                                if(list.get(listctr).Token.equals(";")){
                                         iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<structdec> ",iden));
                                listctr++;
                                }
                                else{
                            ErrorTxt.setText(
"<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';'. ");
                            flag=1;
                          break;
                          }//else
                             }
                             else{
                            ErrorTxt.setText(
"<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';' or Identifier. ");
                            flag=1;
                          break;
                          }//else
                             }
                          
                          else{
                                      
                            ErrorTxt.setText(
"<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Datatype or '}'. ");
                            flag=1;
                          break;
                          }//else
                            } 
                 }//if VAR DEC
                                    else if(list.get(listctr).Token.equals("}")){
                                      iden="";
                            iden = iden.concat("}");
                            listSyntax.add(new Syntax("<structdec> ",iden));
                             listctr++;
                             if(list.get(listctr).Token.equals(";")){
                                        iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<structdec> ",iden));
                             listctr++;
                             
                             }
                             else if(list.get(listctr).Token.equals("Identifier")){
                                        iden="";
                            iden = iden.concat("<objvar");
                            listSyntax.add(new Syntax("<structdec> ",iden));
                             listctr++;
                                if(list.get(listctr).Token.equals(";")){
                                        iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<structdec> ",iden));
                                listctr++;
                                }
                                else{
                            ErrorTxt.setText(
"<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';'. ");
                            flag=1;
                          break;
                          }//else
                             }
                             else{
                            ErrorTxt.setText(
"<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';' or Identifier. ");
                            flag=1;
                          break;
                          }//else
                             }
                                                      else{
                                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '}' or 'int', 'flt', 'chr', 'str', 'bool'. ");
                                                        flag=1;
                                                        break;
                                                    }
                      
                                }
                                else{
                                   ErrorTxt.setText(
"<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '{'. ");
                                   flag=1;
                                      break; 
                                }
                   
                    }
                    else{
                       ErrorTxt.setText(
"<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier. ");
                       flag=1;
                          break; 
                    }
                   
                   }
                    if(flag==1)
                        break;
                        }//if struct dec
                    
                  ////////////////////////////////////END OF STRUCT DEC///////////////////////////////////////////////////
                 ////////////////////////////////1 VARIABLE DECLARATION///////////////////////////////////////////////////////
                    else if(list.get(listctr).Token=="int"||list.get(listctr).Token=="flt"||list.get(listctr).Token=="chr"||list.get(listctr).Token=="bool"||list.get(listctr).Token=="str"||list.get(listctr).Token=="onduty"){
                    while(list.get(listctr).Token=="int"||list.get(listctr).Token=="flt"||list.get(listctr).Token=="chr"||list.get(listctr).Token=="bool"||list.get(listctr).Token=="str"||list.get(listctr).Token=="onduty"){
                      dtype =list.get(listctr).Token;
                        listctr++;
                            
                            if(list.get(listctr).Token=="Identifier"){
                                listctr++;
                                
                                if(list.get(listctr).Token.equals("=")){
                                           iden="";
                            iden = iden.concat("<vardec>");
                            listSyntax.add(new Syntax("<gdeclr> ",iden));
                            
                                   iden="";
                            iden = iden.concat("<vdtype>");
                            listSyntax.add(new Syntax("<vardec> ",iden));
                            
                                    listctr++;
                                    if(list.get(listctr).Token=="true"||list.get(listctr).Token=="false"||list.get(listctr).Token=="Int Literal"||list.get(listctr).Token=="Float Literal"||list.get(listctr).Token.equals("(")||list.get(listctr).Token.equals("Character Literal")||list.get(listctr).Token.equals("String Literal")){
                                        if(dtype=="int"&&list.get(listctr).Token=="Int Literal"){
                                            iden="";
                            iden = iden.concat("int id");
                            listSyntax.add(new Syntax("<vdtype> ",iden));
                                                 iden="";
                            iden = iden.concat("<init1>");
                            listSyntax.add(new Syntax("<vdtype> ",iden));
                                    iden="";
                            iden = iden.concat("=");
                            listSyntax.add(new Syntax("<init1> ",iden));
                            iden="";
                            iden = iden.concat("<intlit1>");
                            listSyntax.add(new Syntax("<init1> ",iden));
                                                iden="";
                            iden = iden.concat("integer literal");
                            listSyntax.add(new Syntax("<intlit1> ",iden));
                            
                                            listctr++;
                                        }
                                         else if(dtype=="flt"&&list.get(listctr).Token=="Float Literal"){
                                             iden="";
                                             iden = iden.concat("flt id");
                            listSyntax.add(new Syntax("<vdtype> ",iden));
                                                 iden="";
                            iden = iden.concat("<init2>");
                            listSyntax.add(new Syntax("<vdtype> ",iden));
                                    iden="";
                            iden = iden.concat("=");
                            listSyntax.add(new Syntax("<init2> ",iden));
                            iden="";
                            iden = iden.concat("<fltlit>");
                            listSyntax.add(new Syntax("<init2> ",iden));
                                                iden="";
                            iden = iden.concat("integer literal");
                            listSyntax.add(new Syntax("<fltlit> ",iden));
                                            listctr++;
                                        }
                                         else if(dtype=="bool"&&list.get(listctr).Token=="true"||
                                                 dtype=="bool"&&list.get(listctr).Token=="false"){
                                                 iden="";
                                             iden = iden.concat("bool id");
                            listSyntax.add(new Syntax("<vdtype> ",iden));
                                                 iden="";
                            iden = iden.concat("<init5>");
                            listSyntax.add(new Syntax("<vdtype> ",iden));
                                    iden="";
                            iden = iden.concat("=");
                            listSyntax.add(new Syntax("<init5> ",iden));
                            iden="";
                            iden = iden.concat("<boollit>");
                            listSyntax.add(new Syntax("<init5> ",iden));
                                                iden="";
                            iden = iden.concat("boolean literal");
                            listSyntax.add(new Syntax("<boollit> ",iden));
                                            listctr++;
                                        }
                                     else if(dtype=="str"&&list.get(listctr).Token=="String Literal"){
                                                    iden="";
                           iden = iden.concat("<init4>");
                            listSyntax.add(new Syntax("<cdtype> ",iden));
                                    iden="";
                            
                                            iden = iden.concat("=");
                            listSyntax.add(new Syntax("<init4> ",iden));
                                                 iden="";
                            iden = iden.concat("<strlit>");
                            listSyntax.add(new Syntax("<init4> ",iden));
                                            iden="";
                            iden = iden.concat("String Literal");
                            listSyntax.add(new Syntax("<strlit> ",iden));
                                            listctr++;
                                        }
                                      else if(dtype=="chr"&&list.get(listctr).Token=="Character Literal"){
                                                    iden="";
                            iden = iden.concat("<init5>");
                            listSyntax.add(new Syntax("<cdtype> ",iden));
                                    iden="";
                            iden = iden.concat("=");
                            listSyntax.add(new Syntax("<init5> ",iden));
                                                iden="";
                            iden = iden.concat("Character Literal");
                            listSyntax.add(new Syntax("<chrlit> ",iden));
                                            listctr++;
                                        }
                                        else if(dtype=="int"&&list.get(listctr).Token.equals("(")){
                                            listctr++;
                                            if(list.get(listctr).Token=="Negative Int Literal"){
                                                listctr= listctr+2;
                                            }
                                      
                                            else{
                                             ErrorTxt.setText(
"<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting 'Negative Integer Literal'. ");  
                                             flag=1;
                                       break; 
                                            }//else not negative int lit
                                        }
                                        else{
                                         ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '"+dtype+" literal'. "); 
                                         flag=1;
                                       break;
                                        }
                     /////////////////////////////////SEPARATOR LOOP FOR a=number///////////////////////////////////////////////////////////////////   
                                         if(list.get(listctr).Token.equals(";")||list.get(listctr).Token.equals(" ;")){
                                                 iden="";
                                             iden = iden.concat(";");
                            listSyntax.add(new Syntax("<vardec> ",iden));
                                      
                                            listctr++;
                                            break;
                                        }
                                    else if(list.get(listctr).Token.equals(",")){
                                        
                                    while(list.get(listctr).Token.equals(",")){
                                        
                                    listctr++;
                                    if(list.get(listctr).Token.equals("Identifier")){
                                        listctr++;
                                        if(list.get(listctr).Token.equals("=")){
                                    listctr++;
                                    if(list.get(listctr).Token=="true"||list.get(listctr).Token=="false"||list.get(listctr).Token=="Int Literal"||list.get(listctr).Token=="Float Literal"||list.get(listctr).Token.equals("Character Literal")||list.get(listctr).Token.equals("String Literal")){
                                        if(dtype.equals("int")&&list.get(listctr).Token.equals("Int Literal")){
                                            listctr++;
                                        }//if int lit
                                        else if(dtype.equals("flt")&&list.get(listctr).Token.equals("Float Literal")){
                                            listctr++;
                                        }//if int lit
                                       else if(dtype.equals("chr")&&list.get(listctr).Token.equals("Character Literal")){
                                            listctr++;
                                        }//if int lit
                                       else if(dtype.equals("str")&&list.get(listctr).Token.equals("String Literal")){
                                            listctr++;
                                        }//if int lit
                                       else if((dtype.equals("bool")&&list.get(listctr).Token.equals("true"))||(dtype.equals("bool")&&list.get(listctr).Token.equals("false"))){
                                            listctr++;
                                        }//if int lit
                                    else{
                                          ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting "+dtype+" literal. ");
                                          flag=1;
                                break;
                                    }//else
                                    }//if ALL literal
                                    else{
                                          ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '"+dtype+" literal'. ");
                                          flag=1;
                                break;
                                    }//else
                                    
                                        }
                                        if(list.get(listctr).Token.equals(";")||list.get(listctr).Token.equals(" ;")){
                                          iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<vardec> ",iden));
                                            listctr++;
                                       
                                         break;
                                        }
                                        else if(list.get(listctr).Token.equals(",")){
                                        continue;
                                        }
                                        else{
                                            if(list.get(listctr-1).Token.equals("Identifier")){
                                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'.  Expecting '=', ',' or ';'. ");  }
                                            else{
                                                   ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'.  Expecting ',' or ';'. "); 
                                            }
                                            flag=1;
                                            
                                           
                                       break; 
                                        }
                                        
                                    }//if
                                    
                                    else{
                                          ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier. ");
                                          flag=1;
                                break;
                                    }//else
                                     //System.out.print("comma na"+list.get(listctr).Token);
                                }//while ,
                                 }//else if ,
                                         else{
                                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'.  Expecting ',' or ';'. ");  //ss
                                            flag=1;
                                       break; 
                                         }
                                    }//if LITERAL
                                    else{
                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '"+dtype+" literal'. ");
                                        flag=1;
                                    break;
                                    }//else not LITERAL
                                }//if =
                            /////////////////////////////////END OF SEPARATOR LOOP FOR a=number///////////////////////////////////////////////////////////////////  
                                 else if(list.get(listctr).Token.equals(",")){
                                     arrayFlag=0;
                                    int result2[] = comma(listctr, iden, flag, fontfamily, arrayFlag);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                                 }//else if ,
                                else if(list.get(listctr).Token.equals(";")){
                                    iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<vardec> ",iden));
                                    listctr++;
                                    break;
                                }
////////////////////////////////////////ARRAY DECLARATION//////////////////////////////////////////////////////////////////
                                else if(list.get(listctr).Token.equals("[")){
                                    arrayFlag=1;
                             int result1[] = arrayAccess(listctr, iden, flag, fontfamily, arrayFlag,dtype);
                                                         flag=result1[0];
                                                         listctr=result1[1];
                                  //  System.out.print("Array Dec"+list.get(listctr-9).Token);
                                 //  System.out.print("Array Dec"+list.get(listctr-6).Token);
                           arrayFlag=0;
                           
                                    
                                }

//////////////////////////////END OF ARRAY DECLARATION//////////////////////////////////////////////////////////////////////                                
                                ////////////////////////FUNCTION DECLARATION///////////////////////////////////////////
                                        else if(list.get(listctr).Token.equals("(")&&list.get(listctr-2).Token=="int"||list.get(listctr).Token.equals("(")&&list.get(listctr-2).Token=="flt"||list.get(listctr).Token.equals("(")&&list.get(listctr-2).Token=="onduty"){
                                        listctr++;
                                       
                                              if(list.get(listctr).Token=="int"||list.get(listctr).Token=="flt"||list.get(listctr).Token=="chr"||list.get(listctr).Token=="bool"||list.get(listctr).Token=="str"){
                                            while(list.get(listctr).Token=="int"||list.get(listctr).Token=="flt"||list.get(listctr).Token=="chr"||list.get(listctr).Token=="bool"||list.get(listctr).Token=="str"){
                                              dtype =list.get(listctr).Token;
                                                listctr++;
                                             if(list.get(listctr).Token=="Identifier"){
                                                 listctr++;
                                                 if(list.get(listctr).Token.equals(")")){
                                                     iden="";
                            iden = iden.concat(")");
                            listSyntax.add(new Syntax("<funcdec> ",iden));
                                                 listctr++;
                                                 if(list.get(listctr).Token.equals(";")){
                                                     iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<funcdec> ",iden));
                                                 listctr++;
                                                 break;}
                                                  ////FUNCTION DEFINTION///
                                
                                      else if(list.get(listctr).Token==("startmission")){
                                        listctr++;
                                        System.out.print("Printmona:"+list.get(listctr).Token+"\n");
                                        if(list.get(listctr).Token.equals(":")){
                                            System.out.print("Print:"+list.get(listctr).Token);
                                            listctr--;
                                    int result2[] = command(listctr, iden, flag, fontfamily);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                                      listctr=listctr-2;
                                       System.out.print("Print~~~~~:"+flag+"~~~~");
                                           if(flag==1){
                                               break;}
                                           else if(list.get(listctr).Token.equals("endmission")){
                                                listctr++;
                                                 System.out.print("\n mf");
                                                flag=1;
                                                //continue;
                                               }//if endmission
                                      /*continue*/
                                        }
                                        else{
                                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ':'. ");
                                                 flag=1;
                                                  break; 
                                        }
                                     }
                                //FUNCTION DEFINITION////
                                    
                                                   else{
                                                  ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';', or 'startmission:'. ");
                                                 flag=1;
                                                  break; 
                                             }//else not ;
                                                 
                                             }
                                             else if(list.get(listctr).Token.equals(",")){
                                                 listctr++;
                                                 //System.out.print("Comma");
                                                 if(list.get(listctr).Token.equals("int")||list.get(listctr).Token.equals("flt")||
                                                    list.get(listctr).Token.equals("chr")||list.get(listctr).Token.equals("str")
                                                         ||list.get(listctr).Token.equals("bool")){
                                                     System.out.print("Cont");
                                                 continue;
                                                 
                                                 }
                                                 else{
                                                      ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting 'int', 'flt', 'bool', 'chr', or 'str'. ");
                                                flag=1;
                                                      break; 
                                                 }
                                                 
                                                 
                                             }
                                             else{
                                                  ErrorTxt.setText(
"<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ',', ';'.  ");
                                                  
                                                flag=1;
                                                  break; 
                                             }//else not id
                                             }
                                             else{
                                                  ErrorTxt.setText(
"<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier. ");
                                                flag=1;
                                                  break; 
                                             }//else not id
                                            }
                                              }
                                              else if(list.get(listctr).Token.equals(")")){
                                                     iden="";
                            iden = iden.concat(")");
                            listSyntax.add(new Syntax("<funcdec> ",iden));
                                                 listctr++;
                                                 if(list.get(listctr).Token.equals(";")){
                                                     iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<funcdec> ",iden));
                                                 listctr++;
                                                 break;}
                                                                 ////FUNCTION DEFINTION///
                                
                                      else if(list.get(listctr).Token==("startmission")){
                                        listctr++;
                                        System.out.print("Printmona:"+list.get(listctr).Token+"\n");
                                        if(list.get(listctr).Token.equals(":")){
                                            System.out.print("Print:"+list.get(listctr).Token);
                                            listctr--;
                                    int result2[] = command(listctr, iden, flag, fontfamily);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                                      listctr=listctr-2;
                                       System.out.print("Print~~~~~:"+flag+"~~~~");
                                           if(flag==1){
                                               break;}
                                           else if(list.get(listctr).Token.equals("endmission")){
                                                listctr++;
                                                 System.out.print("\n mf");
                                                flag=1;
                                                //continue;
                                               }//if endmission
                                      /*continue*/
                                        }
                                        else{
                                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ':'. ");
                                                 flag=1;
                                                  break; 
                                        }
                                     }
                                //FUNCTION DEFINITION////
                                                   else{
                                                  ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';' or startmission:. ");
                                                 flag=1;
                                                  break; 
                                             }//else not ;
                                                 
                                             }
                                              else{
                                                  ErrorTxt.setText(
"<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting 'int', 'flt', 'bool', 'chr', or 'str' or ')'. ");
                                                flag=1;
                                                  break; 
                                              }
                                        }
                 /////////////////////////////////////////END OF FUNCTION DECLARATION//////////////////////////////////
                                else{
                                        ErrorTxt.setText(
"<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '=', ',' or ';'. ");
                             flag=1;
                                        break;
                                }//else not = 
                                
                            }//if Identifier
                            else{
                             ErrorTxt.setText(
"<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier. ");   
                           flag=1;
                             break;
                            }//else not Identifier
                          }//while VAR DEC
                 }//if VAR DEC
                       /**   else{
                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on <span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>,<span style=\"color:red\"> Line "+list.get(listctr).Line+"</span>. Datatype not found. ");
                          break;
                          }//else
                          * */
                 else
                        listctr++;
                 //////////////////////////////////END OF VARIABLE DECLARATION/////////////////////////////////////////////
                  }//WHILE GLOBAL DECLARATION
           //IF NO ERROR in variable declaration
           if(flag==0){
                  //////BODY///////////////////////////////////////////////////////////////////////
                  
         
           if(list.get(listctr).Token==("command")){
                     command(listctr, iden, flag, fontfamily);
                 }
           else if(list.get(listctr).Token==("dismiss")&&(list.size()-1)==listctr){
           System.out.print("");
           }
           
       
           else{
               if(flag!=1){
                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting 'dismiss', 'int' 'flt' or 'onduty', 'const', 'assemble', 'command()'. "); //yyyy
                        flag=1;
                             break;}
                     }
           } //if fllag==0
              }//if global declaration
             ////////////////////////////////////////END OF GLOBAL DECLARATION////////////////
             //////BODY///////////////////////////////////////////////////////////////////////
            else if(list.get(listctr).Token==("command")){
           command(listctr, iden, flag, fontfamily);
                 }
           else if(list.get(listctr).Token==("dismiss")&&(list.size()-1)==listctr){
           System.out.print("");
           }
           else{
                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting 'dismiss', 'int' 'flt' or 'onduty','const', 'assemble', 'command()'." );
                             break;
                     }
        
        
        break;
        }//while loop
}//if LexError=0
       return listSyntax; 
        
    }
    
    //=====================================END OF SYNTAX ANALYZER
    
    public int[] command(int listctr, String iden, int flag, String fontfamily){
        try {
            listctr++;
            ArrayList<Lexical> list = ListLexical();
            ArrayList<Syntax> listSyntax = new ArrayList<Syntax>();
            int operandCtr=0;
           
             System.out.print("command"+list.get(listctr).Token);
                   if(list.get(listctr).Token.equals("(")||list.get(listctr).Token.equals(":")){
                       if(list.get(listctr).Token.equals("("))                 
                       listctr++;
                       else
                           System.out.print("\n hmm");
                          if(list.get(listctr).Token.equals(")")||list.get(listctr).Token.equals(":")){
                                        if(list.get(listctr).Token.equals(")"))                 
                       listctr++;
                       else
                           System.out.print("\n zzz");
                                        
                     if(list.get(listctr).Token.equals("startmission")||list.get(listctr).Token.equals(":")){
                      if(list.get(listctr).Token.equals("startmission"))                 
                       listctr++;
                       else
                           System.out.print("\n ff");
                                      if(list.get(listctr).Token.equals(":")){
                                        listctr++;
                                        
                                       System.out.print("\n hh"+list.get(listctr).Token+"\n");
                           if(list.get(listctr).Token==("command")||list.get(listctr).Token==("endmission")||list.get(listctr).Token==("fallin")||list.get(listctr).Token==("Identifier")
                              ||list.get(listctr).Token==("if")||list.get(listctr).Token==("for")||list.get(listctr).Token==("do")||list.get(listctr).Token==("++")||list.get(listctr).Token==("report")
                               ||list.get(listctr).Token==("execute")||list.get(listctr).Token.equals("Int Literal")||list.get(listctr).Token.equals("Float Literal")){
                                if(list.get(listctr).Token.equals("endmission")){
                                                listctr++;
                                                // System.out.print("\n mf");
                                                flag=1;
                                               }//if endmission
                           while(list.get(listctr).Token!="endmission")
                           {//WHILE LOOP COMMAND INDIE 
                                        
                                                if(list.get(listctr).Token.equals("endmission")){
                                                listctr++;
                                                 System.out.print("\n mf");
                                                flag=1;
                                               }//if endmission
                                                   ////////////////////////STATEMENTS2 OR STATEMENT IN BODY//////////////////////////////////////////////////////////////////////////////////////
                                               else if(list.get(listctr).Token.equals("fallin")){
                                                listctr++;
                                                    if(list.get(listctr).Token.equals("{")){
                                                    listctr++;
                                                    
                                                     if(list.get(listctr).Token.equals("}")){
                                                    listctr++;
                                                    flag=1;
                                                     }
                                                      ////////////////////////////////VARIABLE DECLARATION///////////////////////////////////////////////////////
                    else if(list.get(listctr).Token=="int"||list.get(listctr).Token=="flt"||list.get(listctr).Token=="chr"||list.get(listctr).Token=="bool"||list.get(listctr).Token=="str"){
                    while(list.get(listctr).Token=="int"||list.get(listctr).Token=="flt"||list.get(listctr).Token=="chr"||list.get(listctr).Token=="bool"||list.get(listctr).Token=="str"){
                      dtype =list.get(listctr).Token;
                        listctr++;
                            if(list.get(listctr).Token=="Identifier"){
                                listctr++;
                                
                                if(list.get(listctr).Token.equals("=")){
                                    listctr++;
                                    
                                    if(list.get(listctr).Token=="true"||list.get(listctr).Token=="false"||list.get(listctr).Token=="Int Literal"||list.get(listctr).Token=="Float Literal"||list.get(listctr).Token.equals("(")||list.get(listctr).Token.equals("Character Literal")||list.get(listctr).Token.equals("String Literal")){
                                        if(dtype=="int"&&list.get(listctr).Token=="Int Literal"){
                                            listctr++;
                                        }
                                         else if(dtype=="flt"&&list.get(listctr).Token=="Float Literal"){
                                            listctr++;
                                        }
                                         else if(dtype=="bool"&&list.get(listctr).Token=="true"||
                                                 dtype=="bool"&&list.get(listctr).Token=="false"){
                                            listctr++;
                                        }
                                    else if(dtype=="str"&&list.get(listctr).Token=="String Literal"){
                                            listctr++;
                                        }
                                     else if(dtype=="chr"&&list.get(listctr).Token=="Character Literal"){
                                            listctr++;
                                        }
                                        else if(dtype=="int"&&list.get(listctr).Token.equals("(")){
                                            listctr++;
                                            if(list.get(listctr).Token=="Negative Int Literal"){
                                                listctr= listctr+2;
                                            }
                                      
                                            else{
                                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Negative Integer Literal. "
); 
                                             flag=1;
                                       break; 
                                            }//else not negative int lit
                                        }
                                        else{
                                            System.out.print("JJEEKEE");
                                         ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '"+dtype+" literal'. ");  
                                       flag=1;
                                       listctr++;
                                         break;
                                        }
                                        
                     /////////////////////////////////SEPARATOR LOOP FOR a=number///////////////////////////////////////////////////////////////////   
                    // if(flag==0){
                     if(list.get(listctr).Token.equals(";")||list.get(listctr).Token.equals(" ;")){
                                            listctr++;
                                            
                                            //break;
                                        }
                                    else if(list.get(listctr).Token.equals(",")){
                                    arrayFlag=0;
                              int result2[] = comma(listctr, iden, flag, fontfamily, arrayFlag);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                                 }//else if ,
                                         else{
                                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '=', ',' or ';'.");  
                                            flag=1;
                                       break; 
                                         }
                                   
                                    }//if LITERAL
                                    else{
                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '"+dtype+" literal'. ");
                                        flag=1;
                                    break;
                                    }//else not LITERAL
                                }//if =
                            /////////////////////////////////END OF SEPARATOR LOOP FOR a=number///////////////////////////////////////////////////////////////////  
                                 else if(list.get(listctr).Token.equals(",")){
                                     arrayFlag=0;
                              int result2[] = comma(listctr, iden, flag, fontfamily, arrayFlag);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                                 }//else if ,
                                else if(list.get(listctr).Token.equals(";")){
                                    listctr++;
                                    System.out.print("LOLOL");
                                  //  break;
                                }
                                ////////////////////////////////////////ARRAY DECLARATION//////////////////////////////////////////////////////////////////
                                else if(list.get(listctr).Token.equals("[")){
                                    arrayFlag=1;
                             int result1[] = arrayAccess(listctr, iden, flag, fontfamily, arrayFlag,dtype);
                                                         flag=result1[0];
                                                         listctr=result1[1];
                        
                           arrayFlag=0;
                        
                                    
                                }

//////////////////////////////END OF ARRAY DECLARATION//////////////////////////////////////////////////////////////////////  
                                         else{
                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '=', ',', '[', or ';'. ");
                                        flag=1;
                                break;
                                }//else not = 
                                
                            }//if Identifier
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier. ");   
                             flag=1;
                            break;
                            }//else not Identifier
                          }//while VAR DEC
                    if(flag==0){
                    if(list.get(listctr).Token.equals("}")||list.get(listctr).Token.equals(" }")){
                                                listctr++;
                                                //flag=1;
                                               }//if }
                    else{
                                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '}' or 'int', 'flt', 'chr', 'str', 'bool'. ");
                                                        flag=1;
                                                        break;
                                                    }
                     } //IF FLAG==0 FOR SEPARATOR
                 }//if VAR DEC
                                                      else{
                                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '}' or 'int', 'flt', 'chr', 'str', 'bool'. ");
                                                        flag=1;
                                                        break;
                                                    }
                                                   //  flag=0;
                                                   }//if }
                                                    else{
                                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '{'. ");
                                                        flag=1;
                                                        break;
                                                    }
                                                     if(flag==0){ if(list.get(listctr).Token.equals("endmission")){
                                                System.out.print("here"); break;
                                               }//if endmission
                                                     else if(list.get(listctr).Token==("command")||list.get(listctr).Token==("fallin")||list.get(listctr).Token==("Identifier")
                              ||list.get(listctr).Token==("if")||list.get(listctr).Token==("for")||list.get(listctr).Token==("do")||list.get(listctr).Token==("++")||list.get(listctr).Token==("report")
                               ||list.get(listctr).Token==("execute")||list.get(listctr).Token.equals("Int Literal")||list.get(listctr).Token.equals("Float Literal")){
                                                     continue;
                                                     }
                                                   else{
                                                         if(flag!=1){
                                                 ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting 'fallin', 'report', 'execute', 'Identifier', 'if', 'for', 'do', '++', '--', 'endmission'. ");
                                                 flag=1;
                                                        break;}
                                                }//else NOT endmission >>startmission
                                                     }//if flag==0 startmission
                                               }//if fallin
                                                          /////////////////////////////////STATEMENTS2//////////////////
                                                    else if(list.get(listctr).Token.equals("execute")){
                                                        iden="";
                                                   iden = iden.concat("<statements2>");
                            listSyntax.add(new Syntax("<statements4> ",iden));
                            
                              iden="";
                                                   iden = iden.concat("<input>");
                            listSyntax.add(new Syntax("<statements2> ",iden));
                              iden="";
                                                   iden = iden.concat("execute");
                            listSyntax.add(new Syntax("<input> ",iden));
                                                
                                                   listctr++;
                                                   
                                                   if(list.get(listctr).Token.equals("Identifier")){
                                                         iden="";
                                                   iden = iden.concat("id");
                            listSyntax.add(new Syntax("<input> ",iden));
                                                       listctr++;
                                                   }
                                                   else{
                                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier. ");
                                                        flag=1;
                                                        break;
                                                   }
                                                       if(list.get(listctr).Token.equals(";")){
                                                             iden="";
                                                   iden = iden.concat(";");
                            listSyntax.add(new Syntax("<statements2> ",iden));
                                                       listctr++;
                                                       System.out.print("HMMM");
                                                       //flag=1;
                                                   }
                                                   else{
                                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';'. ");
                                                        flag=1;
                                                        break;
                                                   }
                                               }
                                 else if(list.get(listctr).Token.equals("report")){
                                                                      iden="";
                                                   iden = iden.concat("<statements2");
                            listSyntax.add(new Syntax("<statements4> ",iden));
                              iden="";
                                                   iden = iden.concat("<output>");
                            listSyntax.add(new Syntax("<statements2> ",iden));
                              iden="";
                          iden = iden.concat("report");
                            listSyntax.add(new Syntax("<output> ",iden));
                                                   listctr++;
                             if(list.get(listctr).Token.equals("Identifier")){
                                                       
                              iden="";
                                                   iden = iden.concat("<display>");
                            listSyntax.add(new Syntax("<output> ",iden));
                              iden="";
                                                   iden = iden.concat("id");
                            listSyntax.add(new Syntax("<display> ",iden));
                                                       listctr++;
                                                                          if(list.get(listctr).Token.equals("+")){
                                                            
                                                       while((list.get(listctr).Token.equals("+"))){
                                                         //  System.out.print("unit test");
                                                            listctr++;
                                                         if(list.get(listctr).Token.equals("String Literal")||list.get(listctr).Token.equals("Identifier")){
                                                         listctr++;
                                                         
                                                         }
                                                         else{
                                                                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, or String Literal. ");
                                                        flag=1;
                                                        break;  
                                                        
                                                        }
                                                       }
                                                   }
                                                        if(flag==1)
                                                            break;
                                                   }
                             else if(list.get(listctr).Token.equals("String Literal")){
                                                       
                              iden="";
                                                   iden = iden.concat("<display>");
                            listSyntax.add(new Syntax("<output> ",iden));
                              iden="";
                                                   iden = iden.concat("strlit");
                            listSyntax.add(new Syntax("<display> ",iden));
                                                       listctr++;
                                                       
                                                        if(list.get(listctr).Token.equals("+")){
                                                            
                                                       while((list.get(listctr).Token.equals("+"))){
                                                         //  System.out.print("unit test");
                                                            listctr++;
                                                         if(list.get(listctr).Token.equals("String Literal")||list.get(listctr).Token.equals("Identifier")){
                                                         listctr++;
                                                         
                                                         }
                                                         else{
                                                                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, or String Literal. ");
                                                        flag=1;
                                                        break;  
                                                        
                                                        }
                                                       }
                                                   }
                                                        if(flag==1)
                                                            break;
                                                        
                             }
                                                   else{
                                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, or String Literal. ");
                                                        flag=1;
                                                        break;
                                                   }
                                                       if(list.get(listctr).Token.equals(";")&&flag!=1){
                                                            iden="";
                                                   iden = iden.concat("\"");
                                                   
                            listSyntax.add(new Syntax("<statements2> ",iden));
                                                       listctr++;
                                                       //flag=1;
                                                       
                                                   }

                                                   else{
                                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';' or '+'. ");
                                                        flag=1;
                                                        break;
                                                   }
                                               }
                             
       ////////////////////////////IF CONDITION INDIE/////////////////
                     else if(list.get(listctr).Token.equals("if")){
                         int stringFlag=0;
                         System.out.print("IFFF\n");
                             iden="";
                           iden = iden.concat("<statements2>");
                            listSyntax.add(new Syntax("<statements4> ",iden));
                              iden="";
                            iden = iden.concat("<statements2>");
                            listSyntax.add(new Syntax("<statements4> ",iden));
                              iden="";
                            iden = iden.concat("<conditional1>");
                            listSyntax.add(new Syntax("<statements2> ",iden));
                              iden="";
                              iden = iden.concat("if");
                            listSyntax.add(new Syntax("<conditional1> ",iden));
                                                   listctr++;
                                                 if(list.get(listctr).Token.equals("(")){
                                                       iden="";
                                                    iden = iden.concat("(");
                                                  listSyntax.add(new Syntax("<conditional1> ",iden));
                                                     listctr++;
                                                     
                                                  //CONDITON OPERAND///
                                                  if(list.get(listctr).Token.equals("String Literal")||list.get(listctr).Token.equals("Character Literal")){
                                                     stringFlag=1;
                                                      listctr++;
                                                      if(list.get(listctr).Token.equals("==")||list.get(listctr).Token.equals("!=")){
                                                          listctr++;
                                                          if(list.get(listctr).Token.equals("String Literal")||list.get(listctr).Token.equals("Character Literal")){
                                                      //listctr++;
                                                      System.out.print("");
                                                          }
                                                              else{
                                                          if(flag!=1){
                                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting String Literal or Character Literal. ");
                                                        flag=1;
                                                        break;
                                                          }
                                                      }
                                                      }
                                                      else{
                                                          if(flag!=1){
                                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '==' or '!='. ");
                                                        flag=1;
                                                        break;
                                                          }
                                                      }
                                                          
                                                  }
                                                  
                                                  
                                                  else if(list.get(listctr).Token.equals("!")){
                                                      stringFlag=1;
                                                   while(list.get(listctr).Token.equals("!")) {
                                                       listctr++;
                                                   } 
                                                   if(list.get(listctr).Token.equals("Identifier"))
                                                        System.out.print("");
                                                   else if(list.get(listctr).Token.equals("true")||list.get(listctr).Token.equals("false"))
                                                       System.out.print("");
                                                   else if(list.get(listctr).Token.equals("(")){
                                                       System.out.print("\n======HERE===\n");
                                                   listctr++;
                                                    operandCtr=1;
                                                    int result4[]= operand(listctr, iden, flag, fontfamily, operandCtr);
                                                    flag=result4[0];
                                                    listctr=result4[1];
                                                   if(flag==1){
                                                     
                                                       break;}
                                                   else{
                                                       listctr++;
                                                      
                                                   }
                                                 if(list.get(listctr).Token.equals(")")){
                                                 listctr++;System.out.print("~~~JJ~~~");
                                               
                                             }
                                                 else if((list.get(listctr).Token.equals("==")||list.get(listctr).Token.equals("!=")||
                                              list.get(listctr).Token.equals("<")||list.get(listctr).Token.equals(">")
                                              ||list.get(listctr).Token.equals("<=")||list.get(listctr).Token.equals(">="))){
                                                       while(list.get(listctr).Token.equals("==")||list.get(listctr).Token.equals("!=")||
                                              list.get(listctr).Token.equals("<")||list.get(listctr).Token.equals(">")
                                              ||list.get(listctr).Token.equals("<=")||list.get(listctr).Token.equals(">=")){
                                                           listctr++;
                                                    operandCtr=0;
                                                    int result2[]= operand(listctr, iden, flag, fontfamily, operandCtr);
                                                    flag=result2[0];
                                                    listctr=result2[1];
                                                    //listctr++;
                                                       }
                                                           if(list.get(listctr).Token.equals(")")){
                                                 listctr++;
                                                 if(list.get(listctr).Token.equals(")")){
                                                 System.out.print("~~~HHMM~~~");}
                                             }
                                                          else{
                                                            if(flag!=1){
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '<', '>', '==', '<=', '>=', '!='. ");
                                                        flag=1;
                                                        break;}
                                                 }
                                                   }
                                             else{
                                               
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '<', '>', '==', '<=', '>=', '!='. ");
                                               
                                                        flag=1;
                                                        break;
                                                 }
                                                   }//if relexp or (
                                                   else{
                                                        if(flag!=1){
                                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                                           + "</span>'. Expecting 'true', 'false', Identifier or '('. ");
                                                        flag=1;
                                                        break;
                                                          }
                                                   }
                                                  }
                                                  else{
                                                  operandCtr=1;
                                                    int result[]= operand(listctr, iden, flag, fontfamily, operandCtr);
                                                    flag=result[0];
                                                    listctr=result[1];
                                                     System.out.print("gg "+flag+" HEEgg");}
                                                   if(flag==1){
                                                      // System.out.print("OOOOOO");
                                                       break;}
                                               
                                                   else{
                                                       listctr++;
                                                   }
                                                  ///CONDITION OPERAND END///
                                             if(list.get(listctr).Token.equals(")")){
                                                 listctr++;
                                             }
                                                 else if(stringFlag!=1&&(list.get(listctr).Token.equals("==")||list.get(listctr).Token.equals("!=")||
                                              list.get(listctr).Token.equals("<")||list.get(listctr).Token.equals(">")
                                              ||list.get(listctr).Token.equals("<=")||list.get(listctr).Token.equals(">="))){
                                                       while(list.get(listctr).Token.equals("==")||list.get(listctr).Token.equals("!=")||
                                              list.get(listctr).Token.equals("<")||list.get(listctr).Token.equals(">")
                                              ||list.get(listctr).Token.equals("<=")||list.get(listctr).Token.equals(">=")){
                                                           listctr++;
                                                    operandCtr=0;
                                                    int result2[]= operand(listctr, iden, flag, fontfamily, operandCtr);
                                                    flag=result2[0];
                                                    listctr=result2[1];
                                                    //listctr++;
                                                       }
                                                        if(list.get(listctr).Token.equals(")")){
                                                            listctr++;
                                                        }
                                                          else{
                                                            if(flag!=1){
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '<', '>', '==', '<=', '>=', '!='. ");
                                                        flag=1;
                                                        break;}
                                                 }
                                                   }
                                             else{
                                                if(stringFlag==0){
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '<', '>', '==', '<=', '>=', '!='. ");}
                                                else{
                                                      ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')'. ");
                                                }
                                                        flag=1;
                                                        break;
                                                 }
                                             
                                             
                                if(list.get(listctr).Token.equals("{")){
                                   
                                                   if(list.get(listctr).Token.equals("}")||list.get(listctr).Token.equals("fallin")||list.get(listctr).Token==("Identifier")
                              ||list.get(listctr).Token.equals("if")||list.get(listctr).Token.equals("for")||list.get(listctr).Token.equals("do")||list.get(listctr).Token.equals("++")||list.get(listctr).Token.equals("report")
                               ||list.get(listctr).Token.equals("execute")||list.get(listctr).Token.equals("Int Literal")||list.get(listctr).Token.equals("Float Literal")){
                                if(list.get(listctr).Token.equals("}")){
                                    
                                                listctr++;
                                                // System.out.print("\n mf");
                                                flag=1;
                                               }//if endmission
                                                 }
                                                   else{
                                                       
                                                      int result2[] = statements1(listctr, iden, flag, fontfamily, braceFlag);
                                                         flag=result2[0];
                                                         listctr=result2[1]; 
                                                         braceFlag=result2[2]; 
                                          
                                          
                                                         if(flag==1&&braceFlag!=1){
                                                             System.out.print("\nTOken"+flag+"\n");//flag=0;
                                                             break;}
                                                         else{
                                                             
                                                             flag=0;
                                                         }
                                                         ////ELSEIF
                                                  if(list.get(listctr).Token.equals("elseif")){
                                                    while(list.get(listctr).Token.equals("elseif")){
                                                     
                                                        stringFlag=0;

                                                   listctr++;
                                                 if(list.get(listctr).Token.equals("(")){
                                                       iden="";
                                                     listctr++;
                                                     
                                                  //CONDITON OPERAND///
                                                  if(list.get(listctr).Token.equals("String Literal")||list.get(listctr).Token.equals("Character Literal")){
                                                     stringFlag=1;
                                                      listctr++;
                                                      if(list.get(listctr).Token.equals("==")||list.get(listctr).Token.equals("!=")){
                                                          listctr++;
                                                          if(list.get(listctr).Token.equals("String Literal")||list.get(listctr).Token.equals("Character Literal")){
                                                      //listctr++;
                                                      System.out.print("");
                                                          }
                                                              else{
                                                          if(flag!=1){
                                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting String Literal or Character Literal. ");
                                                        flag=1;
                                                        break;
                                                          }
                                                      }
                                                      }
                                                      else{
                                                          if(flag!=1){
                                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '==' or '!='. ");
                                                        flag=1;
                                                        break;
                                                          }
                                                      }
                                                          
                                                  }
                                                  
                                                  
                                                  else if(list.get(listctr).Token.equals("!")){
                                                      stringFlag=1;
                                                   while(list.get(listctr).Token.equals("!")) {
                                                       listctr++;
                                                   } 
                                                   if(list.get(listctr).Token.equals("Identifier"))
                                                        System.out.print("");
                                                   else if(list.get(listctr).Token.equals("true")||list.get(listctr).Token.equals("false"))
                                                       System.out.print("");
                                                   else if(list.get(listctr).Token.equals("(")){
                                                       System.out.print("\n======HERE===\n");
                                                   listctr++;
                                                    operandCtr=1;
                                                    int result4[]= operand(listctr, iden, flag, fontfamily, operandCtr);
                                                    flag=result4[0];
                                                    listctr=result4[1];
                                                   if(flag==1){
                                                     
                                                       break;}
                                                   else{
                                                       listctr++;
                                                      
                                                   }
                                                 if(list.get(listctr).Token.equals(")")){
                                                 listctr++;System.out.print("~~~JJ~~~");
                                               
                                             }
                                                 else if((list.get(listctr).Token.equals("==")||list.get(listctr).Token.equals("!=")||
                                              list.get(listctr).Token.equals("<")||list.get(listctr).Token.equals(">")
                                              ||list.get(listctr).Token.equals("<=")||list.get(listctr).Token.equals(">="))){
                                                       while(list.get(listctr).Token.equals("==")||list.get(listctr).Token.equals("!=")||
                                              list.get(listctr).Token.equals("<")||list.get(listctr).Token.equals(">")
                                              ||list.get(listctr).Token.equals("<=")||list.get(listctr).Token.equals(">=")){
                                                           listctr++;
                                                    operandCtr=0;
                                                    int result5[]= operand(listctr, iden, flag, fontfamily, operandCtr);
                                                    flag=result5[0];
                                                    listctr=result5[1];
                                                    //listctr++;
                                                       }
                                                           if(list.get(listctr).Token.equals(")")){
                                                 listctr++;
                                                 if(list.get(listctr).Token.equals(")")){
                                                 System.out.print("~~~HHMM~~~");}
                                             }
                                                          else{
                                                            if(flag!=1){
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '<', '>', '==', '<=', '>=', '!='. ");
                                                        flag=1;
                                                        break;}
                                                 }
                                                   }
                                             else{
                                               
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '<', '>', '==', '<=', '>=', '!='. ");
                                               
                                                        flag=1;
                                                        break;
                                                 }
                                                   }//if relexp or (
                                                   else{
                                                        if(flag!=1){
                                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                                           + "</span>'. Expecting 'true', 'false', Identifier or '('. ");
                                                        flag=1;
                                                        break;
                                                          }
                                                   }
                                                  }
                                                  else{
                                                  operandCtr=1;
                                                    int result[]= operand(listctr, iden, flag, fontfamily, operandCtr);
                                                    flag=result[0];
                                                    listctr=result[1];
                                                     System.out.print("gg "+flag+" HEEgg");}
                                                   if(flag==1){
                                                      // System.out.print("OOOOOO");
                                                       break;}
                                               
                                                   else{
                                                       listctr++;
                                                   }
                                                  ///CONDITION OPERAND END///
                                             if(list.get(listctr).Token.equals(")")){
                                                 listctr++;
                                             }
                                                 else if(stringFlag!=1&&(list.get(listctr).Token.equals("==")||list.get(listctr).Token.equals("!=")||
                                              list.get(listctr).Token.equals("<")||list.get(listctr).Token.equals(">")
                                              ||list.get(listctr).Token.equals("<=")||list.get(listctr).Token.equals(">="))){
                                                       while(list.get(listctr).Token.equals("==")||list.get(listctr).Token.equals("!=")||
                                              list.get(listctr).Token.equals("<")||list.get(listctr).Token.equals(">")
                                              ||list.get(listctr).Token.equals("<=")||list.get(listctr).Token.equals(">=")){
                                                           listctr++;
                                                    operandCtr=0;
                                                    int result6[]= operand(listctr, iden, flag, fontfamily, operandCtr);
                                                    flag=result6[0];
                                                    listctr=result6[1];
                                                    //listctr++;
                                                       }
                                                        if(list.get(listctr).Token.equals(")")){
                                                            listctr++;
                                                        }
                                                          else{
                                                            if(flag!=1){
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '<', '>', '==', '<=', '>=', '!='. ");
                                                        flag=1;
                                                        break;}
                                                 }
                                                   }
                                             else{
                                                if(stringFlag==0){
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '<', '>', '==', '<=', '>=', '!='. ");}
                                                else{
                                                      ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')'. ");
                                                }
                                                        flag=1;
                                                        break;
                                                 }
                                             
                                             
                                if(list.get(listctr).Token.equals("{")){
                                   
                                                   if(list.get(listctr).Token.equals("}")||list.get(listctr).Token.equals("fallin")||list.get(listctr).Token==("Identifier")
                              ||list.get(listctr).Token.equals("if")||list.get(listctr).Token.equals("for")||list.get(listctr).Token.equals("do")||list.get(listctr).Token.equals("++")||list.get(listctr).Token.equals("report")
                               ||list.get(listctr).Token.equals("execute")||list.get(listctr).Token.equals("Int Literal")||list.get(listctr).Token.equals("Float Literal")){
                                if(list.get(listctr).Token.equals("}")){
                                    
                                                listctr++;
                                                // System.out.print("\n mf");
                                                flag=1;
                                               }//if endmission
                                                 }
                                                   else{
                                                       
                                                      int result7[] = statements1(listctr, iden, flag, fontfamily, braceFlag);
                                                         flag=result7[0];
                                                         listctr=result7[1]; 
                                                         braceFlag=result7[2]; 
                                          
                                                          if(flag==1&&braceFlag!=1)
                                                             break;
                                                         else{
                                                            //  braceFlag=1;
                                                             flag=0;
                                                             listctr--;
                                                             System.out.print("\n====HELLO"+list.get(listctr).Token+"====\n");
                                                         }
                                                         
                                                         
                                                   }
                                                   
                                                 }
                                
                                
                                               else{
                                                 if(flag!=1){
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '{'. ");
                                                        flag=1;
                                                        break;}
                                                 }
                              if(list.get(listctr).Token.equals("}")&&braceFlag==1){
                                                                                listctr++;
                                                             //                break; 
                                                             
                                                   ////////////////////////////IF CONDITION INDIE/////////////////
                 
                      
                                                            }
                              
                                                             else{
                                                 if(flag!=1){
                                                     //System.out.print("\n====YELLO"+braceFlag+"====\n");
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '}'.");
                                                        flag=1;
                                                        break;}
                                                 }
                                                 }
                                                 
                                                 else{
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '('. ");
                                                        flag=1;
                                                        break;
                                                 }
                                                 
                                                  }  
                                         }
                                                         
                                                   }
                                                   
                                                 }
                                               else{
                                                 if(flag!=1){
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '{'. ");
                                                        flag=1;
                                                        break;}
                                                 }
                              if(list.get(listctr-1).Token.equals("}")&&braceFlag==1){
                                                             System.out.print("");
                          ////////////////////////////////////////////ELSE/////////////////////////////
                                                                if(list.get(listctr).Token.equals("else")){
                                                                    listctr++;
                                                                       if(list.get(listctr).Token.equals("{")){
                                   
                                                   if(list.get(listctr).Token.equals("}")||list.get(listctr).Token.equals("fallin")||list.get(listctr).Token==("Identifier")
                              ||list.get(listctr).Token.equals("if")||list.get(listctr).Token.equals("for")||list.get(listctr).Token.equals("do")||list.get(listctr).Token.equals("++")||list.get(listctr).Token.equals("report")
                               ||list.get(listctr).Token.equals("execute")||list.get(listctr).Token.equals("Int Literal")||list.get(listctr).Token.equals("Float Literal")){
                                if(list.get(listctr).Token.equals("}")){
                                    
                                                listctr++;
                                                // System.out.print("\n mf");
                                                flag=1;
                                               }//if endmission
                                                 }
                                                   else{
                                                       
                                                      int result7[] = statements1(listctr, iden, flag, fontfamily, braceFlag);
                                                         flag=result7[0];
                                                         listctr=result7[1]; 
                                                         braceFlag=result7[2]; 
                                          
                                                          if(flag==1&&braceFlag!=1)
                                                             break;
                                                         else{
                                                            //  braceFlag=1;
                                                             flag=0;
                                                             listctr--;
                                                             System.out.print("\n====HELLO"+list.get(listctr).Token+"====\n");
                                                         }
                                                         
                                                         
                                                   }
                                                   
                                                 }
                                
                                
                                               else{
                                                 if(flag!=1){
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '{'. ");
                                                        flag=1;
                                                        break;}
                                                 }
                              if(list.get(listctr).Token.equals("}")&&braceFlag==1){
                                                                                listctr++;
                                                             //                break; 
                                                             
                                                   ////////////////////////////IF CONDITION INDIE/////////////////
                 
                      
                                                            }
                              
                                                             else{
                                                 if(flag!=1){
                                                     //System.out.print("\n====YELLO"+braceFlag+"====\n");
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '}'. ");
                                                        flag=1;
                                                        break;}
                                                 }   
                                                                }
                                                            }
                              
                                                             else{
                                                 if(flag!=1){
                                                     //System.out.print("\n====YELLO"+list.get(listctr).Token+"====\n");
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '}'. ");
                                                        flag=1;
                                                        break;}
                                                 }
                                                 }
                                                 
                                                 else{
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '('. ");
                                                        flag=1;
                                                        break;
                                                 }
                                                 
                                              
                                         }
                                                                
      ////////////////////////////END OF IF CONDITION INDIE/////////////////         
      ///////////////////////////EXPRESSION INDIE//////////////////////////
                         else if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("++")
                                 ||list.get(listctr).Token.equals("--")||list.get(listctr).Token.equals("Int Literal")
                                 ||list.get(listctr).Token.equals("Float Literal")){
                             iden="";
                           iden = iden.concat("<statements2>");
                            listSyntax.add(new Syntax("<statements4> ",iden));
                              iden="";
                            iden = iden.concat("<statements2>");
                            listSyntax.add(new Syntax("<statements4> ",iden));
                              iden="";
                            iden = iden.concat("<exp>");
                            listSyntax.add(new Syntax("<statements2> ",iden));
                          
                            if(list.get(listctr).Token.equals("Identifier")){
                                 //arithexp2///////////////////
                                 listctr++;
                                 if(list.get(listctr).Token.equals("=")){
                            iden="";
                            iden = iden.concat("<arithexp2>");
                            listSyntax.add(new Syntax("<exp> ",iden));
                             iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                             iden="";
                            iden = iden.concat("=");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                             iden="";
                            iden = iden.concat("<operand>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                            
                            listctr++;
                            
                            if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                                   int result2[] = arithexp1(listctr, iden, flag, fontfamily, arrayFlag);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;      
                                                 break;
                            }
                            if(flag==0){
                            if(list.get(listctr).Token.equals(";")){
                            iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<exp> ",iden));
                            listctr++;
                            flag=0;
                            continue;
                            }
                             else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("<arithop>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                         if(list.get(listctr).Token.equals("+")){
                            iden="";
                            iden = iden.concat("+");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("-")){
                            iden="";
                            iden = iden.concat("-");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("*")){
                            iden="";
                            iden = iden.concat("*");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("/")){
                            iden="";
                            iden = iden.concat("/");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("%");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         
                      
                            }
                              else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';', '+','-','*','/','%' ");
                                                 flag=1;      
                                                 break;
                         }
                           
                           if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            
                            ///////////////////////////////ARRAY ACCESS///////////////////////////////////////////////////
                            if(list.get(listctr).Token.equals("[")){
                            //   int arrayFlag=0;
                             int result1[] = arrayAccess(listctr, iden, flag, fontfamily, arrayFlag,dtype);
                                                         flag=result1[0];
                                                         listctr=result1[1];
                            }
 /////////////////////////////////////END OF ARRAY ACCESS/////////////////////////////
 /////////////////////STRUCT ACCESS///////////////////////////////
                            else if(list.get(listctr).Token.equals(".")){
                                int result2[] = structAccess(listctr, iden, flag, fontfamily);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                            }
                            
    ///////////////////////////////END OF STRUCT ACCESS//////////////////////////////////////////////////
                            
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            //////// PARENTHESIS ARITHEXP1 ////
                            if(list.get(listctr).Token.equals(")")){
                                iden="";
                            iden = iden.concat(")");
                            listSyntax.add(new Syntax("<operand>",iden));
                                listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")){
                                
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            
                            //loop
                           if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")|| list.get(listctr).Token.equals(")")){
                               
                               if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   //break;
                               }
                               else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                                
                             while(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")||list.get(listctr).Token.equals(")")){
                                  
                             if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   break;
                               }
                                 else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("<arithop>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                         if(list.get(listctr).Token.equals("+")){
                            iden="";
                            iden = iden.concat("+");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("-")){
                            iden="";
                            iden = iden.concat("-");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("*")){
                            iden="";
                            iden = iden.concat("*");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("/")){
                            iden="";
                            iden = iden.concat("/");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("%");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                          if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            
                                 //////// PARENTHESIS ARITHEXP1 ////
                            if(list.get(listctr).Token.equals(")")){
                                iden="";
                            iden = iden.concat(")");
                            listSyntax.add(new Syntax("<operand>",iden));
                                listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")){
                                
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            
                            //loop
                           if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")|| list.get(listctr).Token.equals(")")){
                               
                               if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   //break;
                               }
                               else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                                
                             while(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")||list.get(listctr).Token.equals(")")){
                                  
                             if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   break;
                               }
                                 else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("<arithop>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                         if(list.get(listctr).Token.equals("+")){
                            iden="";
                            iden = iden.concat("+");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("-")){
                            iden="";
                            iden = iden.concat("-");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("*")){
                            iden="";
                            iden = iden.concat("*");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("/")){
                            iden="";
                            iden = iden.concat("/");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("%");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                          if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                                     
                             if(list.get(listctr).Token.equals(")")){
                           
                            listctr++;
                            break;
                            }
                             else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%"))
                                 continue;
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')','+','-','*','/','%'. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                             
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                                        
                            }
                              else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%' ");
                                                 flag=1;      
                                                 break;
                         }
                            
                     
                                   
                                   }//WHILE 
                    
                               }//if arithexp3
                           }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%'. ");
                                                 flag=1;      
                                                 break;
                            }
                            ////////////////
                                
                            }
                            else{
                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or ')'. ");
                                                 flag=1;      
                                                 break;  
                            }
                 /////// END OF PARENTHESIS ARITHEXP1 ///
                            }
                            
                           
                             if(list.get(listctr).Token.equals(")")){
                           
                            listctr++;
                            break;
                            }
                             else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%"))
                                 continue;
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')','+','-','*','/','%'. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                             
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                                        
                            }
                              else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%' ");
                                                 flag=1;      
                                                 break;
                         }
                            
                     
                                   
                                   }//WHILE 
                    
                               }//if arithexp3
                           }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%'. ");
                                                 flag=1;      
                                                 break;
                            }
                            ////////////////
                                
                            }
                            else{
                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or ')'. ");
                                                 flag=1;      
                                                 break;  
                            }
                 /////// END OF PARENTHESIS ARITHEXP1 ///
                            }
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;      
                                                 break;
                            }
                           
                           if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")|| list.get(listctr).Token.equals(";")){
                               
                               if(list.get(listctr).Token.equals(";")){
                                   listctr++;
                                   flag=0;
                                   //break;
                               }
                               else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                                
                             while(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")||list.get(listctr).Token.equals(";")){
                                  
                             if(list.get(listctr).Token.equals(";")){
                                   listctr++;
                                   flag=0;
                                   break;
                               }
                                 else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("<arithop>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                         if(list.get(listctr).Token.equals("+")){
                            iden="";
                            iden = iden.concat("+");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("-")){
                            iden="";
                            iden = iden.concat("-");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("*")){
                            iden="";
                            iden = iden.concat("*");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("/")){
                            iden="";
                            iden = iden.concat("/");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("%");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                                        if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            //////// PARENTHESIS ARITHEXP1 ////
                            if(list.get(listctr).Token.equals(")")){
                                iden="";
                            iden = iden.concat(")");
                            listSyntax.add(new Syntax("<operand>",iden));
                                listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")){
                                
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            
                            //loop
                           if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")|| list.get(listctr).Token.equals(")")){
                               
                               if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   //break;
                               }
                               else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                                
                             while(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")||list.get(listctr).Token.equals(")")){
                                  
                             if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   break;
                               }
                                 else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("<arithop>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                         if(list.get(listctr).Token.equals("+")){
                            iden="";
                            iden = iden.concat("+");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("-")){
                            iden="";
                            iden = iden.concat("-");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("*")){
                            iden="";
                            iden = iden.concat("*");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("/")){
                            iden="";
                            iden = iden.concat("/");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("%");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                          if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            
                                 //////// PARENTHESIS ARITHEXP1 ////
                            if(list.get(listctr).Token.equals(")")){
                                iden="";
                            iden = iden.concat(")");
                            listSyntax.add(new Syntax("<operand>",iden));
                                listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")){
                                
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            
                            //loop
                           if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")|| list.get(listctr).Token.equals(")")){
                               
                               if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   //break;
                               }
                               else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                                
                             while(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")||list.get(listctr).Token.equals(")")){
                                  
                             if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   break;
                               }
                                 else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("<arithop>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                         if(list.get(listctr).Token.equals("+")){
                            iden="";
                            iden = iden.concat("+");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("-")){
                            iden="";
                            iden = iden.concat("-");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("*")){
                            iden="";
                            iden = iden.concat("*");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("/")){
                            iden="";
                            iden = iden.concat("/");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("%");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                          if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                                     
                             if(list.get(listctr).Token.equals(")")){
                           
                            listctr++;
                            break;
                            }
                             else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%"))
                                 continue;
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')','+','-','*','/','%'. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                             
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                                        
                            }
                              else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%' ");
                                                 flag=1;      
                                                 break;
                         }
                            
                     
                                   
                                   }//WHILE 
                    
                               }//if arithexp3
                           }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%'. ");
                                                 flag=1;      
                                                 break;
                            }
                            ////////////////
                                
                            }
                            else{
                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or ')'. ");
                                                 flag=1;      
                                                 break;  
                            }
                 /////// END OF PARENTHESIS ARITHEXP1 ///
                            }
                            
                           
                             if(list.get(listctr).Token.equals(")")){
                           
                            listctr++;
                            break;
                            }
                             else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%"))
                                 continue;
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')','+','-','*','/','%'. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                             
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                                        
                            }
                              else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%' ");
                                                 flag=1;      
                                                 break;
                         }
                            
                     
                                   
                                   }//WHILE 
                    
                               }//if arithexp3
                           }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%'. ");
                                                 flag=1;      
                                                 break;
                            }
                            ////////////////
                                
                            }
                            else{
                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or ')'. ");
                                                 flag=1;      
                                                 break;  
                            }
                 /////// END OF PARENTHESIS ARITHEXP1 ///
                            }
                                     
                             if(list.get(listctr).Token.equals(";")){
                           
                            listctr++;
                            break;
                            }
                             else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%"))
                                 continue;
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';','+','-','*','/','%'. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                             
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                                        
                            }
                              else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';', '+','-','*','/','%' ");
                                                 flag=1;      
                                                 break;
                         }
                            
                     
                                   
                                   }//WHILE 
                    
                               }//if arithexp3
                           }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';', '+','-','*','/','%'. ");
                                                 flag=1;      
                                                 break;
                            }
                                 }
                            
                          }
                 //======iterate=============//
                    else if(list.get(listctr).Token.equals("++")||list.get(listctr).Token.equals("--")){
                           if(list.get(listctr).Token.equals("++")){
                            iden="";
                            iden = iden.concat("<iterate>");
                            listSyntax.add(new Syntax("<exp> ",iden));
                             iden="";
                            iden = iden.concat("<unexp>");
                            listSyntax.add(new Syntax("<iterate> ",iden));
                              iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<unexp> ",iden));
                            iden="";
                            iden = iden.concat("<unop>");
                            listSyntax.add(new Syntax("<unexp> ",iden));
                            iden="";
                            iden = iden.concat("++");
                            listSyntax.add(new Syntax("<unop> ",iden));
                            listctr++;
                            }
                        else if(list.get(listctr).Token.equals("--")){
                            iden="";
                            iden = iden.concat("<iterate>");
                            listSyntax.add(new Syntax("<exp> ",iden));
                             iden="";
                            iden = iden.concat("<unexp>");
                            listSyntax.add(new Syntax("<iterate> ",iden));
                              iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<unexp> ",iden));
                            iden="";
                            iden = iden.concat("<unop>");
                            listSyntax.add(new Syntax("<unexp> ",iden));
                            iden="";
                            iden = iden.concat("--");
                            listSyntax.add(new Syntax("<unop> ",iden));
                            listctr++;
                            }
                           if(list.get(listctr).Token.equals(";")){
                            iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<exp> ",iden));
                           listctr++;
                           flag=0;
                           }
                           
                           else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';'. ");
                                                 flag=1;      
                                                 break;
                                 }
                    
                    }
                     //=======end of iterate====================================================//
                    ///ASSIGNMENT EXPRESSION//////////////////////////////////////////////
                    
                                else if(list.get(listctr).Token.equals("+=")||list.get(listctr).Token.equals("-=")
                                       ||list.get(listctr).Token.equals("/=")||list.get(listctr).Token.equals("*=")
                                       ||list.get(listctr).Token.equals("%=")){
                           if(list.get(listctr).Token.equals("+=")){
                            iden="";
                            iden = iden.concat("<assignexp>");
                            listSyntax.add(new Syntax("<exp> ",iden));
                             iden="";
                            iden = iden.concat("<assignexp2>");
                            listSyntax.add(new Syntax("<assignexp> ",iden));
                              iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("<assignop2>");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("+=");
                            listSyntax.add(new Syntax("<assignop2> ",iden));
                            listctr++;
                            }
                           else if(list.get(listctr).Token.equals("-=")){
                            iden="";
                            iden = iden.concat("<assignexp>");
                            listSyntax.add(new Syntax("<exp> ",iden));
                             iden="";
                            iden = iden.concat("<assignexp2>");
                            listSyntax.add(new Syntax("<assignexp> ",iden));
                              iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("<assignop2>");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("-=");
                            listSyntax.add(new Syntax("<assignop2> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("*=")){
                            iden="";
                            iden = iden.concat("<assignexp>");
                            listSyntax.add(new Syntax("<exp> ",iden));
                             iden="";
                            iden = iden.concat("<assignexp2>");
                            listSyntax.add(new Syntax("<assignexp> ",iden));
                              iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("<assignop2>");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("*=");
                            listSyntax.add(new Syntax("<assignop2> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("/=")){
                            iden="";
                            iden = iden.concat("<assignexp>");
                            listSyntax.add(new Syntax("<exp> ",iden));
                             iden="";
                            iden = iden.concat("<assignexp2>");
                            listSyntax.add(new Syntax("<assignexp> ",iden));
                              iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("<assignop2>");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("/=");
                            listSyntax.add(new Syntax("<assignop2> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("%=")){
                            iden="";
                            iden = iden.concat("<assignexp>");
                            listSyntax.add(new Syntax("<exp> ",iden));
                             iden="";
                            iden = iden.concat("<assignexp2>");
                            listSyntax.add(new Syntax("<assignexp> ",iden));
                              iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("<assignop2>");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("%=");
                            listSyntax.add(new Syntax("<assignop2> ",iden));
                            listctr++;
                            }
                           if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                        
                            
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand>",iden));
                            listctr++;
                            
                            
                             //////// PARENTHESIS ARITHEXP1 ////
                            if(list.get(listctr).Token.equals(")")){
                                iden="";
                            iden = iden.concat(")");
                            listSyntax.add(new Syntax("<operand>",iden));
                                listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")){
                                
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            
                      
                           //loop
                             if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")|| list.get(listctr).Token.equals(")")){
                               
                               if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   //break;
                               }
                               else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                                
                             while(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")||list.get(listctr).Token.equals(")")){
                                  
                             if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   break;
                               }
                                 else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("<arithop>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                         if(list.get(listctr).Token.equals("+")){
                            iden="";
                            iden = iden.concat("+");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("-")){
                            iden="";
                            iden = iden.concat("-");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("*")){
                            iden="";
                            iden = iden.concat("*");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("/")){
                            iden="";
                            iden = iden.concat("/");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("%");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                          if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            
                                 //////// PARENTHESIS ARITHEXP1 ////
                            if(list.get(listctr).Token.equals(")")){
                                iden="";
                            iden = iden.concat(")");
                            listSyntax.add(new Syntax("<operand>",iden));
                                listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")){
                                
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            
                            //loop
                           if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")|| list.get(listctr).Token.equals(")")){
                               
                               if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   //break;
                               }
                               else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                                
                             while(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")||list.get(listctr).Token.equals(")")){
                                  
                             if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   break;
                               }
                                 else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("<arithop>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                         if(list.get(listctr).Token.equals("+")){
                            iden="";
                            iden = iden.concat("+");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("-")){
                            iden="";
                            iden = iden.concat("-");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("*")){
                            iden="";
                            iden = iden.concat("*");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("/")){
                            iden="";
                            iden = iden.concat("/");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("%");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                          if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                                     
                             if(list.get(listctr).Token.equals(")")){
                           
                            listctr++;
                            break;
                            }
                             else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%"))
                                 continue;
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')','+','-','*','/','%'. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                             
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                                        
                            }
                              else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%' ");
                                                 flag=1;      
                                                 break;
                         }
                            
                     
                                   
                                   }//WHILE 
                    
                               }//if arithexp3
                           }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%'. ");
                                                 flag=1;      
                                                 break;
                            }
                            ////////////////
                                
                            }
                            else{
                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or ')'. ");
                                                 flag=1;      
                                                 break;  
                            }
                 /////// END OF PARENTHESIS ARITHEXP1 ///
                            }
                            
                           
                             if(list.get(listctr).Token.equals(")")){
                           
                            listctr++;
                            break;
                            }
                             else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%"))
                                 continue;
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')','+','-','*','/','%'. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                             
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                                        
                            }
                              else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%' ");
                                                 flag=1;      
                                                 break;
                         }
                            
                     
                                   
                                   }//WHILE 
                    
                               }//if arithexp3
                           }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%', ';'. ");
                                                 flag=1;      
                                                 break;
                            }
                            ////////////////
                                
                            }
                            else{
                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or ')'. ");
                                                 flag=1;      
                                                 break;  
                            }
                 /////// END OF PARENTHESIS ARITHEXP1 ///
                            
                            
                            }
                            
                            if(list.get(listctr).Token.equals(";")){
                            iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<exp> ",iden));
                            listctr++;
                            flag=0;
                            continue;
                            }
                            else{
                                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';'. ");
                                                 flag=1;      
                                                 break; 
                            }
                            
                           }
                           
                           else{
                                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, Integer Literal, Float Literal, or '('. ");
                                                 flag=1;      
                                                 break;
                           }
                                }
                 ///ASSIGNMENT EXPRESSION//////////////////////////////////////////////
               
                             //ELSE ON IDENTIFIER
                                 else{
                                    if(flag!=1){
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '+=', '-=', '*=', '/=', '%=', '=', '++' or '--'. ");
                                                 flag=1;      
                                                 break;}
                                 }
                            
                            }
                                //======iterate=============//
                    else if(list.get(listctr).Token.equals("++")||list.get(listctr).Token.equals("--")){
                                 iden="";
                           iden = iden.concat("<statements2>");
                            listSyntax.add(new Syntax("<statements4> ",iden));
                              iden="";
                            iden = iden.concat("<statements2>");
                            listSyntax.add(new Syntax("<statements4> ",iden));
                              iden="";
                            iden = iden.concat("<exp>");
                            listSyntax.add(new Syntax("<statements2> ",iden));
                            iden="";
                            iden = iden.concat("<iterate>");
                            listSyntax.add(new Syntax("<exp> ",iden));
                             iden="";
                            iden = iden.concat("<unexp>");
                            listSyntax.add(new Syntax("<iterate> ",iden));
                            iden="";
                            iden = iden.concat("<unop>");
                            listSyntax.add(new Syntax("<unexp> ",iden));
                            
                           if(list.get(listctr).Token.equals("++")){
                             iden="";
                            iden = iden.concat("++");
                            listSyntax.add(new Syntax("<unop> ",iden));
                            listctr++;
                            }
                        else if(list.get(listctr).Token.equals("--")){
                             iden="";
                            iden = iden.concat("--");
                            listSyntax.add(new Syntax("<unop> ",iden));
                            listctr++;
                            }
                           if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<unexp> ",iden));
                           listctr++;
                           //flag=0;
                           
                            if(list.get(listctr).Token.equals(";")){
                            iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<exp> ",iden));
                           listctr++;
                           flag=0;
                           }
                           
                           else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';'. ");
                                                 flag=1;      
                                                 break;
                                 }
                           }
                           
                           else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier. ");
                                                 flag=1;      
                                                 break;
                                 }
                    
                    }
                //=======end of iterate====================================================//
                            else{
                                listctr++;
                                flag=0;
                            }
                         }
                         else if(flag==1){
                             break;
                         }
      ///////////////////////////////END OF EXPRESSION INDIE///////////////////////
        ///////////////////////////////////////////////END OF STATEMENTS2/////////////////////////////////
               //////////////////////////END OF STATEMENTS IN BODY//////////////////////////////////////////////////////////////////////////////////////////////
                                                             else{
                             if(flag!=1){
                                                 ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting 'fallin', 'report', 'execute', 'Identifier', 'if', 'for', 'do', '++', '--', 'endmission'. ");
                                                 flag=1;      
                                                 break;}
                                                }//else NOT statement >>startmission:
                                      }//WHILE LOOP COMMAND INDIE
                     }//IF COMMAND INDIE
                                                if(flag==0){
                                                     if(list.get(listctr).Token.equals("endmission")){
                                                       listctr++;
                                                   }
                                                         else{
                                                 ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting 'endmission'. ");
                                                 flag=1;      
                                    //             break;
                                                }//else NOT endmission >>startmission:
                                                }
                             //flag=0;                   
                                       }//if :
                                      
                                        else{
                                         ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ':'. ");
                                         flag=1;      
                                  //       break;
                                                
                                        }//else NOT : >>startmission:
                                      
                                      if(flag==0){
                                          //FOLLOW SET NG COMMAND() == DISMISS OR USER DEF
                       if(list.get(listctr).Token.equals("dismiss")||list.get(listctr).Token.equals("int")||list.get(listctr).Token.equals("flt")
                          ||list.get(listctr).Token.equals("onduty")){
                           listctr++;
                       }
                       else{
                           ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting 'int', 'flt', 'chr', 'str', 'bool', 'command()', 'dismiss', 'const' or 'onduty'. ");
                                //                break;
                       }
                                      }
                                      
                     }//if startmission
                        else{
                           ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting 'startmission'. ");
                                //                break;
                       }
                     
//                     else{
//                      // listctr++; //OKAY LANG NA NULL 
//                       //FOLLOW SET NG COMMAND() == DISMISS OR USER DEF
//                       if(list.get(listctr).Token.equals("dismiss")||list.get(listctr).Token=="int"||list.get(listctr).Token=="flt"
//                               ||list.get(listctr).Token.equals("onduty")){
//                           if(list.get(listctr).Token==("dismiss")&&(list.size()-1)==listctr)
//                          System.out.print("");
//                           else
//                           listctr++;
//                          
//                       }
//                       else{
//                           ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr-1).Lexeme+""
//                             + "</span>'. Expecting 'int', 'flt', 'chr', 'str', 'bool', 'command()', 'dismiss', 'const' or 'onduty'. ");
//                              //                  break;
//                       }
//                     }
                                 
                     }//if )
                     else{
                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', 'int', 'flt', 'chr', 'bool', 'str'. ");
                            // break;
                     }
                     }//if (
                     else{
                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '('. ");
                          //   break;
                     }
        } catch (BadLocationException ex) {
            Logger.getLogger(Frame.class.getName()).log(Level.SEVERE, null, ex);
        }
         return new int[] {flag, listctr};
    }
    
     public int[] statements1(int listctr, String iden, int flag, String fontfamily, int braceFlag){
        try {
            listctr++;
            braceFlag=0;
            ArrayList<Lexical> list = ListLexical();
            ArrayList<Syntax> listSyntax = new ArrayList<Syntax>();
                         while(list.get(listctr).Token!="}")
                           {//WHILE LOOP statements INDIE 
                                        
                                                if(list.get(listctr).Token.equals("}")){
                                                listctr++;
                                                 System.out.print("\n mf");
                                                flag=1;
                                                braceFlag=1;
                                               }//if endmission
                                                   ////////////////////////STATEMENTS2 OR STATEMENT IN BODY//////////////////////////////////////////////////////////////////////////////////////
                                               else if(list.get(listctr).Token.equals("fallin")){
                                                listctr++;
                                                braceFlag=1;
                                                    if(list.get(listctr).Token.equals("{")){
                                                    listctr++;
                                                    
                                                     if(list.get(listctr).Token.equals("}")){
                                                    listctr++;
                                                   // flag=1;
                                                     }
                                                      ////////////////////////////////VARIABLE DECLARATION///////////////////////////////////////////////////////
                    else if(list.get(listctr).Token=="int"||list.get(listctr).Token=="flt"||list.get(listctr).Token=="chr"||list.get(listctr).Token=="bool"||list.get(listctr).Token=="str"){
                    while(list.get(listctr).Token=="int"||list.get(listctr).Token=="flt"||list.get(listctr).Token=="chr"||list.get(listctr).Token=="bool"||list.get(listctr).Token=="str"){
                      dtype =list.get(listctr).Token;
                        listctr++;
                            if(list.get(listctr).Token=="Identifier"){
                                listctr++;
                                
                                if(list.get(listctr).Token.equals("=")){
                                    listctr++;
                                    
                                    if(list.get(listctr).Token=="true"||list.get(listctr).Token=="false"||list.get(listctr).Token=="Int Literal"||list.get(listctr).Token=="Float Literal"||list.get(listctr).Token.equals("(")||list.get(listctr).Token.equals("Character Literal")||list.get(listctr).Token.equals("String Literal")){
                                        if(dtype=="int"&&list.get(listctr).Token=="Int Literal"){
                                            listctr++;
                                        }
                                         else if(dtype=="flt"&&list.get(listctr).Token=="Float Literal"){
                                            listctr++;
                                        }
                                         else if(dtype=="bool"&&list.get(listctr).Token=="true"||
                                                 dtype=="bool"&&list.get(listctr).Token=="false"){
                                            listctr++;
                                        }
                                    else if(dtype=="str"&&list.get(listctr).Token=="String Literal"){
                                            listctr++;
                                        }
                                     else if(dtype=="chr"&&list.get(listctr).Token=="Character Literal"){
                                            listctr++;
                                        }
                                        else if(dtype=="int"&&list.get(listctr).Token.equals("(")){
                                            listctr++;
                                            if(list.get(listctr).Token=="Negative Int Literal"){
                                                listctr= listctr+2;
                                            }
                                      
                                            else{
                                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Negative Integer Literal. "
); 
                                             flag=1;
                                       break; 
                                            }//else not negative int lit
                                        }
                                        else{
                                            System.out.print("JJEEKEE");
                                         ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '"+dtype+" literal'. ");  
                                       flag=1;
                                       listctr++;
                                         break;
                                        }
                                        
                     /////////////////////////////////SEPARATOR LOOP FOR a=number///////////////////////////////////////////////////////////////////   
                    // if(flag==0){
                     if(list.get(listctr).Token.equals(";")||list.get(listctr).Token.equals(" ;")){
                                            listctr++;
                                            
                                            //break;
                                        }
                                    else if(list.get(listctr).Token.equals(",")){
                                    arrayFlag=0;
                              int result2[] = comma(listctr, iden, flag, fontfamily, arrayFlag);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                                 }//else if ,
                                         else{
                                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '=', ',' or ';'.");  
                                            flag=1;
                                       break; 
                                         }
                                   
                                    }//if LITERAL
                                    else{
                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '"+dtype+" literal'. ");
                                        flag=1;
                                    break;
                                    }//else not LITERAL
                                }//if =
                            /////////////////////////////////END OF SEPARATOR LOOP FOR a=number///////////////////////////////////////////////////////////////////  
                                 else if(list.get(listctr).Token.equals(",")){
                                     arrayFlag=0;
                              int result2[] = comma(listctr, iden, flag, fontfamily, arrayFlag);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                                 }//else if ,
                                else if(list.get(listctr).Token.equals(";")){
                                    listctr++;
                                    System.out.print("LOLOL");
                                  //  break;
                                }
                                ////////////////////////////////////////ARRAY DECLARATION//////////////////////////////////////////////////////////////////
                                else if(list.get(listctr).Token.equals("[")){
                                    arrayFlag=1;
                             int result1[] = arrayAccess(listctr, iden, flag, fontfamily, arrayFlag,dtype);
                                                         flag=result1[0];
                                                         listctr=result1[1];
                        
                           arrayFlag=0;
                        
                                    
                                }

//////////////////////////////END OF ARRAY DECLARATION//////////////////////////////////////////////////////////////////////  
                                         else{
                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '=', ',', '[', or ';'. ");
                                        flag=1;
                                break;
                                }//else not = 
                                
                            }//if Identifier
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier. ");   
                             flag=1;
                            break;
                            }//else not Identifier
                          }//while VAR DEC
                    if(flag==0){
                    if(list.get(listctr).Token.equals("}")||list.get(listctr).Token.equals(" }")){
                                                listctr++;
                                                //flag=1;
                                               }//if }
                    else{
                                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '}' or 'int', 'flt', 'chr', 'str', 'bool'. ");
                                                        flag=1;
                                                        break;
                                                    }
                     } //IF FLAG==0 FOR SEPARATOR
                 }//if VAR DEC
                                                      else{
                                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '}' or 'int', 'flt', 'chr', 'str', 'bool'. ");
                                                        flag=1;
                                                        break;
                                                    }
                                                   //  flag=0;
                                                   }//if }
                                                    else{
                                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '{'. ");
                                                        flag=1;
                                                        break;
                                                    }
                                                     if(flag==0){ if(list.get(listctr).Token.equals("endmission")){
                                                System.out.print("here"); break;
                                               }//if endmission
                                                     else if(list.get(listctr).Token==("command")||list.get(listctr).Token==("fallin")||list.get(listctr).Token==("Identifier")
                              ||list.get(listctr).Token==("if")||list.get(listctr).Token==("for")||list.get(listctr).Token==("do")||list.get(listctr).Token==("++")||list.get(listctr).Token==("report")
                               ||list.get(listctr).Token==("execute")||list.get(listctr).Token.equals("Int Literal")||list.get(listctr).Token.equals("Float Literal")){
                                                     continue;
                                                     }
                                                   else{
                                                         if(flag!=1){
                                                 ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting 'fallin', 'report', 'execute', 'Identifier', 'if', 'for', 'do', '++', '--', 'endmission'. ");
                                                 flag=1;
                                                        break;}
                                                }//else NOT endmission >>startmission
                                                     }//if flag==0 startmission
                                               }//if fallin
                                                          /////////////////////////////////STATEMENTS2//////////////////
                                                    else if(list.get(listctr).Token.equals("execute")){
                                                        iden="";
                                                   iden = iden.concat("<statements2>");
                            listSyntax.add(new Syntax("<statements4> ",iden));
                            
                              iden="";
                                                   iden = iden.concat("<input>");
                            listSyntax.add(new Syntax("<statements2> ",iden));
                              iden="";
                                                   iden = iden.concat("execute");
                            listSyntax.add(new Syntax("<input> ",iden));
                                                
                                                   listctr++;
                                                   
                                                   if(list.get(listctr).Token.equals("Identifier")){
                                                         iden="";
                                                   iden = iden.concat("id");
                            listSyntax.add(new Syntax("<input> ",iden));
                                                       listctr++;
                                                   }
                                                   else{
                                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier. ");
                                                        flag=1;
                                                        break;
                                                   }
                                                       if(list.get(listctr).Token.equals(";")){
                                                             iden="";
                                                   iden = iden.concat(";");
                            listSyntax.add(new Syntax("<statements2> ",iden));
                                                       listctr++;
                                                       System.out.print("HMMM");
                                                       //flag=1;
                                                   }
                                                   else{
                                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';'. ");
                                                        flag=1;
                                                        break;
                                                   }
                                               }
                                 else if(list.get(listctr).Token.equals("report")){
                                                                      iden="";
                                                   iden = iden.concat("<statements2");
                            listSyntax.add(new Syntax("<statements4> ",iden));
                              iden="";
                                                   iden = iden.concat("<output>");
                            listSyntax.add(new Syntax("<statements2> ",iden));
                              iden="";
                          iden = iden.concat("report");
                            listSyntax.add(new Syntax("<output> ",iden));
                                                   listctr++;
                             if(list.get(listctr).Token.equals("Identifier")){
                                                       
                              iden="";
                                                   iden = iden.concat("<display>");
                            listSyntax.add(new Syntax("<output> ",iden));
                              iden="";
                                                   iden = iden.concat("id");
                            listSyntax.add(new Syntax("<display> ",iden));
                                                       listctr++;
                                                                          if(list.get(listctr).Token.equals("+")){
                                                            
                                                       while((list.get(listctr).Token.equals("+"))){
                                                         //  System.out.print("unit test");
                                                            listctr++;
                                                         if(list.get(listctr).Token.equals("String Literal")||list.get(listctr).Token.equals("Identifier")){
                                                         listctr++;
                                                         
                                                         }
                                                         else{
                                                                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, or String Literal. ");
                                                        flag=1;
                                                        break;  
                                                        
                                                        }
                                                       }
                                                   }
                                                        if(flag==1)
                                                            break;
                                                   }
                             else if(list.get(listctr).Token.equals("String Literal")){
                                                       
                              iden="";
                                                   iden = iden.concat("<display>");
                            listSyntax.add(new Syntax("<output> ",iden));
                              iden="";
                                                   iden = iden.concat("strlit");
                            listSyntax.add(new Syntax("<display> ",iden));
                                                       listctr++;
                                                       
                                                        if(list.get(listctr).Token.equals("+")){
                                                            
                                                       while((list.get(listctr).Token.equals("+"))){
                                                         //  System.out.print("unit test");
                                                            listctr++;
                                                         if(list.get(listctr).Token.equals("String Literal")||list.get(listctr).Token.equals("Identifier")){
                                                         listctr++;
                                                         
                                                         }
                                                         else{
                                                                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, or String Literal. ");
                                                        flag=1;
                                                        break;  
                                                        
                                                        }
                                                       }
                                                   }
                                                        if(flag==1)
                                                            break;
                                                        
                             }
                                                   else{
                                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, or String Literal. ");
                                                        flag=1;
                                                        break;
                                                   }
                                                       if(list.get(listctr).Token.equals(";")&&flag!=1){
                                                            iden="";
                                                   iden = iden.concat("\"");
                                                   
                            listSyntax.add(new Syntax("<statements2> ",iden));
                                                       listctr++;
                                                       //flag=1;
                                                       flag=0;
                                                   }

                                                   else{
                                                        ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';' or '+'. ");
                                                        flag=1;
                                                        break;
                                                   }
                                               }
                             
       ////////////////////////////IF CONDITION INDIE/////////////////
                     else if(list.get(listctr).Token.equals("if")){
                         braceFlag=1;
                         int stringFlag=0;
                         System.out.print("IFFF\n");
                             iden="";
                           iden = iden.concat("<statements2>");
                            listSyntax.add(new Syntax("<statements4> ",iden));
                              iden="";
                            iden = iden.concat("<statements2>");
                            listSyntax.add(new Syntax("<statements4> ",iden));
                              iden="";
                            iden = iden.concat("<conditional1>");
                            listSyntax.add(new Syntax("<statements2> ",iden));
                              iden="";
                              iden = iden.concat("if");
                            listSyntax.add(new Syntax("<conditional1> ",iden));
                                                   listctr++;
                                                 if(list.get(listctr).Token.equals("(")){
                                                       iden="";
                                                    iden = iden.concat("(");
                                                  listSyntax.add(new Syntax("<conditional1> ",iden));
                                                     listctr++;
                                                     
                                                  //CONDITON OPERAND///
                                                  if(list.get(listctr).Token.equals("String Literal")||list.get(listctr).Token.equals("Character Literal")){
                                                     stringFlag=1;
                                                      listctr++;
                                                      if(list.get(listctr).Token.equals("==")||list.get(listctr).Token.equals("!=")){
                                                          listctr++;
                                                          if(list.get(listctr).Token.equals("String Literal")||list.get(listctr).Token.equals("Character Literal")){
                                                      //listctr++;
                                                      System.out.print("");
                                                          }
                                                              else{
                                                          if(flag!=1){
                                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting String Literal or Character Literal. ");
                                                        flag=1;
                                                        break;
                                                          }
                                                      }
                                                      }
                                                      else{
                                                          if(flag!=1){
                                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '==' or '!='. ");
                                                        flag=1;
                                                        break;
                                                          }
                                                      }
                                                          
                                                  }
                                                  
                                                  
                                                  else if(list.get(listctr).Token.equals("!")){
                                                      stringFlag=1;
                                                   while(list.get(listctr).Token.equals("!")) {
                                                       listctr++;
                                                   } 
                                                   if(list.get(listctr).Token.equals("Identifier"))
                                                        System.out.print("");
                                                   else if(list.get(listctr).Token.equals("true")||list.get(listctr).Token.equals("false"))
                                                       System.out.print("");
                                                   else if(list.get(listctr).Token.equals("(")){
                                                       System.out.print("\n======HERE===\n");
                                                   listctr++;
                                                    int operandCtr=1;
                                                    int result4[]= operand(listctr, iden, flag, fontfamily, operandCtr);
                                                    flag=result4[0];
                                                    listctr=result4[1];
                                                   if(flag==1){
                                                     
                                                       break;}
                                                   else{
                                                       listctr++;
                                                      
                                                   }
                                                 if(list.get(listctr).Token.equals(")")){
                                                 listctr++;System.out.print("~~~JJ~~~");
                                               
                                             }
                                                 else if((list.get(listctr).Token.equals("==")||list.get(listctr).Token.equals("!=")||
                                              list.get(listctr).Token.equals("<")||list.get(listctr).Token.equals(">")
                                              ||list.get(listctr).Token.equals("<=")||list.get(listctr).Token.equals(">="))){
                                                       while(list.get(listctr).Token.equals("==")||list.get(listctr).Token.equals("!=")||
                                              list.get(listctr).Token.equals("<")||list.get(listctr).Token.equals(">")
                                              ||list.get(listctr).Token.equals("<=")||list.get(listctr).Token.equals(">=")){
                                                           listctr++;
                                                    operandCtr=0;
                                                    int result2[]= operand(listctr, iden, flag, fontfamily, operandCtr);
                                                    flag=result2[0];
                                                    listctr=result2[1];
                                                    //listctr++;
                                                       }
                                                           if(list.get(listctr).Token.equals(")")){
                                                 listctr++;
                                                 if(list.get(listctr).Token.equals(")")){
                                                 System.out.print("~~~HHMM~~~");}
                                             }
                                                          else{
                                                            if(flag!=1){
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '<', '>', '==', '<=', '>=', '!='. ");
                                                        flag=1;
                                                        break;}
                                                 }
                                                   }
                                             else{
                                               
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '<', '>', '==', '<=', '>=', '!='. ");
                                               
                                                        flag=1;
                                                        break;
                                                 }
                                                   }//if relexp or (
                                                   else{
                                                        if(flag!=1){
                                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                                           + "</span>'. Expecting 'true', 'false', Identifier or '('. ");
                                                        flag=1;
                                                        break;
                                                          }
                                                   }
                                                  }
                                                  else{
                                                  int operandCtr=1;
                                                    int result[]= operand(listctr, iden, flag, fontfamily, operandCtr);
                                                    flag=result[0];
                                                    listctr=result[1];
                                                     System.out.print("gg "+flag+" HEEgg");}
                                                   if(flag==1){
                                                      // System.out.print("OOOOOO");
                                                       break;}
                                               
                                                   else{
                                                       listctr++;
                                                   }
                                                  ///CONDITION OPERAND END///
                                             if(list.get(listctr).Token.equals(")")){
                                                 listctr++;
                                             }
                                                 else if(stringFlag!=1&&(list.get(listctr).Token.equals("==")||list.get(listctr).Token.equals("!=")||
                                              list.get(listctr).Token.equals("<")||list.get(listctr).Token.equals(">")
                                              ||list.get(listctr).Token.equals("<=")||list.get(listctr).Token.equals(">="))){
                                                       while(list.get(listctr).Token.equals("==")||list.get(listctr).Token.equals("!=")||
                                              list.get(listctr).Token.equals("<")||list.get(listctr).Token.equals(">")
                                              ||list.get(listctr).Token.equals("<=")||list.get(listctr).Token.equals(">=")){
                                                           listctr++;
                                                   int operandCtr=0;
                                                    int result2[]= operand(listctr, iden, flag, fontfamily, operandCtr);
                                                    flag=result2[0];
                                                    listctr=result2[1];
                                                    //listctr++;
                                                       }
                                                        if(list.get(listctr).Token.equals(")")){
                                                            listctr++;
                                                        }
                                                          else{
                                                            if(flag!=1){
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '<', '>', '==', '<=', '>=', '!='. ");
                                                        flag=1;
                                                        break;}
                                                 }
                                                   }
                                             else{
                                                if(stringFlag==0){
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '<', '>', '==', '<=', '>=', '!='. ");}
                                                else{
                                                      ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')'. ");
                                                }
                                                        flag=1;
                                                        break;
                                                 }
                                             
                                             
                                                 }
                                                 
                                                 else{
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '('. ");
                                                        flag=1;
                                                        break;
                                                 }
                                         }
                                                                
      ////////////////////////////END OF IF CONDITION INDIE/////////////////         
      ///////////////////////////EXPRESSION INDIE//////////////////////////
                         else if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("++")
                                 ||list.get(listctr).Token.equals("--")||list.get(listctr).Token.equals("Int Literal")
                                 ||list.get(listctr).Token.equals("Float Literal")){
                             iden="";
                           iden = iden.concat("<statements2>");
                            listSyntax.add(new Syntax("<statements4> ",iden));
                              iden="";
                            iden = iden.concat("<statements2>");
                            listSyntax.add(new Syntax("<statements4> ",iden));
                              iden="";
                            iden = iden.concat("<exp>");
                            listSyntax.add(new Syntax("<statements2> ",iden));
                          
                            if(list.get(listctr).Token.equals("Identifier")){
                                 //arithexp2///////////////////
                                 listctr++;
                                 if(list.get(listctr).Token.equals("=")){
                            iden="";
                            iden = iden.concat("<arithexp2>");
                            listSyntax.add(new Syntax("<exp> ",iden));
                             iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                             iden="";
                            iden = iden.concat("=");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                             iden="";
                            iden = iden.concat("<operand>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                            
                            listctr++;
                            
                            if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                                   int result2[] = arithexp1(listctr, iden, flag, fontfamily, arrayFlag);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;      
                                                 break;
                            }
                            if(flag==0){
                            if(list.get(listctr).Token.equals(";")){
                            iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<exp> ",iden));
                            listctr++;
                            flag=0;
                            continue;
                            }
                             else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("<arithop>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                         if(list.get(listctr).Token.equals("+")){
                            iden="";
                            iden = iden.concat("+");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("-")){
                            iden="";
                            iden = iden.concat("-");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("*")){
                            iden="";
                            iden = iden.concat("*");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("/")){
                            iden="";
                            iden = iden.concat("/");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("%");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         
                      
                            }
                              else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';', '+','-','*','/','%' ");
                                                 flag=1;      
                                                 break;
                         }
                           
                           if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            
                            ///////////////////////////////ARRAY ACCESS///////////////////////////////////////////////////
                            if(list.get(listctr).Token.equals("[")){
                            //   int arrayFlag=0;
                             int result1[] = arrayAccess(listctr, iden, flag, fontfamily, arrayFlag,dtype);
                                                         flag=result1[0];
                                                         listctr=result1[1];
                            }
 /////////////////////////////////////END OF ARRAY ACCESS/////////////////////////////
 /////////////////////STRUCT ACCESS///////////////////////////////
                            else if(list.get(listctr).Token.equals(".")){
                                int result2[] = structAccess(listctr, iden, flag, fontfamily);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                            }
                            
    ///////////////////////////////END OF STRUCT ACCESS//////////////////////////////////////////////////
                            
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            //////// PARENTHESIS ARITHEXP1 ////
                            if(list.get(listctr).Token.equals(")")){
                                iden="";
                            iden = iden.concat(")");
                            listSyntax.add(new Syntax("<operand>",iden));
                                listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")){
                                
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            
                            //loop
                           if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")|| list.get(listctr).Token.equals(")")){
                               
                               if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   //break;
                               }
                               else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                                
                             while(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")||list.get(listctr).Token.equals(")")){
                                  
                             if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   break;
                               }
                                 else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("<arithop>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                         if(list.get(listctr).Token.equals("+")){
                            iden="";
                            iden = iden.concat("+");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("-")){
                            iden="";
                            iden = iden.concat("-");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("*")){
                            iden="";
                            iden = iden.concat("*");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("/")){
                            iden="";
                            iden = iden.concat("/");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("%");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                          if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            
                                 //////// PARENTHESIS ARITHEXP1 ////
                            if(list.get(listctr).Token.equals(")")){
                                iden="";
                            iden = iden.concat(")");
                            listSyntax.add(new Syntax("<operand>",iden));
                                listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")){
                                
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            
                            //loop
                           if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")|| list.get(listctr).Token.equals(")")){
                               
                               if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   //break;
                               }
                               else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                                
                             while(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")||list.get(listctr).Token.equals(")")){
                                  
                             if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   break;
                               }
                                 else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("<arithop>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                         if(list.get(listctr).Token.equals("+")){
                            iden="";
                            iden = iden.concat("+");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("-")){
                            iden="";
                            iden = iden.concat("-");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("*")){
                            iden="";
                            iden = iden.concat("*");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("/")){
                            iden="";
                            iden = iden.concat("/");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("%");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                          if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                                     
                             if(list.get(listctr).Token.equals(")")){
                           
                            listctr++;
                            break;
                            }
                             else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%"))
                                 continue;
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')','+','-','*','/','%'. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                             
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                                        
                            }
                              else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%' ");
                                                 flag=1;      
                                                 break;
                         }
                            
                     
                                   
                                   }//WHILE 
                    
                               }//if arithexp3
                           }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%'. ");
                                                 flag=1;      
                                                 break;
                            }
                            ////////////////
                                
                            }
                            else{
                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or ')'. ");
                                                 flag=1;      
                                                 break;  
                            }
                 /////// END OF PARENTHESIS ARITHEXP1 ///
                            }
                            
                           
                             if(list.get(listctr).Token.equals(")")){
                           
                            listctr++;
                            break;
                            }
                             else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%"))
                                 continue;
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')','+','-','*','/','%'. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                             
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                                        
                            }
                              else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%' ");
                                                 flag=1;      
                                                 break;
                         }
                            
                     
                                   
                                   }//WHILE 
                    
                               }//if arithexp3
                           }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%'. ");
                                                 flag=1;      
                                                 break;
                            }
                            ////////////////
                                
                            }
                            else{
                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or ')'. ");
                                                 flag=1;      
                                                 break;  
                            }
                 /////// END OF PARENTHESIS ARITHEXP1 ///
                            }
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;      
                                                 break;
                            }
                           
                           if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")|| list.get(listctr).Token.equals(";")){
                               
                               if(list.get(listctr).Token.equals(";")){
                                   listctr++;
                                   flag=0;
                                   //break;
                               }
                               else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                                
                             while(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")||list.get(listctr).Token.equals(";")){
                                  
                             if(list.get(listctr).Token.equals(";")){
                                   listctr++;
                                   flag=0;
                                   break;
                               }
                                 else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("<arithop>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                         if(list.get(listctr).Token.equals("+")){
                            iden="";
                            iden = iden.concat("+");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("-")){
                            iden="";
                            iden = iden.concat("-");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("*")){
                            iden="";
                            iden = iden.concat("*");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("/")){
                            iden="";
                            iden = iden.concat("/");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("%");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                                        if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            //////// PARENTHESIS ARITHEXP1 ////
                            if(list.get(listctr).Token.equals(")")){
                                iden="";
                            iden = iden.concat(")");
                            listSyntax.add(new Syntax("<operand>",iden));
                                listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")){
                                
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            
                            //loop
                           if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")|| list.get(listctr).Token.equals(")")){
                               
                               if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   //break;
                               }
                               else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                                
                             while(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")||list.get(listctr).Token.equals(")")){
                                  
                             if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   break;
                               }
                                 else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("<arithop>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                         if(list.get(listctr).Token.equals("+")){
                            iden="";
                            iden = iden.concat("+");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("-")){
                            iden="";
                            iden = iden.concat("-");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("*")){
                            iden="";
                            iden = iden.concat("*");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("/")){
                            iden="";
                            iden = iden.concat("/");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("%");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                          if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            
                                 //////// PARENTHESIS ARITHEXP1 ////
                            if(list.get(listctr).Token.equals(")")){
                                iden="";
                            iden = iden.concat(")");
                            listSyntax.add(new Syntax("<operand>",iden));
                                listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")){
                                
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            
                            //loop
                           if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")|| list.get(listctr).Token.equals(")")){
                               
                               if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   //break;
                               }
                               else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                                
                             while(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")||list.get(listctr).Token.equals(")")){
                                  
                             if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   break;
                               }
                                 else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("<arithop>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                         if(list.get(listctr).Token.equals("+")){
                            iden="";
                            iden = iden.concat("+");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("-")){
                            iden="";
                            iden = iden.concat("-");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("*")){
                            iden="";
                            iden = iden.concat("*");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("/")){
                            iden="";
                            iden = iden.concat("/");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("%");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                          if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                                     
                             if(list.get(listctr).Token.equals(")")){
                           
                            listctr++;
                            break;
                            }
                             else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%"))
                                 continue;
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')','+','-','*','/','%'. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                             
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                                        
                            }
                              else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%' ");
                                                 flag=1;      
                                                 break;
                         }
                            
                     
                                   
                                   }//WHILE 
                    
                               }//if arithexp3
                           }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%'. ");
                                                 flag=1;      
                                                 break;
                            }
                            ////////////////
                                
                            }
                            else{
                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or ')'. ");
                                                 flag=1;      
                                                 break;  
                            }
                 /////// END OF PARENTHESIS ARITHEXP1 ///
                            }
                            
                           
                             if(list.get(listctr).Token.equals(")")){
                           
                            listctr++;
                            break;
                            }
                             else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%"))
                                 continue;
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')','+','-','*','/','%'. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                             
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                                        
                            }
                              else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%' ");
                                                 flag=1;      
                                                 break;
                         }
                            
                     
                                   
                                   }//WHILE 
                    
                               }//if arithexp3
                           }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%'. ");
                                                 flag=1;      
                                                 break;
                            }
                            ////////////////
                                
                            }
                            else{
                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or ')'. ");
                                                 flag=1;      
                                                 break;  
                            }
                 /////// END OF PARENTHESIS ARITHEXP1 ///
                            }
                                     
                             if(list.get(listctr).Token.equals(";")){
                           
                            listctr++;
                            break;
                            }
                             else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%"))
                                 continue;
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';','+','-','*','/','%'. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                             
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                                        
                            }
                              else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';', '+','-','*','/','%' ");
                                                 flag=1;      
                                                 break;
                         }
                            
                     
                                   
                                   }//WHILE 
                    
                               }//if arithexp3
                           }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';', '+','-','*','/','%'. ");
                                                 flag=1;      
                                                 break;
                            }
                                 }
                            
                          }
                 //======iterate=============//
                    else if(list.get(listctr).Token.equals("++")||list.get(listctr).Token.equals("--")){
                           if(list.get(listctr).Token.equals("++")){
                            iden="";
                            iden = iden.concat("<iterate>");
                            listSyntax.add(new Syntax("<exp> ",iden));
                             iden="";
                            iden = iden.concat("<unexp>");
                            listSyntax.add(new Syntax("<iterate> ",iden));
                              iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<unexp> ",iden));
                            iden="";
                            iden = iden.concat("<unop>");
                            listSyntax.add(new Syntax("<unexp> ",iden));
                            iden="";
                            iden = iden.concat("++");
                            listSyntax.add(new Syntax("<unop> ",iden));
                            listctr++;
                            }
                        else if(list.get(listctr).Token.equals("--")){
                            iden="";
                            iden = iden.concat("<iterate>");
                            listSyntax.add(new Syntax("<exp> ",iden));
                             iden="";
                            iden = iden.concat("<unexp>");
                            listSyntax.add(new Syntax("<iterate> ",iden));
                              iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<unexp> ",iden));
                            iden="";
                            iden = iden.concat("<unop>");
                            listSyntax.add(new Syntax("<unexp> ",iden));
                            iden="";
                            iden = iden.concat("--");
                            listSyntax.add(new Syntax("<unop> ",iden));
                            listctr++;
                            }
                           if(list.get(listctr).Token.equals(";")){
                            iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<exp> ",iden));
                           listctr++;
                           flag=0;
                           }
                           
                           else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';'. ");
                                                 flag=1;      
                                                 break;
                                 }
                    
                    }
                     //=======end of iterate====================================================//
                    ///ASSIGNMENT EXPRESSION//////////////////////////////////////////////
                    
                                else if(list.get(listctr).Token.equals("+=")||list.get(listctr).Token.equals("-=")
                                       ||list.get(listctr).Token.equals("/=")||list.get(listctr).Token.equals("*=")
                                       ||list.get(listctr).Token.equals("%=")){
                           if(list.get(listctr).Token.equals("+=")){
                            iden="";
                            iden = iden.concat("<assignexp>");
                            listSyntax.add(new Syntax("<exp> ",iden));
                             iden="";
                            iden = iden.concat("<assignexp2>");
                            listSyntax.add(new Syntax("<assignexp> ",iden));
                              iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("<assignop2>");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("+=");
                            listSyntax.add(new Syntax("<assignop2> ",iden));
                            listctr++;
                            }
                           else if(list.get(listctr).Token.equals("-=")){
                            iden="";
                            iden = iden.concat("<assignexp>");
                            listSyntax.add(new Syntax("<exp> ",iden));
                             iden="";
                            iden = iden.concat("<assignexp2>");
                            listSyntax.add(new Syntax("<assignexp> ",iden));
                              iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("<assignop2>");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("-=");
                            listSyntax.add(new Syntax("<assignop2> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("*=")){
                            iden="";
                            iden = iden.concat("<assignexp>");
                            listSyntax.add(new Syntax("<exp> ",iden));
                             iden="";
                            iden = iden.concat("<assignexp2>");
                            listSyntax.add(new Syntax("<assignexp> ",iden));
                              iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("<assignop2>");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("*=");
                            listSyntax.add(new Syntax("<assignop2> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("/=")){
                            iden="";
                            iden = iden.concat("<assignexp>");
                            listSyntax.add(new Syntax("<exp> ",iden));
                             iden="";
                            iden = iden.concat("<assignexp2>");
                            listSyntax.add(new Syntax("<assignexp> ",iden));
                              iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("<assignop2>");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("/=");
                            listSyntax.add(new Syntax("<assignop2> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("%=")){
                            iden="";
                            iden = iden.concat("<assignexp>");
                            listSyntax.add(new Syntax("<exp> ",iden));
                             iden="";
                            iden = iden.concat("<assignexp2>");
                            listSyntax.add(new Syntax("<assignexp> ",iden));
                              iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("<assignop2>");
                            listSyntax.add(new Syntax("<assignexp2> ",iden));
                            iden="";
                            iden = iden.concat("%=");
                            listSyntax.add(new Syntax("<assignop2> ",iden));
                            listctr++;
                            }
                           if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                        
                            
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand>",iden));
                            listctr++;
                            
                            
                             //////// PARENTHESIS ARITHEXP1 ////
                            if(list.get(listctr).Token.equals(")")){
                                iden="";
                            iden = iden.concat(")");
                            listSyntax.add(new Syntax("<operand>",iden));
                                listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")){
                                
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            
                      
                           //loop
                             if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")|| list.get(listctr).Token.equals(")")){
                               
                               if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   //break;
                               }
                               else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                                
                             while(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")||list.get(listctr).Token.equals(")")){
                                  
                             if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   break;
                               }
                                 else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("<arithop>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                         if(list.get(listctr).Token.equals("+")){
                            iden="";
                            iden = iden.concat("+");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("-")){
                            iden="";
                            iden = iden.concat("-");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("*")){
                            iden="";
                            iden = iden.concat("*");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("/")){
                            iden="";
                            iden = iden.concat("/");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("%");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                          if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            
                                 //////// PARENTHESIS ARITHEXP1 ////
                            if(list.get(listctr).Token.equals(")")){
                                iden="";
                            iden = iden.concat(")");
                            listSyntax.add(new Syntax("<operand>",iden));
                                listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")){
                                
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            
                            //loop
                           if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")|| list.get(listctr).Token.equals(")")){
                               
                               if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   //break;
                               }
                               else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                                
                             while(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")||list.get(listctr).Token.equals(")")){
                                  
                             if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   break;
                               }
                                 else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("<arithop>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                         if(list.get(listctr).Token.equals("+")){
                            iden="";
                            iden = iden.concat("+");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("-")){
                            iden="";
                            iden = iden.concat("-");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("*")){
                            iden="";
                            iden = iden.concat("*");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("/")){
                            iden="";
                            iden = iden.concat("/");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("%");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                          if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                                     
                             if(list.get(listctr).Token.equals(")")){
                           
                            listctr++;
                            break;
                            }
                             else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%"))
                                 continue;
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')','+','-','*','/','%'. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                             
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                                        
                            }
                              else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%' ");
                                                 flag=1;      
                                                 break;
                         }
                            
                     
                                   
                                   }//WHILE 
                    
                               }//if arithexp3
                           }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%'. ");
                                                 flag=1;      
                                                 break;
                            }
                            ////////////////
                                
                            }
                            else{
                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or ')'. ");
                                                 flag=1;      
                                                 break;  
                            }
                 /////// END OF PARENTHESIS ARITHEXP1 ///
                            }
                            
                           
                             if(list.get(listctr).Token.equals(")")){
                           
                            listctr++;
                            break;
                            }
                             else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%"))
                                 continue;
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')','+','-','*','/','%'. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                             
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                                        
                            }
                              else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%' ");
                                                 flag=1;      
                                                 break;
                         }
                            
                     
                                   
                                   }//WHILE 
                    
                               }//if arithexp3
                           }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%', ';'. ");
                                                 flag=1;      
                                                 break;
                            }
                            ////////////////
                                
                            }
                            else{
                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or ')'. ");
                                                 flag=1;      
                                                 break;  
                            }
                 /////// END OF PARENTHESIS ARITHEXP1 ///
                            
                            
                            }
                            
                            if(list.get(listctr).Token.equals(";")){
                            iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<exp> ",iden));
                            listctr++;
                            flag=0;
                            continue;
                            }
                            else{
                                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';'. ");
                                                 flag=1;      
                                                 break; 
                            }
                            
                           }
                           
                           else{
                                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, Integer Literal, Float Literal, or '('. ");
                                                 flag=1;      
                                                 break;
                           }
                                }
                 ///ASSIGNMENT EXPRESSION//////////////////////////////////////////////
               
                             //ELSE ON IDENTIFIER
                                 else{
                                    if(flag!=1){
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '+=', '-=', '*=', '/=', '%=', '=', '++' or '--'. ");
                                                 flag=1;      
                                                 break;}
                                 }
                            
                            }
                                //======iterate=============//
                    else if(list.get(listctr).Token.equals("++")||list.get(listctr).Token.equals("--")){
                                 iden="";
                           iden = iden.concat("<statements2>");
                            listSyntax.add(new Syntax("<statements4> ",iden));
                              iden="";
                            iden = iden.concat("<statements2>");
                            listSyntax.add(new Syntax("<statements4> ",iden));
                              iden="";
                            iden = iden.concat("<exp>");
                            listSyntax.add(new Syntax("<statements2> ",iden));
                            iden="";
                            iden = iden.concat("<iterate>");
                            listSyntax.add(new Syntax("<exp> ",iden));
                             iden="";
                            iden = iden.concat("<unexp>");
                            listSyntax.add(new Syntax("<iterate> ",iden));
                            iden="";
                            iden = iden.concat("<unop>");
                            listSyntax.add(new Syntax("<unexp> ",iden));
                            
                           if(list.get(listctr).Token.equals("++")){
                             iden="";
                            iden = iden.concat("++");
                            listSyntax.add(new Syntax("<unop> ",iden));
                            listctr++;
                            }
                        else if(list.get(listctr).Token.equals("--")){
                             iden="";
                            iden = iden.concat("--");
                            listSyntax.add(new Syntax("<unop> ",iden));
                            listctr++;
                            }
                           if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<unexp> ",iden));
                           listctr++;
                           //flag=0;
                           
                            if(list.get(listctr).Token.equals(";")){
                            iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<exp> ",iden));
                           listctr++;
                           flag=0;
                           }
                           
                           else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';'. ");
                                                 flag=1;      
                                                 break;
                                 }
                           }
                           
                           else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier. ");
                                                 flag=1;      
                                                 break;
                                 }
                    
                    }
                //=======end of iterate====================================================//
                            else{
                                listctr++;
                                flag=0;
                            }
                         }
                         else if(flag==1){
                             break;
                         }
      ///////////////////////////////END OF EXPRESSION INDIE///////////////////////
        ///////////////////////////////////////////////END OF STATEMENTS2/////////////////////////////////
               //////////////////////////END OF STATEMENTS IN BODY//////////////////////////////////////////////////////////////////////////////////////////////
                                                             else{
                             if(flag!=1){
                                                 ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting 'fallin', 'report', 'execute', 'Identifier', 'if', 'for', 'do', '++', '--', '}'. ");
                                                 flag=1;      
                                                 break;}
                                                }//else NOT statement >>startmission:
                                      }//WHILE LOOP COMMAND INDIE
        } catch (BadLocationException ex) {
            Logger.getLogger(Frame.class.getName()).log(Level.SEVERE, null, ex);
        }
         return new int[] {flag, listctr, braceFlag};
    }
     public int[] arrayAccess(int listctr, String iden, int flag, String fontfamily, int arrayFlag, String dtype){
        try {
            int twoDimenFlag =0, temp;
            temp = arrayFlag;
           
             ArrayList<Lexical> list = ListLexical();
            ArrayList<Syntax> listSyntax = new ArrayList<Syntax>();
               iden="";
                            iden = iden.concat("[");
                            listSyntax.add(new Syntax("<arraccess> ",iden));
                            listctr++;
                             if(list.get(listctr).Token.equals("Int Literal")||list.get(listctr).Token.equals("Identifier")){
                             
                            iden="";
                            iden = iden.concat("<arraysize>");
                            listSyntax.add(new Syntax("<arraccess> ",iden));
                            
                                if((list.get(listctr).Token.equals("Identifier"))&&arrayFlag==0){
                                    listctr++;
                                if(list.get(listctr).Token.equals("[")){
                                    arrayFlag=0;
                                //    listctr++;
                               
                                int result1[] = arrayAccess(listctr, iden, flag, fontfamily, arrayFlag,dtype);
                                                         flag=result1[0];
                                                         listctr=result1[1];
                                    
                                    //listctr++;
                                  arrayFlag=temp;
                                }
                                else if(list.get(listctr).Token.equals(".")){
                                    //listctr++;
                                     System.out.print("befor:"+listctr+" ");  int result2[] = structAccess(listctr, iden, flag, fontfamily);
                                                         flag=result2[0];
                                                         listctr=result2[1];System.out.print("arrF:"+listctr+" ");
                                 //   listctr++;
                                }
                            }
                                else{
                            listctr++;}
                            
                            if(list.get(listctr).Token.equals("]")){
                             iden="";
                            iden = iden.concat("]");
                            listSyntax.add(new Syntax("<arraccess> ",iden));
                            listctr++;
                         ///////////////////////////TWO DIMEN//////////   
                         if(list.get(listctr).Token.equals("[")){
                             twoDimenFlag=1;
                             iden="";
                            iden = iden.concat("[");
                            listSyntax.add(new Syntax("<arraccess> ",iden));
                            listctr++;
                             if(list.get(listctr).Token.equals("Int Literal")||list.get(listctr).Token.equals("Identifier")){
                             iden="";
                            iden = iden.concat("<arraysize>");
                            listSyntax.add(new Syntax("<arraccess> ",iden));
                            if(list.get(listctr).Token.equals("Identifier")&&arrayFlag==0){
                                                   listctr++;
                                if(list.get(listctr).Token.equals("[")){
                                    arrayFlag=0;
                                //    listctr++;
                                System.out.print("befor:"+listctr+" "); 
                                int result1[] = arrayAccess(listctr, iden, flag, fontfamily, arrayFlag,dtype);
                                                         flag=result1[0];
                                                         listctr=result1[1];
                                    System.out.print("arrF:"+listctr+" ");
                                    //listctr++;
                                  arrayFlag=temp;
                                }
                                else if(list.get(listctr).Token.equals(".")){
                                    //listctr++;
                                      int result2[] = structAccess(listctr, iden, flag, fontfamily);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                                   // listctr++;
                                }
                            }
                            else{
                            listctr++;
                            }
                            
                            if(list.get(listctr).Token.equals("]")){
                             iden="";
                            iden = iden.concat("]");
                            listSyntax.add(new Syntax("<arraccess> ",iden));
                            listctr++;
                            }
                            else if(flag!=1){
                                  ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ']'. ");
                                                        flag=1;
//                                                        break;
                             }
                            
                            }
                             else{
                                  ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Int Literal or Identifier. ");
                                                        flag=1;
                                                       // break;
                             }
                            }
                 ////////////////////////TWO DIMEN///////////////////////////////
                            }
                             else{
                                  ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ']'. ");
                                                        flag=1;
                                                   //     break;
                             }
                            
                            }
                             else{
                                  ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier or Array Size. ");
                                                        flag=1;
                                                      //  break;
                             }
                            arrayFlag = temp;
                             if(arrayFlag==1&&flag!=1&&twoDimenFlag==1)  {
                             if(list.get(listctr).Token.equals(";")){
                                        listctr++;
                                    }
                                   else if(list.get(listctr).Token.equals(",")){
                                  arrayFlag=0;
                              int result2[] = comma(listctr, iden, flag, fontfamily, arrayFlag);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                                    }
                                
                                   else{
                                 ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';', '=', or ','. ");
                                flag=1;
                                // break;
                                    }
                             }
                               else if(arrayFlag==1&&flag!=1&&twoDimenFlag==0)  {
                             if(list.get(listctr).Token.equals(";")){
                                        listctr++;
                                    }
                             else if(list.get(listctr).Token.equals(",")){
                                  arrayFlag=0;
                              int result2[] = comma(listctr, iden, flag, fontfamily, arrayFlag);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                                    }
                             else if(list.get(listctr).Token.equals("=")){
                                        listctr++;
                                   if(list.get(listctr).Token.equals("{")){
                                       listctr++;
                                       int intlitflag=0;
                                        if(dtype.equals("int")&&list.get(listctr).Token.equals("Int Literal")){
                                            listctr++;
                                            intlitflag =1;
                                            if(list.get(listctr).Token.equals(",")){
                                                while(list.get(listctr).Token.equals(",")){
                                                    listctr++;
                                                    if(list.get(listctr).Token.equals("Int Literal"))
                                                        listctr++;
                                                    else {
                                                        if(flag!=1){
                                                    ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                                + "</span>'. Expecting "+dtype+" literal. ");
                                                   flag=1;
                                                   break;
                                                        }
                                                       }
                                                }
                                                if(list.get(listctr).Token.equals("}"))
                                                 listctr++;
                                                 else{
                                                    if(flag!=1){
                                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                        + "</span>'. Expecting '}' or ','. ");
                                           flag=1;}    }
                                                
                                            } //if comma
                                            else if(list.get(listctr).Token.equals("}")){
                                                 listctr++;
                                           
                                            }//else if }
                                            
                                                   else{
                                            if(flag!=1){
                                                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                            + "</span>'. Expecting ',', or '}'. ");
                                               flag=1;}
                                               // break;
                                                   }
                                               
                                        }
                                        else if(list.get(listctr).Token.equals("(")){
                                            while(list.get(listctr).Token.equals("(")){
                                            intlitflag=0;
                                             listctr++;
                                            if(dtype.equals("int")&&list.get(listctr).Token.equals("Int Literal")){
                                            listctr++;
                                              if(list.get(listctr).Token.equals(",")){
                                                while(list.get(listctr).Token.equals(",")){
                                                    listctr++;
                                                    if(list.get(listctr).Token.equals("Int Literal"))
                                                        listctr++;
                                                    else {
                                                        if(flag!=1){
                                                    ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                                + "</span>'. Expecting "+dtype+" literal. ");
                                                   flag=1;
                                                   break;
                                                        }
                                                       }
                                                }
                                                if(list.get(listctr).Token.equals(")"))
                                                 listctr++;
                                                 else{
                                                    if(flag!=1){
                                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                        + "</span>'. Expecting ')' or ','. ");
                                           flag=1;}    }
                                                
                                            } //if comma
                                           else if(list.get(listctr).Token.equals(")")){
                                                 listctr++;
                                           
                                            }//else if )
                                            
                                                   else{
                                            if(flag!=1){
                                                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                            + "</span>'. Expecting ',', or ')'. ");
                                               flag=1;}
                                               // break;
                                                   }
                                            }
////////////////////////////////////////////////////////////////////////////////////////FLOAT LIT//////////////////////////////////////////
                                            else if(dtype.equals("flt")&&list.get(listctr).Token.equals("Float Literal")){
                                            listctr++;
                                            intlitflag =1;
                                            if(list.get(listctr).Token.equals(",")){
                                                while(list.get(listctr).Token.equals(",")){
                                                    listctr++;
                                                    if(list.get(listctr).Token.equals("Float Literal"))
                                                        listctr++;
                                                    else {
                                                        if(flag!=1){
                                                    ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                                + "</span>'. Expecting "+dtype+" literal. ");
                                                   flag=1;
                                                   break;
                                                        }
                                                       }
                                                }
                                                if(list.get(listctr).Token.equals("}"))
                                                 listctr++;
                                                 else{
                                                    if(flag!=1){
                                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                        + "</span>'. Expecting '}' or ','. ");
                                           flag=1;}    }
                                                
                                            } //if comma
                                            else if(list.get(listctr).Token.equals("}")){
                                                 listctr++;
                                           
                                            }//else if }
                                            
                                                   else{
                                            if(flag!=1){
                                                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                            + "</span>'. Expecting ',', or '}'. ");
                                               flag=1;}
                                               // break;
                                                   }
                                               
                                        }
////////////////////////////////////////////////////////////////////////////////FLOAT LIT////////////////
/////////////////////////////////////////////////////////////////////////////////CHAR LIT//////////////////////////////////
                                       else if(dtype.equals("chr")&&list.get(listctr).Token.equals("Character Literal")){
                                            listctr++;
                                            intlitflag =1;
                                            if(list.get(listctr).Token.equals(",")){
                                                while(list.get(listctr).Token.equals(",")){
                                                    listctr++;
                                                    if(list.get(listctr).Token.equals("Character Literal"))
                                                        listctr++;
                                                    else {
                                                        if(flag!=1){
                                                    ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                                + "</span>'. Expecting "+dtype+" literal. ");
                                                   flag=1;
                                                   break;
                                                        }
                                                       }
                                                }
                                                if(list.get(listctr).Token.equals("}"))
                                                 listctr++;
                                                 else{
                                                    if(flag!=1){
                                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                        + "</span>'. Expecting '}' or ','. ");
                                           flag=1;}    }
                                                
                                            } //if comma
                                            else if(list.get(listctr).Token.equals("}")){
                                                 listctr++;
                                           
                                            }//else if }
                                            
                                                   else{
                                            if(flag!=1){
                                                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                            + "</span>'. Expecting ',', or '}'. ");
                                               flag=1;}
                                               // break;
                                                   }
                                               
                                        }
/////////////////////////////////////////////////////////////////////////////////CHAR LIT//////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////STRING LIT//////////////////////////////////////
                                           else if(dtype.equals("str")&&list.get(listctr).Token.equals("String Literal")){
                                            listctr++;
                                            intlitflag =1;
                                            if(list.get(listctr).Token.equals(",")){
                                                while(list.get(listctr).Token.equals(",")){
                                                    listctr++;
                                                    if(list.get(listctr).Token.equals("String Literal"))
                                                        listctr++;
                                                    else {
                                                        if(flag!=1){
                                                    ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                                + "</span>'. Expecting "+dtype+" literal. ");
                                                   flag=1;
                                                   break;
                                                        }
                                                       }
                                                }
                                                if(list.get(listctr).Token.equals("}"))
                                                 listctr++;
                                                 else{
                                                    if(flag!=1){
                                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                        + "</span>'. Expecting '}' or ','. ");
                                           flag=1;}    }
                                                
                                            } //if comma
                                            else if(list.get(listctr).Token.equals("}")){
                                                 listctr++;
                                           
                                            }//else if }
                                            
                                                   else{
                                            if(flag!=1){
                                                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                            + "</span>'. Expecting ',', or '}'. ");
                                               flag=1;}
                                               // break;
                                                   }
                                               
                                        }
////////////////////////////////////////////////////////////////////////////////STRING LIT////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////BOOL LIT//////////////////////////////////////////
                                            else if(dtype.equals("bool")&&list.get(listctr).Token.equals("true")||dtype.equals("bool")&&list.get(listctr).Token.equals("false")){
                                            listctr++;
                                            intlitflag =1;
                                            if(list.get(listctr).Token.equals(",")){
                                                while(list.get(listctr).Token.equals(",")){
                                                    listctr++;
                                                    if(list.get(listctr).Token.equals("true")||list.get(listctr).Token.equals("false"))
                                                        listctr++;
                                                    else {
                                                        if(flag!=1){
                                                    ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                                + "</span>'. Expecting "+dtype+" literal. ");
                                                   flag=1;
                                                   break;
                                                        }
                                                       }
                                                }
                                                if(list.get(listctr).Token.equals("}"))
                                                 listctr++;
                                                 else{
                                                    if(flag!=1){
                                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                        + "</span>'. Expecting '}' or ','. ");
                                           flag=1;}    }
                                                
                                            } //if comma
                                            else if(list.get(listctr).Token.equals("}")){
                                                 listctr++;
                                           
                                            }//else if }
                                            
                                                   else{
                                            if(flag!=1){
                                                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                            + "</span>'. Expecting ',', or '}'. ");
                                               flag=1;}
                                               // break;
                                                   }
                                               
                                        }
//////////////////////////////////////////////////////////////////////////////BOOL LIT///////////////////////////////////////////////
                                             else{
                                            if(flag!=1){
                                                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                            + "</span>'. Expecting "+dtype+" literal. ");
                                               flag=1;}
                                               // break;
                                                   }
                                            if(list.get(listctr).Token.equals(",")){
                                                listctr++;
                                                if(list.get(listctr).Token.equals("("))
                                                System.out.print("");
                                                else{if(flag!=1){
                                                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                            + "</span>'. Expecting '('. ");
                                               flag=1;}
                                                    
                                                }
                                            }
                                            else if(list.get(listctr).Token.equals("}")){listctr++;
                                                break;}
                                             else{if(flag!=1){
                                                ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                            + "</span>'. Expecting ',', or '}'. ");
                                               flag=1;
                                               // break;
                                                   }}
                                            
                                        }//while (
                                        }
                                         else{
                                            if(flag!=1&&intlitflag==1){
                                 ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting "+dtype+" literal. ");
                                flag=1;}
                                            else if(flag!=1&&intlitflag==0){
                                 ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting "+dtype+" literal or '('. ");
                                flag=1;}
                                //break;
                                    }
                                       if(list.get(listctr).Token.equals(";")&&flag!=1){listctr++;}   
                                         else{
                                        if(flag!=1){    ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                                        + "</span>'. Expecting ';'. ");
                                           flag=1;}}
                                    }//if {
                                    else{if(flag!=1){
                                 ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '{'. ");
                                flag=1;
                                // break;
                                    }}
                             }
                                   else{if(flag!=1){
                                 ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ';', ',', '=', or '['");
                                flag=1;
                                // break;
                                    }}
                             }
            } catch (BadLocationException ex) {
            Logger.getLogger(Frame.class.getName()).log(Level.SEVERE, null, ex);
        }
        return new int[] {flag, listctr};
    }
    
     public int[] comma(int listctr, String iden, int flag, String fontfamily, int arrayFlag){
        try {
             ArrayList<Lexical> list = ListLexical();
            ArrayList<Syntax> listSyntax = new ArrayList<Syntax>();
                         while(list.get(listctr).Token.equals(",")){
                                    listctr++;
                                    
                                    iden = iden.concat("<vardec>");
                            listSyntax.add(new Syntax("<gdeclr> ",iden));
                            
                                   iden="";
                            iden = iden.concat("<vdtype>");
                            listSyntax.add(new Syntax("<vardec> ",iden));
                                       if(list.get(listctr).Token.equals("Identifier")){
                                        listctr++;
                                        if(list.get(listctr).Token.equals("=")){
                                    listctr++;
                                    if(list.get(listctr).Token=="true"||list.get(listctr).Token=="false"||list.get(listctr).Token=="Int Literal"||list.get(listctr).Token=="Float Literal"||list.get(listctr).Token.equals("(")||list.get(listctr).Token.equals("String Literal")||list.get(listctr).Token.equals("Character Literal")){
                                        if(dtype=="int"&&list.get(listctr).Token=="Int Literal"){
                                           
                                            listctr++;
                                        }//if int lit
                            ///////BOOKMARK//////////////
                           else if(dtype=="flt"&&list.get(listctr).Token=="Float Literal"){
                                           
                                            listctr++;
                                        }//if int lit
                                else if(dtype=="chr"&&list.get(listctr).Token=="Character Literal"){
                                           
                                            listctr++;
                                        }//if int lit
                                 else if(dtype=="str"&&list.get(listctr).Token=="String Literal"){
                                           
                                            listctr++;
                                        }//if int lit
                                else if((dtype=="bool"&&list.get(listctr).Token=="true")||(dtype=="bool"&&list.get(listctr).Token=="false")){
                                           
                                            listctr++;
                                        }//if int lit
                            
                                    else{
                                          ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '"+dtype+" literal'. ");
                                          flag=1;
                                break;
                                    }//else
                                    }//if ALL literal
                                    else{
                                          ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '"+dtype+" literal'. ");
                                          flag=1;
                                break;
                                    }//else
                                    if(list.get(listctr).Token.equals(";")||list.get(listctr).Token.equals(" ;")){
                                        iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<vardec> ",iden));
                                        listctr++;
                                      //    System.out.print("Hmm4: "+list.get(listctr).Token+" ");
                                      break;
                                        }
                                        else if(list.get(listctr).Token.equals(",")){
                                        continue;
                                        }
                                        ////////////////////////FUNCTION DECLARATION///////////////////////////////////////////
                                        else if(list.get(listctr).Token.equals("(")&&dtype=="int"){
                                            iden="";
                            iden = iden.concat("<funcdec>");
                            listSyntax.add(new Syntax("<gdeclr> ",iden));
                            iden="";
                            iden = iden.concat("<rtype>");
                            listSyntax.add(new Syntax("<funcdec> ",iden));
                            iden="";
                            iden = iden.concat("int");
                            listSyntax.add(new Syntax("<rtype> ",iden));
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<funcdec> ",iden));
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<funcdec> ",iden));
                                        listctr++;
                                              //  System.out.print("HERE PO");
                                        }
                 /////////////////////////////////////////END OF FUNCTION DECLARATION//////////////////////////////////
                                        else{
                                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ',' or ';'.");  
                                           
                                            flag=1;
                                       break; 
                                        }
                                        }//if =
                 ///////////////////////////FUNCTION DECLARATION///////////////////////////////////////////
                                        else if(list.get(listctr).Token.equals("(")&&list.get(listctr-2).Token=="int"){
                                                                   iden="";
                            iden = iden.concat("<funcdec>");
                            listSyntax.add(new Syntax("<gdeclr> ",iden));
                            iden="";
                            iden = iden.concat("<rtype>");
                            listSyntax.add(new Syntax("<funcdec> ",iden));
                            iden="";
                            iden = iden.concat("int");
                            listSyntax.add(new Syntax("<rtype> ",iden));
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<funcdec> ",iden));
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<funcdec> ",iden));
                                        listctr++;
                                              //  System.out.print("HERE PO");
                                        }
                 /////////////////////////////////////////END OF FUNCTION DECLARATION//////////////////////////////////
////////////////////////////////////////ARRAY DECLARATION//////////////////////////////////////////////////////////////////
                                else if(list.get(listctr).Token.equals("[")){
                                    arrayFlag=1;
                             int result1[] = arrayAccess(listctr, iden, flag, fontfamily, arrayFlag,dtype);
                                                         flag=result1[0];
                                                         listctr=result1[1];
                           arrayFlag=0;
                           
                                }

//////////////////////////////END OF ARRAY DECLARATION//////////////////////////////////////////////////////////////////////  
                                        else if(list.get(listctr).Token.equals(";")||list.get(listctr).Token.equals(" ;")){
                                            iden="";
                            iden = iden.concat(";");
                            listSyntax.add(new Syntax("<vardec> ",iden));
                                        listctr++;
                                          //System.out.print("Hmm4: "+list.get(listctr).Token+" ");
                                          break;
                                        }
                                        else if(list.get(listctr).Token.equals(",")){
                                        continue;
                                        }
                                        else{
                                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '=', ',' or ';'. "); 
                                            flag=1;
                                       break; 
                                        }
                                        
                                    }//if
                                   
                                    else{
                                          ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier. ");
                                          flag=1;
                                break;
                                    }//else
                                }//while ,
        } catch (BadLocationException ex) {
            Logger.getLogger(Frame.class.getName()).log(Level.SEVERE, null, ex);
        }
        return new int[] {flag, listctr};
    }
    
      public int[] arithexp1(int listctr, String iden, int flag, String fontfamily, int arrayFlag){
        try {
             ArrayList<Lexical> list = ListLexical();
            ArrayList<Syntax> listSyntax = new ArrayList<Syntax>();
            
                  if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            ///////////////////////////////ARRAY ACCESS///////////////////////////////////////////////////
                            if(list.get(listctr).Token.equals("[")){
                           // int arrayFlag=0;
                            int result1[] = arrayAccess(listctr, iden, flag, fontfamily, arrayFlag,dtype);
                                                         flag=result1[0];
                                                         listctr=result1[1];
                          
                            }
 /////////////////////////////////////END OF ARRAY ACCESS/////////////////////////////
 /////////////////////STRUCT ACCESS///////////////////////////////
                            else if(list.get(listctr).Token.equals(".")){
                              int result2[] = structAccess(listctr, iden, flag, fontfamily);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                            }
                            
    ///////////////////////////////END OF STRUCT ACCESS//////////////////////////////////////////////////
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                        
                            
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand>",iden));
                            listctr++;
                 //////// PARENTHESIS ARITHEXP1 ////
                            if(list.get(listctr).Token.equals(")")){
                                iden="";
                            iden = iden.concat(")");
                            listSyntax.add(new Syntax("<operand>",iden));
                                listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")){
                                
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            ///////////////////////////////ARRAY ACCESS///////////////////////////////////////////////////
                            if(list.get(listctr).Token.equals("[")){
                               //  int arrayFlag=0; 
                               int result1[] = arrayAccess(listctr, iden, flag, fontfamily, arrayFlag,dtype);
                                                         flag=result1[0];
                                                         listctr=result1[1];
                            }
 /////////////////////////////////////END OF ARRAY ACCESS/////////////////////////////
 /////////////////////STRUCT ACCESS///////////////////////////////
                            else if(list.get(listctr).Token.equals(".")){
                               int result2[] = structAccess(listctr, iden, flag, fontfamily);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                            }
                            
    ///////////////////////////////END OF STRUCT ACCESS//////////////////////////////////////////////////
                            
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            
                      
                           //loop
                             if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")|| list.get(listctr).Token.equals(")")){
                               
                               if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   //break;
                               }
                               else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                                
                             while(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")||list.get(listctr).Token.equals(")")){
                                  
                             if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   break;
                               }
                                 else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("<arithop>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                         if(list.get(listctr).Token.equals("+")){
                            iden="";
                            iden = iden.concat("+");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("-")){
                            iden="";
                            iden = iden.concat("-");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("*")){
                            iden="";
                            iden = iden.concat("*");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("/")){
                            iden="";
                            iden = iden.concat("/");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("%");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                          if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            ///////////////////////////////ARRAY ACCESS///////////////////////////////////////////////////
                            if(list.get(listctr).Token.equals("[")){
                             //    int arrayFlag=0;
                         int result1[] = arrayAccess(listctr, iden, flag, fontfamily, arrayFlag,dtype);
                                                         flag=result1[0];
                                                         listctr=result1[1];
                            }
 /////////////////////////////////////END OF ARRAY ACCESS/////////////////////////////
 /////////////////////STRUCT ACCESS///////////////////////////////
                            else if(list.get(listctr).Token.equals(".")){
                               int result2[] = structAccess(listctr, iden, flag, fontfamily);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                            }
                            
    ///////////////////////////////END OF STRUCT ACCESS//////////////////////////////////////////////////
                            
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            
                                 //////// PARENTHESIS ARITHEXP1 ////
                            if(list.get(listctr).Token.equals(")")){
                                iden="";
                            iden = iden.concat(")");
                            listSyntax.add(new Syntax("<operand>",iden));
                                listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")){
                                
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            
                            ///////////////////////////////ARRAY ACCESS///////////////////////////////////////////////////
                            if(list.get(listctr).Token.equals("[")){
                             //   int arrayFlag=0;
                          int result1[] = arrayAccess(listctr, iden, flag, fontfamily, arrayFlag,dtype);
                                                         flag=result1[0];
                                                         listctr=result1[1];
                            }
 /////////////////////////////////////END OF ARRAY ACCESS/////////////////////////////
 /////////////////////STRUCT ACCESS///////////////////////////////
                            else if(list.get(listctr).Token.equals(".")){
                                int result2[] = structAccess(listctr, iden, flag, fontfamily);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                            }
                            
    ///////////////////////////////END OF STRUCT ACCESS//////////////////////////////////////////////////
                            
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            
                            //loop
                           if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")|| list.get(listctr).Token.equals(")")){
                               
                               if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   //break;
                               }
                               else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                                
                             while(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")||list.get(listctr).Token.equals(")")){
                                  
                             if(list.get(listctr).Token.equals(")")){
                                   listctr++;
                                   flag=0;
                                   break;
                               }
                                 else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("<arithop>");
                            listSyntax.add(new Syntax("<arithexp2> ",iden));
                         if(list.get(listctr).Token.equals("+")){
                            iden="";
                            iden = iden.concat("+");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("-")){
                            iden="";
                            iden = iden.concat("-");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("*")){
                            iden="";
                            iden = iden.concat("*");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("/")){
                            iden="";
                            iden = iden.concat("/");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                         else if(list.get(listctr).Token.equals("%")){
                            iden="";
                            iden = iden.concat("%");
                            listSyntax.add(new Syntax("<arithop> ",iden));
                            listctr++;
                            }
                          if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")){
                            if(list.get(listctr).Token.equals("Identifier")){
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            
                            ///////////////////////////////ARRAY ACCESS///////////////////////////////////////////////////
                            if(list.get(listctr).Token.equals("[")){
                            //     int arrayFlag=0;
                            int result1[] = arrayAccess(listctr, iden, flag, fontfamily, arrayFlag,dtype);
                                                         flag=result1[0];
                                                         listctr=result1[1];
                            }
 /////////////////////////////////////END OF ARRAY ACCESS/////////////////////////////
 /////////////////////STRUCT ACCESS///////////////////////////////
                            else if(list.get(listctr).Token.equals(".")){
                               int result2[] = structAccess(listctr, iden, flag, fontfamily);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                            }
                            
    ///////////////////////////////END OF STRUCT ACCESS//////////////////////////////////////////////////
                            
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                            iden="";
                            iden = iden.concat("(");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                                     
                             if(list.get(listctr).Token.equals(")")){
                           
                            listctr++;
                            break;
                            }
                             else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%"))
                                 continue;
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')','+','-','*','/','%'. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                             
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                                        
                            }
                              else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%' ");
                                                 flag=1;      
                                                 break;
                         }
                            
                     
                                   
                                   }//WHILE 
                    
                               }//if arithexp3
                           }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%'. ");
                                                 flag=1;      
                                                 break;
                            }
                            ////////////////
                                
                            }
                            else{
                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or ')'. ");
                                                 flag=1;      
                                                 break;  
                            }
                 /////// END OF PARENTHESIS ARITHEXP1 ///
                            }
                            
                           
                             if(list.get(listctr).Token.equals(")")){
                           
                            listctr++;
                            break;
                            }
                             else if(list.get(listctr).Token.equals("+")||list.get(listctr).Token.equals("-")||
                              list.get(listctr).Token.equals("*")||list.get(listctr).Token.equals("/")||
                              list.get(listctr).Token.equals("%"))
                                 continue;
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')','+','-','*','/','%'. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                             
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;     
                                                
                                                 break;
                            }
                                        
                            }
                              else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%' ");
                                                 flag=1;      
                                                 break;
                         }
                            
                     
                                   
                                   }//WHILE 
                    
                               }//if arithexp3
                           }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting ')', '+','-','*','/','%', ';'. ");
                                                 flag=1;      
                             //                    break;
                            }
                            ////////////////
                                
                            }
                            else{
                            ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or ')'. ");
                                                 flag=1;      
                                     //            break;  
                            }
                 /////// END OF PARENTHESIS ARITHEXP1 ///
                            }
      
        }catch (BadLocationException ex) {
            Logger.getLogger(Frame.class.getName()).log(Level.SEVERE, null, ex);
        }
        return new int[] {flag, listctr};
      }
      public int[] structAccess(int listctr, String iden, int flag, String fontfamily){
        try {
            ArrayList<Lexical> list = ListLexical();
            ArrayList<Syntax> listSyntax = new ArrayList<Syntax>();
            
                iden="";
                            iden = iden.concat(".");
                            listSyntax.add(new Syntax("<structaccess> ",iden));
                            listctr++;
                             if(list.get(listctr).Token.equals("Identifier")){
                             iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<structaccess> ",iden));
                            listctr++;
                            
     ///////////////////////////////ARRAY ACCESS///////////////////////////////////////////////////
                            if(list.get(listctr).Token.equals("[")){
                                arrayFlag=0;
                            int result1[] = arrayAccess(listctr, iden, flag, fontfamily, arrayFlag,dtype);
                                                         flag=result1[0];
                                                         listctr=result1[1];
                            }
 /////////////////////////////////////END OF ARRAY ACCESS/////////////////////////////
 
                             }
                             else{
                                 ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier. ");
                                                        flag=1;
                                                      //  break;  
                             }
                             
              } catch (BadLocationException ex) {
            Logger.getLogger(Frame.class.getName()).log(Level.SEVERE, null, ex);
        }
        return new int[] {flag, listctr};
        }
      
    public int[] operand(int listctr, String iden, int flag, String fontfamily, int operandCtr){
        try {
            int logOperator =0, idctr=0;
            listctr++;
            ArrayList<Lexical> list = ListLexical();
            ArrayList<Syntax> listSyntax = new ArrayList<Syntax>();
            listctr--;
            System.out.print("\nHere it is: "+list.get(listctr).Token+"\n");
         if(list.get(listctr).Token.equals("Identifier")||list.get(listctr).Token.equals("Int Literal")||
                               list.get(listctr).Token.equals("Float Literal")||list.get(listctr).Token.equals("(")||list.get(listctr).Token.equals("true")
                 ||list.get(listctr).Token.equals("false")){
                            if(list.get(listctr).Token.equals("Identifier")){
                                logOperator =1;
                                idctr=1;
                            iden="";
                            iden = iden.concat("id");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                             System.out.print("Hess: "+operandCtr+" ");
                            ///////////////////////////////ARRAY ACCESS///////////////////////////////////////////////////
                            if(list.get(listctr).Token.equals("[")){
                           arrayFlag=0;
                            int result1[] = arrayAccess(listctr, iden, flag, fontfamily, arrayFlag,dtype);
                                                         flag=result1[0];
                                                         listctr=result1[1];
                            }
 /////////////////////////////////////END OF ARRAY ACCESS/////////////////////////////
 /////////////////////STRUCT ACCESS///////////////////////////////
                            else if(list.get(listctr).Token.equals(".")){
                              int result2[] = structAccess(listctr, iden, flag, fontfamily);
                                                         flag=result2[0];
                                                         listctr=result2[1];
                            }
                            
    ///////////////////////////////END OF STRUCT ACCESS//////////////////////////////////////////////////
                            }
                            else if(list.get(listctr).Token.equals("Int Literal")){
                            iden="";
                            iden = iden.concat("Int Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                        
                            
                            }
                              else if(list.get(listctr).Token.equals("true")){
                            logOperator=1;
                            listctr++;
                        
                            
                            }
                                else if(list.get(listctr).Token.equals("false")){
                           logOperator=1;
                            listctr++;
                        
                            
                            }
                            else if(list.get(listctr).Token.equals("Float Literal")){
                            iden="";
                            iden = iden.concat("Float Literal");
                            listSyntax.add(new Syntax("<operand> ",iden));
                            listctr++;
                            }
                            else if(list.get(listctr).Token.equals("(")){
                                    
                                    int result2[] = arithexp1(listctr, iden, flag, fontfamily, arrayFlag);
                                                         flag=result2[0];
                                                         listctr=result2[1];

                 /////// END OF PARENTHESIS ARITHEXP1 ///
                            }
                            //relop
                            
                            if(operandCtr==1&&flag==0){
                                
                                      if(list.get(listctr).Token.equals("==")||list.get(listctr).Token.equals("!=")||
                                              list.get(listctr).Token.equals("<")||list.get(listctr).Token.equals(">")
                                              ||list.get(listctr).Token.equals("<=")||list.get(listctr).Token.equals(">=")){
                                                        listctr++;
                                                        operandCtr=0;
                                                        
                                                         System.out.print("JJJ???"+list.get(listctr).Token+"GG");
                                                         int result[] = operand(listctr, iden, flag, fontfamily, operandCtr);
                                                         flag=result[0];
                                                  
                                                    }
                                      else if(logOperator==1&&(list.get(listctr).Token.equals("&&")||list.get(listctr).Token.equals("||"))){
                                           listctr++;
                                                        operandCtr=0;
                                                      
                                                         int result[] = operand(listctr, iden, flag, fontfamily, operandCtr);
                                                         flag=result[0];
                                      }
                                      
                                                  else{
                                          System.out.print("Helloo???");
                                          if(logOperator!=1&&idctr==0){
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '==', '<', '>', '<=', '>=', or '!='. ");
                                                        flag=1;}
                                          else if(logOperator==1&&idctr==0){
                                               ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '&&', '||'. ");
                                                        flag=1;
                                          }
                                          else if(idctr==1){
                                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting '==', '<', '>', '<=', '>=', or '!=', '&&', '||'. ");
                                                        flag=1; 
                                          }
                                                        
//                                                      
                                                 }
                                                  
                            }//if
                            
                            }
                            else{
                             ErrorTxt.setText("<body style=\"font-family: "+fontfamily+"\"> SYNTAX ERROR on Line <span style=\"color:red\"> "+list.get(listctr).Line+"</span>. Unexpected '<span style=\"color:red\">"+list.get(listctr).Lexeme+""
                             + "</span>'. Expecting Identifier, integer literal, Float Literal, or '('. ");
                                                 flag=1;      
                                                // break;
                            }
         
           } catch (BadLocationException ex) {
            Logger.getLogger(Frame.class.getName()).log(Level.SEVERE, null, ex);
        }
        System.out.print("fla[[["+flag+"]]\n");
        return new int[] {flag, listctr};
    }
  
    
    /**
     * @param args the command line arguments
     */
    public static void main(String args[]) { 
        /* Set the Nimbus look and feel */
        //<editor-fold defaultstate="collapsed" desc=" Look and feel setting code (optional) ">
        /* If Nimbus (introduced in Java SE 6) is not available, stay with the default look and feel.
         * For details see http://download.oracle.com/javase/tutorial/uiswing/lookandfeel/plaf.html 
         */
        try {
            for (javax.swing.UIManager.LookAndFeelInfo info : javax.swing.UIManager.getInstalledLookAndFeels()) {
                if ("Nimbus".equals(info.getName())) {
                    javax.swing.UIManager.setLookAndFeel(info.getClassName());
                    break;
                }
            }
        } catch (ClassNotFoundException ex) {
            java.util.logging.Logger.getLogger(Frame.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        } catch (InstantiationException ex) {
            java.util.logging.Logger.getLogger(Frame.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        } catch (IllegalAccessException ex) {
            java.util.logging.Logger.getLogger(Frame.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        } catch (javax.swing.UnsupportedLookAndFeelException ex) {
            java.util.logging.Logger.getLogger(Frame.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        }
        //</editor-fold>

        /* Create and display the form */
         java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Frame().setVisible(true);
            }
        });
    }

    // Variables declaration - do not modify//GEN-BEGIN:variables
    private javax.swing.JLabel Clear;
    private javax.swing.JLabel Clear1;
    private javax.swing.JEditorPane ErrorTxt;
    private javax.swing.JEditorPane ErrorTxt1;
    private javax.swing.JEditorPane ErrorTxt2;
    private javax.swing.JEditorPane ErrorTxt3;
    private javax.swing.JLabel Exit;
    private javax.swing.JLabel Exit1;
    private javax.swing.JLabel Lexical;
    private javax.swing.JTable LexicalTbl;
    public static javax.swing.JTextArea SourceTxt;
    private javax.swing.JLabel Syntax;
    private javax.swing.JTable SyntaxTbl;
    private javax.swing.JFrame jFrame1;
    private javax.swing.JFrame jFrame2;
    private javax.swing.JLabel jLabel1;
    private javax.swing.JMenu jMenu1;
    private javax.swing.JMenu jMenu2;
    private javax.swing.JMenu jMenu3;
    private javax.swing.JMenuBar jMenuBar1;
    private javax.swing.JPanel jPanel1;
    private javax.swing.JScrollPane jScrollPane1;
    private javax.swing.JScrollPane jScrollPane2;
    private javax.swing.JScrollPane jScrollPane3;
    private javax.swing.JScrollPane jScrollPane4;
    private javax.swing.JScrollPane jScrollPane5;
    private javax.swing.JScrollPane jScrollPane6;
    private javax.swing.JScrollPane jScrollPane7;
    private javax.swing.JScrollPane jScrollPane9;
    private javax.swing.JTabbedPane jTabbedPane1;
    private javax.swing.JTabbedPane jTabbedPane2;
    private javax.swing.JTabbedPane jTabbedPane3;
    private javax.swing.JTabbedPane jTabbedPane4;
    private javax.swing.JTabbedPane jTabbedPane5;
    private javax.swing.JTabbedPane jTabbedPane6;
    private javax.swing.JTabbedPane jTabbedPane7;
    private javax.swing.JTabbedPane jTabbedPane8;
    public static javax.swing.JScrollPane jsp;
    // End of variables declaration//GEN-END:variables
}
